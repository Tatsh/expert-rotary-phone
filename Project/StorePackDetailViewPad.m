...
// @ 0x4dae8 — build the whole iPad pack-detail panel: a grey, soft-shadowed
// card holding the pack header (jacket + name + comment + copyright + buy
...
- (id)initWithFrame:(CGRect)frame {
    ...
    // Match the main-screen scale on Retina (guarded so it is a no-op on pre-2x
    // OSes).
    ...
    packArtworkView =
        [[StoreImageView alloc] initWithFrame:CGRectMake(18.0f, 33.0f, 160.0f, 160.0f)];
    ...
    labelPackName.minimumScaleFactor =
        18.0f; // raw 0x41900000 in the binary (minimum-font-size semantics)
    ...
    for (int i = 0; i < 4; i++) {
        ...
        [row.buttonLink addTarget:self
                           action:@selector(handleLink:)
                 forControlEvents:UIControlEventTouchUpInside];
        [row.buttonSample addTarget:self
                             action:@selector(handleSample:)
                   forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:row];
    }

    // Start blank/hidden (nils the labels, hides the rows, resets the buy button,
    // etc.).
    [self removePackInfo];
    ...
    // --- loading spinner, centred a little above the middle (attached when a
    // fetch begins) ---
    ...
    // --- artist-site "web" button, right-anchored at x=628, hidden until a URL
    // is present ---
    ...
    // --- dummy cover VC: a 325x212 transparent panel (centred on self) with a 2x
    // grey spinner,
    //     shown over a song row while a recommend-pack registration POST is in
    //     flight ---
    dummyView = [[UIViewController alloc] init];
    dummyView.view.frame = CGRectMake(325.0f,
                                      438.0f,
                                      325.0f,
                                      212.0f); // overridden by the centre below
    ...
    UIActivityIndicatorView *coverSpinner =
    ...
    // The binary factors this into a -[UIView setAutoresizingCenter] category
    // (keep the loading spinner centred on resize); inlined here as the
    // equivalent flexible-margin mask.
...
// packInfo (@ 0x50b48) / setPackInfo: (@ 0x50b58, objc_setProperty) are
// synthesized in the binary; the addresses are annotated on the @property in
// the header.

// @ 0x4f680 — kick the pack-detail download. No-op without a bound pack. If the
// pack already carries its song list (musicInfos != nil), just tint the card
...
- (void)loadInfo {
    ...
    if ([m_PackInfo musicInfos] == nil) {
        ...
        [m_StorePackInfoDownloader
            downloadDetail:YES]; // @0x4f806 movs r2,#1 (user-open; pad passes YES)
    } else {
...
// @ 0x4f318 — populate the detail card from the bound pack (once). Tint the
// background, fill in the name / comment / copyright / jacket URL, pick the
// right buy-button label, reveal the pack header, then for each of the 4 rows:
// if a song exists at that index bind it (flagging whether a playable .acv
// chart is present by matching acMusicId against the pack's acvMusicInfos) and
...
- (void)showPackInfo {
    ...
    for (int i = 0; i < 4; i++) {
        if (i < [musicInfos count]) {
            ...
            // Is there an .acv (playable chart) for this song? Scan the pack's
            // acvMusicInfos for one whose acMusicId matches this song's musicID.
            BOOL isExistAcv = NO;
...
// @ 0x4f1d0 — "installed" state: if already recommended, fall through to the
// greyed-out "INSTALLED" (setButtonTextInstalledForce); otherwise offer the
...
- (void)setButtonTextInstalled {
    ...
    NSString *title = [[NSBundle mainBundle] localizedStringForKey:@"友達に勧める"
                                                             value:@""
                                                             table:nil];
    [buttonPurchase setTitle:title forState:UIControlStateNormal];
    [buttonPurchase setEnabled:YES];
...
// @ 0x4ecd0 — abort a pending pack-detail fetch. Re-points the downloader's
// delegate at this view (so any late callback is handled here, not by a stale
// delegate) before cancelling, then releases + nils it. The (id) cast keeps the
...
// @ 0x4fdf0 — a row's sample button was tapped. Find the row, then toggle: stop
...
- (void)handleSample:(id)sender {
    for (int i = 0; i < 4; i++) {
        ...
        m_SampleDownloader = [[Downloader alloc] initWithURL:[NSURL URLWithString:info.sampleURL]
                                                    delegate:(id)self];
...
// @ 0x50154 — the age-gate modal closed with a birthday now on record. Drop the
...
// @ 0x50278 — Downloader delegate: a fetch finished. Two flows share this
// callback:
//   * the sample-preview downloader -> play the clip as looping BGM (no fade)
//   and flip the
//     row to its "playing" state, then drop the downloader;
//   * the recommend-registration POST -> on success persist the pack as
//   recommended, mark the
...
- (void)downloaderFinished:(Downloader *)downloader {
    ...
    if ([json objectForKey:@"ErrorCode"] == nil) {
        ...
        // Award 300 treasure points, capped at 9999 (Ghidra: (tp+300)<<16 vs
        // 9999<<16).
        ...
        // Byte-verified: title "Error", message the network-connection failure
        // text.
...
// @ 0x4eaa0 — clear the panel back to its empty state (before binding a new
...
// @ 0x4f828 — the purchase button's decision tree. Stop the preview audio, then
// branch on whether the pack is already owned:
//   * owned but not fully downloaded -> ask the host to re-download it;
//   * owned + downloaded + not yet recommended -> POST it to the "recommend"
//   endpoint;
//   * not owned -> if a birthday is on record (or the gate was cancelled) run
//   the spending-
...
// m_BirthDayView == nil age-gate block @0x4faac) after `isPurchasable` returns
// false, so that block is reached both from the outer else (bday == nil &&
// !cancelled) and from the cancelled-but-not-purchasable-with-no-birthday case.
// The reconstruction below only opens the age gate from the outer else, so when
// the birthday is cancelled, absent, and the price is over the limit it does
// nothing instead of re-showing the gate. Verify and restructure before marking.
- (void)doPurchase:(id)sender {
    ...
    if ([pm isPurchased:productID]) {
        if (![self allDownloaded]) {
            ...
            NSString *body =
                [NSString stringWithFormat:@"uuid=%@&pack_id=%d", uuid, m_PackInfo.packID];
            recommendDownloader =
...
// @ 0x50840 — StorePackInfoDownloader delegate: the detail fetch failed. Stop +
...
// @ 0x50990 — teardown. The binary is MRC: it releases packView, the 4 song
// rows, the jacket, the labels, the copyright/loading views and spinner, both
// downloaders, packInfo, the birthday modal and the cached recommend-id array,
