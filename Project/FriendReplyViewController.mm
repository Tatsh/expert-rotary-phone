...
//  pop'n rhythmin
//
//  See FriendReplyViewController.h. Reconstructed from Ghidra project rb420, program PopnRhythmin
//  (initWithStyle: @ 0xa7854, viewDidLoad @ 0xa8060, didReceiveMemoryWarning @ 0xa81c0,
//  dealloc @ 0xa81ec, startReplyFriendHttp:reply: @ 0xa82d8, numberOfSectionsInTableView: @ 0xa8434,
//  tableView:numberOfRowsInSection: @ 0xa8438, tableView:cellForRowAtIndexPath: @ 0xa8460,
//  tableView:titleForHeaderInSection: @ 0xa8580, tableView:didSelectRowAtIndexPath: @ 0xa8584,
//  downloaderFinished: @ 0xa8598, downloaderProceed: @ 0xa861c, downloaderError: @ 0xa8620,
...
// @ 0xa7854 — grouped table styling, message header, loading overlay, back button, placeholder.
...
// @ 0xa82d8 — accept (reply == 1) / reject (reply == 0) a request: POST and show the loading cover.
- (void)startReplyFriendHttp:(NSString *)playerId reply:(int)reply {
    ...
    NSString *body = [NSString stringWithFormat:@"uuid=%@&player_id=%@&reply=%d",
                      AppDelegate.appDelegate.uuId, playerId, reply];
    dlReplyFriend = [[Downloader alloc]
        initWithURL:[StoreUtil replyFriendURL]
           delegate:self
               Post:[body dataUsingEncoding:NSUTF8StringEncoding]
        ContextType:@"application/x-www-form-urlencoded"];
...
// @ 0xa8598 — dispatch a completed download to the right handler, then hide the loading cover.
...
// @ 0xa8dc0 — reply POST done: on success drop the replied row + update headers/badge; alert either way.
