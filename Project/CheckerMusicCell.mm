...
//  .mm because init/-setData: reach the C++ engine bridge (neSceneManager::isPadDisplay()).
//
...
// @ 0xd1ecc — tear down recycled subviews, install the banner background (device/OS
// dependent), then rebuild the date / title / genre labels from the score record.
- (void)setData:(ArcadeScoreData *)scoreData {
    _scoreData = scoreData;

    // Reuse cleanup.
    [_dateLbl removeFromSuperview];  _dateLbl = nil;
    [_titleLbl removeFromSuperview]; _titleLbl = nil;
    [_genreLbl removeFromSuperview]; _genreLbl = nil;
    ...
    if (!isPad) {
        ...
        if (!isOS7) {
            ...
            // iOS 7+ phone: center the banner inside a full-width (59pt tall) background view.
            UIView *container = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f,
                                                                         self.bounds.size.width,
                                                                         59.0f)];
            CGFloat cw = container.frame.size.width;
            [bg setFrame:CGRectMake((cw - baseImg.size.width) * 0.5f, 0.0f,
                                    baseImg.size.width, container.frame.size.height)];
            [container addSubview:bg];
            self.backgroundView = container;
        }
    } else {
        // iPad: fixed-size banner (300x59) at bgX, kept as a content subview.
        UIImageView *bg = [[UIImageView alloc] initWithFrame:CGRectMake((CGFloat)bgX, 0.0f,
                                                                        300.0f, 59.0f)];
        [bg setImage:baseImg];
        _bgImg = bg;
    ...
    _dateLbl.textColor = [UIColor colorWithRed:(195.0f / 255.0f) green:(51.0f / 255.0f) blue:(1.0f / 255.0f) alpha:1.0f];
    ...
    _titleLbl.textColor = [UIColor colorWithRed:(87.0f / 255.0f) green:(81.0f / 255.0f) blue:(76.0f / 255.0f) alpha:1.0f];
    ...
    _genreLbl.textColor = [UIColor colorWithRed:(245.0f / 255.0f) green:(245.0f / 255.0f) blue:(245.0f / 255.0f) alpha:1.0f];
