...
//  pop'n rhythmin
//
//  See FriendRequestViewController.h. Reconstructed from Ghidra project rb420, program PopnRhythmin:
//    init                                                      @ 0xb1c08
//    dealloc                                                   @ 0xb27bc
//    viewDidLoad                                               @ 0xb28ac
...
//     "uuid=<uuId>&player_id=<typedId>&message=", Content-Type "application/json". A 200 with an
//     empty (non-JSON) body is treated as success; a JSON body carries a numeric "ErrorCode".
//   - -downloadMainFinished: is the DownloadMainDelegate cancel-friend callback: the controller
//     registers itself as +[DownloadMain getInstance].delegateCancelFriend in -viewDidLoad (and
//     detaches in -dealloc). That protocol is adopted privately in the class extension below; the
//     public header exposes only <UITextFieldDelegate, DownloaderDelegate>, matching the binary's
//     class protocol list.
//   - All alert strings are exact CFString decodes (UTF-16LE). Title "フレンド申請"; OK button.
//     The ErrorCode->message table (codes 0/2/7 reuse the transport-failure string; the isNumber
//     check failing shows the title with a nil message, exactly as the binary does).
//   - init's subview coordinates use the binary's exact offsets. iPad shifts everything by
//     (ox=15, oy=44@iOS7); the top-right spinner sits at view.width-36 (DAT_000b2680 = -36.0) on
//     phone / x=214 on iPad.
//
...
// @ 0xb1c08 — lay out the own-id label, the target-id field + request button, the spinner, the
// nav-bar buttons (back + recommended-friend list), and the embedded sent-requests table.
- (instancetype)init {
    if (!(self = [super init])) {
        return nil;
    }
    ...
    // iPad shifts the whole form; iVar10 (horizontal) / iVar13 (vertical) offsets.
    int ox = isPad ? 15 : 0;
    ...
    _playerIdField = [[UITextField alloc]
        initWithFrame:CGRectMake(ox + 43, oy + 119, 206.0f, 38.0f)];
    ...
    UIButton *freeBtn = [[UIButton alloc]
        initWithFrame:CGRectMake(0, 0, freeImg.size.width, freeImg.size.height)];
    [freeBtn setBackgroundImage:freeImg forState:UIControlStateNormal];
    [freeBtn addTarget:self action:@selector(touchedFreeRequestButton:)
      forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItem =
        [[UIBarButtonItem alloc] initWithCustomView:freeBtn];
...
// @ 0xb28ac — register as the cancel-friend delegate so a cancel from a row updates this screen.
- (void)viewDidLoad {
    [super viewDidLoad];
...
// @ 0xb2960 — cap the entry at 7 characters (the resulting string must stay under 8).
...
// @ 0xb29c8 — validate the typed id and fire the request (blocked while not the top VC on phone).
- (void)touchedRequestButton:(id)sender {
...
- (void)touchedFreeRequestButton:(id)sender {
    ...
    [self.navigationController.navigationBar
        setBackgroundImage:[UIImage imageNamed:@"fpl_navbar"]
...
// @ 0xb2ccc — request result. Empty (non-JSON) body = success (clear the field + reload the sent
// list); a JSON "ErrorCode" maps to a message. Always drops the downloader, stops the spinner, and
// shows the "フレンド申請" alert.
- (void)downloaderFinished:(Downloader *)downloader {
    ...
    // The binary shows the alert unconditionally (a nil message just yields a titled, empty card).
    CommonAlertView *alert = [[CommonAlertView alloc]
        initWithTitle:@"フレンド申請"
              message:message
             delegate:nil
...
// @ 0xb2ecc — request failed: drop the downloader, stop the spinner, show the failure alert.
...
// @ 0xb2f98 — a friend request was cancelled elsewhere: acknowledge and reload the sent list.
...
// @ 0xb317c — back button: restore the friend-hub nav bar art and pop (only when top VC).
- (void)backButtonFunc {
    if (self.navigationController.topViewController != self) {
        return;
    }
...
// @ 0xb27bc — cancel the in-flight request (so no late callback fires into a dead controller) and
// detach from DownloadMain's cancel-friend delegate. Kept under ARC because it cancels a Downloader
// and clears a non-owning delegate reference; the object-ivar releases are ARC-managed.
- (void)dealloc {
    if (_downloader != nil) {
        [_downloader cancel];
    }
