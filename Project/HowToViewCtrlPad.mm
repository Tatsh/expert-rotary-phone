...
//  pop'n rhythmin
//
//  See HowToViewCtrlPad.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin. The iPad how-to overlay differs from the phone HowToViewCtrl:
//  it installs a dimmed, tappable cover view in -viewDidLoad, builds the
//  centred paging strip and custom page-dot images lazily in -viewDidAppear:,
//  and opens / closes with a UIView fade animation instead of a navbar back
//  button. Several frame origins are NEON-spilled in the decompile and are
//  flagged where reconstructed. Uses the neEngine bridge (.mm) for the cancel /
//  page system SE.
//
...
- (void)viewDidLoad {
    ...
    // Dimmed full-screen cover: 50% black, exclusive-touch, with a tap recogniser
    // that closes.
    ...
    // Page tracker, kept hidden (the visible dots are the custom _pageImgs
    // strip).
...
// @ 0x16b40 — build the centred paging strip, the scroll view, the (hidden)
// multi-page page control and the custom page-dot strip once the view is on
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    // The pages are sized to the first how-to image; the strip is count images
    // wide.
    ...
    _scrollView =
        [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, pageSize.width, pageSize.height)];
    ...
    // With more than one page, size the page tracker (kept hidden — the custom
    // strip is visible).
    if (count > 1) {
    ...
    CGFloat dotsCenterY =
        _scrollView.frame.origin.y + _scrollView.frame.size.height + 20.0f +
        dotImg.size.height * 0.5f; // 20.0: vmov.f32 d16,#0x41a00000 @ 0x17038 (exact)
