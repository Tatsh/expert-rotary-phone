...
// One friend-score row, as wrapped in the NSValue passed to -setScoreData:. Obj-C
...
// Rank "place" badges (1st..9th) for rows 0..8; two-digit rows compose digit glyphs.
...
static NSString *const kScoreImg[4] = {
    @"frisco_sco_1st", @"frisco_sco_2nd", @"frisco_sco_3rd", @"frisco_sco_cmn",
};
// Score-rank badge, keyed by scoreToRank() (0 = perfect-S .. 6 = D).
static NSString *const kScoreRankImg[7] = {
    @"frisco_rank_perfect_s", @"frisco_rank_aaa", @"frisco_rank_aa",
    @"frisco_rank_a", @"frisco_rank_b", @"frisco_rank_c", @"frisco_rank_d",
};

// Score -> rank index (0 best .. 6 worst). The binary shares one routine (Ghidra
// FUN_00028a40, also reconstructed file-local in PlayScene.mm).
...
@implementation FriendScoreTableCell {
    ...
    // Built lazily in setScoreData: and torn down on reuse. Every one is owned by its
...
// @ 0xae288 — render one friend-score row from an NSValue-wrapped ScoreDataStruct. Rebuilt
// on every reuse, so it first strips the subviews it built last time. Phone uses the iOS6/7
// x-offset ivars above; iPad uses its own fixed metrics (only the pre-iOS7 -10pt shift,
// folded into `padX0`, carries over). The self row (nil name) shows the "you" marker.
- (void)setScoreData:(NSValue *)scoreData {
    ...
    if (_scoreRankImgView)     { [_scoreRankImgView removeFromSuperview];     _scoreRankImgView = nil; }
    if (_fullcomboMarkImgView) { [_fullcomboMarkImgView removeFromSuperview]; _fullcomboMarkImgView = nil; }

    // Row background image. On phone it is installed as the cell's backgroundView; on iPad
    // it is added to the content view (shifted right by 45pt) and also parents every other
    // element on the row.
    ...
    if (!neSceneManager::isPadDisplay()) {
        ...
        if (data.playerId == nil || data.name != nil) {
            if (!data.isNotice) {
                if (data.score == -1)    baseName = @"frisco_base_player_04";
                else if (data.rank == 2) baseName = @"frisco_base_player_03";
                else if (data.rank == 1) baseName = @"frisco_base_player_02";
                else if (data.rank == 0) baseName = @"frisco_base_player_01";
                else                     baseName = @"frisco_base_player_04";
            } else {
    ...
    // "You" marker — phone only, on the local player's own row (playerId set, name nil).
    ...
    // Rank badge (only when a score exists). Rows 0..8 use a single "place" glyph; rows 9+
    // compose the one-based place number from two digit glyphs.
    if (data.score != -1) {
    ...
    if (data.playerId != nil) {
        ...
        [_charaBgImgView setImage:[UIImage imageNamed:(data.rank < 3 ? kCharaIconBg[(int)data.rank]
                                                                     : @"frisco_icon_cmn")]];
    ...
    // Player name, score plaque and score value (only when a player occupies the row).
    if (data.playerId != nil) {
        ...
        // The self row carries no name; fall back to the saved player name (or "YOU").
        if (data.name == nil) {
        ...
        _scoreBaseImgView = [[UIImageView alloc]
            initWithFrame:CGRectMake(baseX, baseY, 135.0f, 20.0f)];
...
// dealloc @ 0xae25c — ARC-omitted (chains to super only; every subview is owned by its
