...
//  pop'n rhythmin
//
//  See UIView+neSystemAddFunc.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin. Float words decoded from the decompile:
...
//  ARC: the CGPath handle is a C object, released with CGPathRelease (not ARC-managed).
//
...
- (void)SetFlashEffectDuration:(float)duration Start:(float)startOpacity End:(float)endOpacity {
    ...
    anim.timingFunction =
        [CAMediaTimingFunction functionWithControlPoints:0.5f:0.0f:0.75f:0.8f];
...
- (void)SetJumpEffectBaseX:(float)baseX BaseY:(float)baseY {
    ...
    // Four upward bounces with decreasing amplitude, each a cubic curve whose control
    // points sit at the peak so the view rises to (baseY + amp) and falls back to baseY.
    ...
    anim.timingFunction =
        [CAMediaTimingFunction functionWithControlPoints:0.25f:0.1f:0.5f:0.5f];
    anim.repeatCount = FLT_MAX;              // 0x7f7fffff
    anim.removedOnCompletion = NO;
    CGPathRelease(path);
...
- (void)setPopupEffect {
    ...
    anim.values = @[
        ...
        [NSValue valueWithCATransform3D:CATransform3DScale(base, 1.2f, 1.2f, 1.0f)],   // 0x3f99999a
        [NSValue valueWithCATransform3D:CATransform3DScale(base, 0.9f, 0.9f, 1.0f)],   // 0x3f666666
