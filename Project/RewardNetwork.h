...
// Campaign flag from NSUserDefaults, gated on ad-tracking + initializeFlg==1;
// -2 otherwise. @ 0xee448
+ (int)campaignFlg;
...
// Persist the session appli URL / parameters / method into NSUserDefaults. @
// 0xee804
+ (void)setSessionParameters:(id)parameters url:(NSString *)url method:(NSString *)method;

// Start a reward session: check login, then run `block` with any error. @
// 0xeed2c
+ (void)startSessionWithBlock:(RewardNetworkErrorBlock)block;

// Create/register the device UDID, writing to the first empty storage slot. @
// 0xef274
...
+ (void)clearUDID;
// Delete the legacy keychain UDID + reset session state if all UDIDs gone. @
// 0xf2fb4
...
// Report whether the banner is enabled (uses the banner cache when fresh). @
// 0xf3714
...
// initializeFlg getter also gates on ad-tracking being enabled (returns 0 if
// not). @ 0xee3f8
- (int)initializeFlg;
...
// Expiring key/value cache backed by NSUserDefaults (archived {Value,Expire}).
// @ 0xf2030
...
// Fetch the installed-appli id list (GET
// /reward/app/install/appliid/index.php). @ 0xf22e0
- (void)appliIdListWithType:(int)type callback:(RewardNetworkCallback)callback;

// Report installed applis in batches of 10 (POST
// /reward/app/install/report/regist.php), chaining the remainder. @ 0xf25fc
// NOTE: the completion is error-only in the binary (invoked with a single
// NSError* argument).
...
// @ 0xf2a48 NOTE: the completion is error-only in the binary (invoked with a
// single NSError* argument).
...
// Ensure the SDK is started (reads the persisted appliId/URL/method/env), then
// run `block`; reports a parameter error if any default is missing. @ 0xef058
+ (void)startWithBlock:(void (^)(NSError *error))block;

// Start the applilink SDK for `appliId` in environment `env`: persists them,
// ensures the reward UDID exists, and posts the install record; `callback`
// receives nil / a localized error. @ 0xee8f0
