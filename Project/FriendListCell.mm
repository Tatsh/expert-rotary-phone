...
//  setFriendData:rank:isBestScoreSort: @ 0xb34c0). Objective-C++ for the
//  neSceneManager::isPadDisplay() device check.
//
...
// Rank "place" badges (1st..9th) used for rows 0..8; two-digit rows compose
// these digits.
...
// Score plaque, keyed by place (1st/2nd/3rd/common), separate art for total vs.
// best-score sort.
...
@implementation FriendListCell {
    ...
    // Built lazily in setFriendData:rank:isBestScoreSort: and torn down on reuse.
    // Each is added to a superview (which owns it) and nilled on reuse; ARC needs
    // no explicit dealloc.
...
// @ 0xb34c0 — render one ranking row from an NSValue-wrapped FriendListData.
// `rank` is the 0-based row (0 == 1st place); `isBestScoreSort` selects which
...
- (void)setFriendData:(NSValue *)friendData rank:(int)rank isBestScoreSort:(BOOL)isBestScoreSort {
    ...
    // Row background: on phone it is the cell's backgroundView; on iPad it is a
    // plain subview of the content view (shifted left 10pt on the pre-iOS7
    // metrics) and also acts as the parent for every other element (so the whole
    // row moves as a unit).
    ...
    // Everything else parents to the content view on phone, or the background
    // image on iPad.
    ...
    // Rank badge: rows 0..8 use a single "place" badge; rows 9+ compose two digit
    // glyphs from the one-based place number (row 9 -> "10").
    if (rank < 9) {
    ...
    _playerNameLbl.font = [UIFont fontWithName:@"BullyBold"
    ...
    // Binary sends -setMinimumScaleFactor: the literal 14.0 (0x41600000). That is
    // out of the documented 0..1 range for scale factor and reads like the value
    // the deprecated -setMinimumFontSize: expected; reproduced verbatim to match
    // the binary.
    ...
    _scoreLbl.font = [UIFont fontWithName:@"BullyBold"
                                     size:18.0f]; // getFontNameBullyBold() @ 0x5ef90
    _scoreLbl.textAlignment = NSTextAlignmentRight;
    _scoreLbl.text = [NSString stringWithFormat:@"%d", score];
    [parent addSubview:_scoreLbl];
}

// dealloc @ 0xb3494 — ARC-omitted (chains to super only; every subview is owned
// by its superview, so nothing needs releasing here).
