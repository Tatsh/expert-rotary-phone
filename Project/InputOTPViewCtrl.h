 ...
 * a decide button, and a custom back button, plus a dimmed dummy cover and activity spinner used
 * while the parent runs the score sync. Entering a non-empty code calls back
 * -startGetArcadeScoreHttpWithOtp: on the owning CheckerCategoryViewController and pops.
 *
 * Reconstructed from Ghidra project rb420, program PopnRhythmin (initWithCategoryView: @ 0x78d18
 * and the button, text-field, and keyboard flow).
 *
 * The implementation is .mm because it drives the C++ "ne" engine singletons via neEngineBridge:
 * the decide and cancel system SE and the scene-manager root view controller callback.
 */
