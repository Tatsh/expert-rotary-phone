...
//  cover + activity spinner used while the parent runs the score sync. Entering a
//  non-empty code calls back -startGetArcadeScoreHttpWithOtp: on the owning
//  CheckerCategoryViewController and pops.
//
//  Reconstructed from Ghidra project rb420, program PopnRhythmin
//  (initWithCategoryView: @ 0x78d18 and the button / text-field / keyboard flow).
//
//  .mm because it drives the C++ "ne" engine singletons via neEngineBridge (the
//  decide / cancel system SE and the scene-manager root view controller callback).
//
...
@interface InputOTPViewCtrl : UIViewController <UITextFieldDelegate> {
    CheckerCategoryViewController *_categoryView;  // owner; receives -startGetArcadeScoreHttpWithOtp:
