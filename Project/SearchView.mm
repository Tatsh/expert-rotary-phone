...
//  pop'n rhythmin
//
//  Objective-C++ because it drives the C++ engine bridge (neEngine /
//  neSceneManager). Reconstructed from Ghidra project rb420, program
//  PopnRhythmin. Every method cites the address it was decompiled from. ARC:
//  the binary's manual retain/release/autorelease and [super dealloc] are
//  dropped; object stores are left to ARC.
//
...
static NSString *const kNetworkErrorMessage =
...
// .cxx_construct @ 0x88b04 — compiler-emitted C++ ivar constructor; not
// hand-written.
...
- (id)initAtNavigationController __attribute__((objc_method_family(none))) {
    ...
    // cannot assign self — but UIViewController init returns the same receiver in
    // place, so self stays valid and matches the binary's IVar4.
    if (![super init]) {
        return nil;
    }
    ...
    UIButton *posButton =
        [[UIButton alloc] initWithFrame:CGRectMake(0, 0, posSize.width, posSize.height)];
    ...
    [posButton addTarget:self
        ...
        forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:posButton];
...
- (void)viewDidLoad {
    ...
    // Fill the container on rotation (the binary calls a UIView
    // "setAutoresizingAll" category helper; inlined here as the equivalent full
    // flexible mask to avoid a category seam).
    ...
    MKCoordinateRegion initialRegion =
        MKCoordinateRegionMake(CLLocationCoordinate2DMake(kInitialLatitude, kInitialLongitude),
    ...
    if (!m_MessageLabel) {
        ...
        m_MessageLabel.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin |
    ...
    if (!m_ErrorLabel) {
        ...
        m_ErrorLabel.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin |
                                        UIViewAutoresizingFlexibleRightMargin |
                                        UIViewAutoresizingFlexibleBottomMargin;
...
// NOTE: -didReceiveMemoryWarning (@ 0x861f8) and -viewWillDisappear: (@
// 0x86224) exist in the binary but only forward to super with no added
// behaviour, so they are intentionally omitted.
...
- (void)showError:(NSString *)message {
    ...
    if (m_ErrorLabel.isHidden) {
        ...
        [UIView animateWithDuration:kErrorFadeDuration
                         animations:^{
...
- (void)startSearchMaster {
    ...
    m_MasterDownloader = [[Downloader alloc] initWithURL:[StoreUtil searchMasterURL] delegate:self];
...
- (void)startGameCenter:(MKCoordinateRegion)region {
    ...
    m_ListDownloader = [[Downloader alloc] initWithURL:[StoreUtil searchURL]
                                              delegate:self
...
- (void)onCurrentPosButton {
    ...
    if (![self gotoCurrentPosition]) {
        CommonAlertView *alert = [[CommonAlertView alloc] initWithTitle:kLocationDisabledTitle
...
- (void)mapView:(MKMapView *)mapView regionDidChangeAnimated:(BOOL)animated {
    ...
    if (region.span.longitudeDelta <= kMarkerSpanThreshold) {
        ...
        // Drop any pin that has scrolled out of the visible rect (never the user
        // dot).
        ...
        if (region.center.latitude > kJPLatMin && region.center.latitude < kJPLatMax &&
            ...
            CLLocation *last = [[CLLocation alloc] initWithLatitude:m_LastRegion.center.latitude
        ...
        // Zoomed out too far: strip every pin (except the user dot) and show the
        // prompt.
        for (id<MKAnnotation> annotation in m_Map.annotations) {
            if (m_Map.userLocation != annotation) {
                [m_Map removeAnnotation:annotation];
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation {
    ...
    MKAnnotationView *view = [mapView dequeueReusableAnnotationViewWithIdentifier:model];
    if (view) {
    ...
    // Resolve the marker image: this model's master image, else the "sunny"
    // fallback.
    UIImage *image = nil;
...
- (void)mapView:(MKMapView *)mapView
    ...
    m_GoogleMapURL =
        [[NSString alloc] initWithFormat:@"http://map.google.com/maps?q=%0.6f,%0.6f+(%@)",
                                         coordinate.latitude,
                                         coordinate.longitude,
                                         urlEncodeString(title)];
    ...
    CommonAlertView *alert = [[CommonAlertView alloc] initWithTitle:view.annotation.title
    ...
    if (m_ListDownloader == downloader) {
        ...
        if (json) {
            ...
            if ([list isKindOfClass:[NSArray class]]) {
                MKCoordinateRegion region = MKCoordinateRegionMake(CLLocationCoordinate2DMake(0, 0),
                ...
                for (id item in list) {
                    ...
                    // Pick the marker model: the arcade's model with the lowest master
                    // index (falls back to its first string model).
        ...
        if (!json) {
            ...
            if (![version isKindOfClass:[NSString class]]) {
                [self showError:kDataErrorMessage];
            } else if (!appVersion || [appVersion compare:version
                    ...
                    for (id item in mark) {
                        ...
                        if ([order isKindOfClass:[NSString class]] &&
                            ...
                            [m_Models addObject:[NSMutableDictionary
                                                    dictionaryWithObjectsAndKeys:order,
                                                                                 @"Order",
                                                                                 model,
                                                                                 @"Model",
                                                                                 name,
