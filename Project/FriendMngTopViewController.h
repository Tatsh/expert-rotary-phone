...
// The hosting controller (MainViewController on iPhone; the split hub on iPad)
// that section taps / close are forwarded to. Synthesized accessors backing
// m_Delegate. Ghidra: delegate @ 0xa6c00 / setDelegate: @ 0xa6c10.
@property(nonatomic, assign) id delegate;

// Build the hub view + its navigation controller (custom back button, section
// buttons) and return the navigation controller. Ghidra:
// initAtNavigationController @ 0xa59f0.
- (UINavigationController *)initAtNavigationController __attribute__((objc_method_family(none)));

// Fade the hub (and its nav view) in / out over 0.5 s. Ghidra:
// startOpenAnimation @ 0xa6590 / startCloseAnimation.
- (void)startOpenAnimation;
- (void)startCloseAnimation;

// Section buttons. Ghidra selectors onListButtonTouched: /
// onRequestButtonTouched: / onReplyButtonTouched:.
