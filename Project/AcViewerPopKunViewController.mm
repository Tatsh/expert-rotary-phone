...
//  This one edits POP-KUN (two values: OFF, BEAT POP). Objective-C++ (.mm) because it
//  drives the C++ "ne" engine singletons via neEngineBridge (the scene manager
//  pad-display flag, the AC-viewer event-center selection that seeds the header, and
//  the system-SE hooks).
//
...
// @ 0x7d01c — build the list: a transparent, separator-less UITableView; a back button;
...
// @ 0x7d8f0 — after loading, poke the scene manager (populates the pad-display flag).
- (void)viewDidLoad {
    [super viewDidLoad];
...
// @ 0x7da68 — a new value: store it, refresh, play the decide SE and pop back to the
...
// @ 0x7db38 — BACK: (on a real tap) play the cancel SE, refresh the option list behind
