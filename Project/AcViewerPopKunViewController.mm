//
//  AcViewerPopKunViewController.mm
//  pop'n rhythmin
//
...
//  AcViewerOptionViewController. This one edits POP-KUN (two values: OFF, BEAT
//  POP). Objective-C++ (.mm) because it drives the C++ "ne" engine singletons
//  via neEngineBridge (the scene manager pad-display flag, the AC-viewer
//  event-center selection that seeds the header, and the system-SE hooks).
//
...
@implementation AcViewerPopKunViewController

// @ 0x7d01c — build the list: a transparent, separator-less UITableView; a back
...
- (instancetype)init {
    if (!(self = [super initWithStyle:UITableViewStyleGrouped])) { // Ghidra:
                                                                   // initWithStyle:1
        return nil;
    }
...
// @ 0x7d8f0 — after loading, poke the scene manager (populates the pad-display
// flag).
- (void)viewDidLoad {
    [super viewDidLoad];
...
// @ 0x7d924 — two POP-KUN values in section 0.
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return (section == 0) ? 2 : 0;
}

// @ 0x7d930 — one AcViewerDetailCell per value (reused by "Cell%ld_%ld"), bound
// to the POP-KUN option kind (1) and the row's value label.
...
// @ 0x7da60 — no section headers.
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    return nil;
}

// @ 0x7da64 — no accessory (private UITableView delegate hook).
- (UITableViewCellAccessoryType)tableView:(UITableView *)tableView
         accessoryTypeForRowWithIndexPath:(NSIndexPath *)indexPath {
    return UITableViewCellAccessoryNone;
}

// @ 0x7da68 — a new value: store it, refresh, play the decide SE and pop back
...
// @ 0x7db38 — BACK: (on a real tap) play the cancel SE, refresh the option list
