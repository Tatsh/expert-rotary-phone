 ...
 * the two class factories below.
 *
 * +localizedRewardNetworkErrorWithCode:userInfo: (@ 0xf3f00) builds an NSError in the
 * "ApplilinkErrorDomain", filling NSLocalizedDescriptionKey from a lazily built, process-wide
 * message table whose strings come from the SDK's localised "Error" table, via the
 * NSBundle+RewardNetwork `rewardBundle` category.
 */
