...
/**
 * The cached playable local-song array, rebuilt when its dirty flag is set.
...
/**
 * The cached arcade-song array, rebuilt when its dirty flag is set.
...
/**
 * Every treasure song bundled with the app, one per main map.
 * @return An array of MusicData.
...
/**
 * Linear-search the cached local-song array by its @c MusicID.
...
/**
 * Linear-search the cached arcade-song array by its @c acMusicId.
...
/**
 * Build the @c %09d.orb data-file name for a local song. Class method: stateless.
...
/**
 * Build the @c ac%09d.acv data-file name for an arcade song.
...
/**
 * Rebuild the local-song cache from all unlock sources and apply level patches, clearing the
 * dirty flag.
...
/**
 * Rebuild the arcade-song cache, clearing the dirty flag.
 * @ghidraAddress 0xcaabc
 */
- (void)createAcMusicDataArray;

/**
 * Load the downloadable @c rhythmin_lv level patches.
 * @ghidraAddress 0xcb610
 */
- (void)createMusicLvPatchArray;

/**
 * Build the always-available bundled song table (IDs {1, 2, 3}).
 * @ghidraAddress 0xc8384
 */
- (void)createDefaultMusics;

/**
 * Build the treasure-song table, gated per main map.
 * @ghidraAddress 0xc8440
 */
- (void)createOpenTreasureMusics;

/**
 * Build the invite-reward song table (id 4), gated by the invite predicate.
 * @ghidraAddress 0xc8554
 */
- (void)createOpenInviteMusics;

/**
 * Build the BEMANI-collabo song table (id 5), gated by the collabo predicate.
 * @ghidraAddress 0xc8604
 */
- (void)createOpenCollaboMusics;

/**
 * Build the login-bonus song table (id 6), gated by the login-bonus predicate.
 * @ghidraAddress 0xc86b4
 */
- (void)createOpenLoginBonusMusics;

/**
 * Build the default arcade catalogue table (IDs {1, 2, 3, 300000000}).
 * @ghidraAddress 0xc8764
 */
- (void)createAcDefaultMusics;

/**
 * Load and Blowfish-decrypt the purchased-song lists (@c mulist / @c acmulist).
 * @ghidraAddress 0xc8820
 */
- (void)loadPurchasedMusics;

/**
 * Blowfish-encrypt and write the purchased-song lists back to @c mulist / @c acmulist.
 * @ghidraAddress 0xc8bec
 */
- (void)savePurchasedMusics;

/**
 * The in-memory purchased local-song list.
...
/**
 * The in-memory purchased arcade-song list.
...
/**
 * Merge a purchased local song into the list.
...
/**
 * Merge a purchased arcade song into the list.
 * @param item The purchased-song info to merge.
 * @return @c YES when the list changed.
...
/**
 * Delete a downloaded local (@c .orb) song file.
...
/**
 * Delete a downloaded arcade (@c .acv) song file.
...
/**
 * Whether @p packID appears in the encrypted @c recpack recommended list.
...
/**
 * Re-evaluate the treasure unlock gate and mark the local cache dirty.
 * @ghidraAddress 0xcafc0
 */
- (void)openTreasureMusic;

/**
 * Re-evaluate the invite unlock gate and mark the local cache dirty.
 * @ghidraAddress 0xcaff0
 */
- (void)openInviteMusic;

/**
 * Re-evaluate the collabo unlock gate and mark the local cache dirty.
 * @ghidraAddress 0xcb020
 */
- (void)openCollaboMusic;

/**
 * Re-evaluate the login-bonus unlock gate and mark the local cache dirty.
 * @ghidraAddress 0xcb050
 */
- (void)openLoginBonusMusic;

/**
 * A flat list of all currently-available local song IDs (default, purchased, and treasure).
...
/**
 * A flat list of all currently-available arcade song IDs (arcade-default and purchased-ac).
...
/**
 * The loaded level-patch records (@c rhythmin.lv).
...
/**
 * Mark the local-song cache stale so the next getter rebuilds it.
 * @ghidraAddress 0xcae18
 */
- (void)setMusicDataArrayDirty;

/**
 * Mark the arcade-song cache stale so the next getter rebuilds it.
 * @ghidraAddress 0xcae2c
 */
- (void)setAcMusicDataArrayDirty;

/**
 * Release the cached music data. A no-op in this build.
 * @ghidraAddress 0xcb248
 */
- (void)releaseChacheMusicData;

/**
 * Resolve a song id to its bundled or purchased (@c .orb) path. Class method: no instance
...
/**
 * Resolve a purchased local song id to its @c .orb path under Application Support.
 * @param musicId The song id to resolve.
...
/**
 * Resolve a purchased arcade song id to its @c .acv path under Application Support.
...
/**
 * Copy a shipped @c assets/ list into Documents once, if not already present.
 *
...
/**
 * Reconcile the purchased lists with the charts actually present.
 *
...
/**
 * Prune stale/duplicate entries from a purchased list and discover new charts.
 *
...
/**
 * The recommended-pack id list, decoded from the encrypted @c recpack file.
...
/**
 * Add a pack id to the encrypted @c recpack list. A no-op if already present.
...
/**
 * Whether the invite-reward song is open for the given reward tier.
...
/**
 * Whether @p musicId is the invite-reward song. @c YES if id == 4.
...
/**
 * Whether the BEMANI-collabo song is open.
 * @return @c YES when the song is unlocked.
 */
+ (BOOL)isOpenBemaniCollaboMusic;

/**
 * Whether the login-bonus song is open for the given reward tier.
 * @param index The reward tier to test.
 * @return @c YES when the song is unlocked.
 */
