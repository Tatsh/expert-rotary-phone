...
+ (id)init:(NSManagedObjectContext *)context {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if ([self getTreasureData:mainMapId subMapId:0 inManagedObjectContext:context] == nil) {
            [self addRecordWithMainMapId:mainMapId subMapId:0 inManagedObjectContext:context];
        }
    }
    ...
    for (short mapId = 0; mapId < 9; mapId++) {
        ...
        if ([self getTreasureData:mapId subMapId:0 inManagedObjectContext:context] != nil) {
            continue;
        }
        if ([self getTreasureData:parentMapId subMapId:0 inManagedObjectContext:context] != nil) {
            [self addRecordWithMainMapId:mapId subMapId:0 inManagedObjectContext:context];
        }
    }
    return nil;
}
