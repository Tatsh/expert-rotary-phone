...
//  image (memcpy'd straight in/out of the NSData blob), so the field layout
//  below is byte-exact and the struct is packed to alignment 1 to match. Only
//  subMapId is semantically recovered from a verified call site; the remaining
//  fields keep their Ghidra byte offsets and are named by offset until a call
//  site pins their meaning. Total size: 83 (0x53) bytes.
//
