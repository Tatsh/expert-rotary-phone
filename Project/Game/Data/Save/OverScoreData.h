...
// Delete every "over" record for a given music + sheet from the store (the
// result screen clears them for the just-played chart before re-fetching).
// Ghidra:
// +[OverScoreData deleteRecordWithMusic:sheet:inManagedObjectContext:]
// (selector
// @ 0x15a8f4), called from PlayResultTask::resultSetup (FUN_0003dfe0 @
// 0x3e2ec).
+ (void)deleteRecordWithMusic:(int)music
                        sheet:(short)sheet
       inManagedObjectContext:(NSManagedObjectContext *)context;

// Every persisted "over" record on the context (the whole rival table). Ghidra:
// +[OverScoreData getAllOverScoreData:] @ 0xba2b0. Read by the friend-score
// screen to mark which difficulties a rival already beat.
...
// Update (or create) the "over" record's date for a music + sheet + remote
// playerId.
