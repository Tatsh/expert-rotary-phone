...
//  pop'n rhythmin
//
//  Marsaglia xorshift128 pseudo-random generator. Reconstructed from Ghidra project
//  rb420, program PopnRhythmin; the original translation unit is named in an assert
...
//  canonical constants. The object carries a vtable (its only virtual is an empty
//  destructor), so its layout is { vptr, x, y, z, w } — matching the arcade task, which
//  embeds one at this+0x4f4.
//
...
public:
    ...
    // Reset x/y/z to the canonical constants and take `seed` as the w word (the game
    // seeds this with time() before a shuffle). Ghidra: FUN_00062b5c.
    void setSeed(uint32_t seed);

    // A uniformly-distributed integer in [0, max). Asserts max >= 0. Ghidra:
    // FUN_00062be0 (GetRandRangeInt): advances the generator and reduces the low 31
    // bits modulo max.
