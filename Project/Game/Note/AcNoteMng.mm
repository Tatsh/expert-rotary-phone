...
int AcNoteMng::initPlayData(const void *data, int size, int difficulty) {
    ...
    for (int i = 0; i <= lastIndex; i++) {
        ...
        switch (m_records[i].type) {
            case AC_NOTE_TAP:
                ...
                break;
            case AC_NOTE_END:   // type 3: the BGM-start anchor -> the drift-sync reference time
    ...
    // Thread the fixed node pool onto the free list, then prime the first frame — the binary
    // ends InitPlayData with exactly this pool build + a single update() call.
    initNodePool();
    update();
    return 0;
}

// Ghidra: the tail of InitPlayData (FUN_0007a774) — append every pooled node to the free list
// (the same O(n^2) tail walk the binary does; run once at play setup).
...
// Ghidra: FUN_0007aa90 — walk the chart from the first record; register a scroll segment for
// each tempo (type 4) event and count measure lines (type 10); stop at the end marker (type 6).
void AcNoteMng::registerTempoEvents() {
    for (int i = 0; i < m_recordCount; i++) {
        ...
        if (r.type == 10) {              // measure line
            ...
            // The binary asserts ("AdvanceRegisterEvent") if the segment table overflows.
            assert(registerScrollSegment((int16_t)r.value, r.tick) == 0);
        }
    }
}

// Ghidra: FUN_0007ba3c — insert one tempo/scroll segment, kept sorted by startTick. The scroll
// speed is bpm * 1024 / 480000 (DAT_0007bb28 / DAT_0007bb24). Returns non-zero if the table is
// full (max 63). Also refreshes the spawn look-ahead.
...
// Ghidra: the shared tail of FUN_0007ba3c / FUN_0007aaf8 — walk up to 8 front segments summing
// 60000/BPM (ms), advancing when the next segment's startTick is reached; the total is the
// spawn look-ahead. The sentinel startTick (-1) on unregistered segments blocks over-advance.
...
// Ghidra: FUN_0007b5e0 — wall-clock ms since the play clock was armed (0 until then).
...
// Ghidra: FUN_0007aeb4 — the current chart position. Uses live elapsed time unless the
// hold bit is set (then the last cached elapsed), applies the per-play offset, and adds
// the excess past the start threshold onto the smoothed scroll base.
...
// ---------------------------------------------------------------------------
// Arcade per-frame update (FUN_0007ac00 and its closure). The active-note pool is a dual
// linked list: nodes live on the free list (m_freeHead) until spawned onto the active list
// (m_activeHead), and return to the free list once retired. All geometry/flag constants are
// byte/disasm-verified against the binary.
// ---------------------------------------------------------------------------

// Detach `node` from the free list and append it to the active-list tail. Shared tail of
// makeNoteEvent / makeEvent / makeAdjustEvent (each first pops m_freeHead into `node`).
...
// Unlink `node` from the active list and append it to the free-list tail (retirement).
...
// Ghidra: FUN_0007b2f4 — spawn a playable note onto the active list (lane from the chart,
// optionally rotated in lane-mode 3), draw position primed at 1024.
...
// Ghidra: FUN_0007b790 — inject the BGM re-sync ("adjust") event once. Resets the scroll base
// and arms a type-0xf event whose judgement (applyBgmSync) corrects the scroll for BGM drift.
...
// Ghidra: FUN_0007b4f0 — the type-0xf adjust event: clear the in-flight flag and, if the BGM
// is playing, nudge the scroll target by the measured drift (BGM playhead vs expected time);
// otherwise re-arm the adjust a little later.
...
// Ghidra: FUN_0007aef8 — spawn every chart record that has come due (within the look-ahead).
// Off-window taps in auto-play are counted immediately; chart events fire their side effects.
    ...
    do {
        ...
        switch (rec->type) {
            ...
            case 10:            // 0xa: measure boundary
                if (dt < 4000) { makeEvent(rec); }
                else { m_barCount++; m_beatCount = 0; }
                break;
...
// Ghidra: FUN_0007b028 — first per-note pass: once a note's time has arrived, fire its
// event side effect (BGM start, bar/beat counters, adjust sync) and mark it handled (bit 5).
void AcNoteMng::judgeActiveNote(AcActiveNote *node, uint32_t pos) {
    ...
    switch (node->record->type) {
        case 2: case 7: case 8: case 9: case 0xc: case 0xd:
...
// Ghidra: FUN_0007b0a8 — second per-note pass: retire notes that have scrolled fully past
// (>4 s old; auto-miss-counted in auto-play) or that were already resolved (bits 4/5), then
...
// Ghidra: FUN_0007b1bc — refresh the per-lane "nearest note" candidate for input, and (in
// auto-play) auto-hit notes at/after their time, feeding the combo counter.
...
// Ghidra: FUN_0007b268 — refresh a note's on-screen scroll position; once past the judge line
// mark it judged (bit 2) and, when not auto-playing, let it slide off; clamp non-past notes at 0.
...
// Ghidra: FUN_0007bb30 — integrate scroll speed x elapsed span across the scroll-speed
// segments from `pos` up to the note's tick, scale by the hi-speed multiplier, clamp +-8192.
float AcNoteMng::computeScrollY(const AcActiveNote *node, uint32_t pos) const {
    ...
    if (pos < tick) {
        ...
        do {
            ...
            // Segment `seg`'s speed applies until the NEXT segment's startTick (a -1 sentinel on
            // the last one leaves the whole remaining span at this speed).
...
void AcNoteMng::update() {
    ...
    // Rebuild the per-lane "nearest note" table (9 lanes) from scratch each frame.
