...
//
//  Honesty notes:
//   - The four button y-positions are exact float-hex decodes (200 / 300 / 400
//   / 500), each
//     sized to its background art
//     ("custom_bt_game"/"custom_bt_howto"/"custom_bt_inquiry"/
//     "custom_bt_other"); all use setExclusiveTouch:YES.
//   - -init's phone branch adds a "friman_bg" image view to the view; the
//   binary computes that
//     image view's frame (from its own .frame and the image size) but never
//     applies it, so the image view keeps its intrinsic frame here too
//     (faithful to the discarded computation).
...
@implementation SettingTopViewController {
    BOOL _isAnimationing; // @0x… animation-in-flight guard (ivar type "c")
                          // _settingTopDelegate is the @synthesize'd backing ivar (assign,
                          // id<...Dalegate>).
}
...
- (instancetype)init {
    if ((self = [super init])) {
        if (!neSceneManager::isPadDisplay()) {
            // Phone: a "friman_bg" backdrop image view. (The binary computes a frame
            // for it but discards the result, so the intrinsic image size is kept.)
        ...
        const SEL actions[4] = {@selector(onGameButtonTouched:),
                                @selector(onHowtoButtonTouched:),
                                @selector(onCustomerButtonTouched:),
                                @selector(onOtherButtonTouched:)};
...
- (void)onGameButtonTouched:(id)sender {
    ...
    if (!neSceneManager::isPadDisplay()) {
        SettingGameTableViewController *vc =
...
// settingTopDelegate @ 0x14b80 / setSettingTopDelegate: @ 0x14b90 —
// @synthesize'd assign accessors.

// dealloc — ARC-omitted (BOOL + assign delegate only; nothing owned to
// release).
