...
// @ 0xaf848 — build the terms card (gradient card + embedded content nav + three buttons).
- (instancetype)init {
    self = [super init];
    if (self == nil) {
        return nil;
    ...
    gradient.colors = @[
        ...
        (id)[UIColor colorWithRed:0.99608099f green:0.63529414f blue:0.68235296f alpha:1.0f].CGColor,
    ];
    ...
    CustomTextView *textView = [[CustomTextView alloc]
        initWithFrame:CGRectMake(10.0f, isPad ? 60.0f : 55.0f,
                                 navFrame.size.width - 20.0f, navFrame.size.height - 20.0f)];
    ...
    // "詳細" (show full terms) button — horizontally centred in the content nav view.
    ...
    [detailBtn addTarget:self action:@selector(onDetailBtn:)
    ...
    rejectBtn.frame = CGRectMake(15.0f, detailBtn.frame.origin.y + detailImg.size.height + 17.0f,
                                 rejectImg.size.width, rejectImg.size.height);
    rejectBtn.exclusiveTouch = YES;
    [rejectBtn addTarget:self action:@selector(onNoBtn:)
        forControlEvents:UIControlEventTouchUpInside];
    ...
    [acceptBtn addTarget:self action:@selector(onYesBtn:)
        forControlEvents:UIControlEventTouchUpInside];
...
// dealloc @ 0xb02bc — object-only (releases _policyView + _naviCtrl); ARC-omitted.

// @ 0xb032c — accept: play the decide SE, record acceptance, run the close fade.
- (void)onYesBtn:(id)sender {
...
// @ 0xb03ac — "詳細": lazily build the full-terms overlay (PolicyView in its own nav
// controller) and add it over the root scene view.
- (void)onDetailBtn:(id)sender {
    neEngine::playSystemSe(1);
    if (_policyView == nil) {
...
// @ 0xb04e4 — detail back: play the cancel SE, hide the detail overlay, re-show the card.
- (void)onBackBtn:(id)sender {
    neEngine::playSystemSe(2);
...
// @ 0xb0718 — pull the view, notify the root VC the policy modal closed, clear the guard.
- (void)endCloseAnimation {
