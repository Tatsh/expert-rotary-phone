...
// image (top / bar / under depending on the row's position), a left caption label
// and a right value label showing the player's stored setting for that kind.
- (void)setData:(int)optionKind {
    ...
    NSString *bgName;
    switch (optionKind) {
        case 0:  bgName = @"acv_custom_option_top";   break;  // first row
        case 3:  bgName = @"acv_custom_option_under"; break;  // last row
        default: bgName = @"acv_custom_option_bar";   break;  // 1, 2 (middle rows)
    }
    ...
    _optionKindLbl.textColor = [UIColor colorWithRed:48.0f / 255.0f green:48.0f / 255.0f blue:48.0f / 255.0f alpha:1.0f];
    ...
    [_optionKindLbl setMinimumScaleFactor:10.0f];  // raw 0x41200000 (legacy minimum-font-size value)
    _optionKindLbl.frame = CGRectMake(50.0f, 9.0f, 100.0f, 26.0f);
    [self.contentView addSubview:_optionKindLbl];
    switch (optionKind) {
        case 0: _optionKindLbl.text = @"HI-SPEED"; break;
        case 1: _optionKindLbl.text = @"POP-KUN";  break;
        case 2: _optionKindLbl.text = @"HID-SUD";  break;
        case 3: _optionKindLbl.text = @"RAN-MIR";  break;
    }

    // Right value label.
    _optionDetailLbl = [[UILabel alloc] init];
    _optionDetailLbl.backgroundColor = [UIColor clearColor];
    _optionDetailLbl.textColor = [UIColor colorWithRed:120.0f / 255.0f green:120.0f / 255.0f blue:120.0f / 255.0f alpha:1.0f];
    ...
    switch (optionKind) {
        case 0: {
            static NSString *const kHiSpeed[] = {
                @"OFF", @"HI-SP 1.5", @"HI-SP 2.0", @"HI-SP 2.5", @"HI-SP 3.0", @"HI-SP 3.5",
                @"HI-SP 4.0", @"HI-SP 4.5", @"HI-SP 5.0", @"HI-SP 5.5", @"HI-SP 6.0"
            };
