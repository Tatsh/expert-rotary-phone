...
//  pop'n rhythmin
//
//  See DownloadImageView.h. Reconstructed from Ghidra project rb420, program PopnRhythmin.
//

#import "DownloadImageView.h"

@interface DownloadImageView ()
// Build the spinner (called from the initializers). Named to match the binary selector.
- (void)SetupView;
@end
...
- (id)initWithURLString:(NSString *)urlString {
    self = [super init];
    if (self != nil) {
        // Faithful: the binary stores the (autoreleased) string without retaining it; the strong
        // ivar under ARC keeps it alive.
...
// @ 0x62c5c — image view for `urlString` showing `image` until the download completes.
...
// @ 0x62e24 — start the fetch once: spin, create the downloader, hand it the URL and self as
// delegate, and kick it off after a 1s delay.
- (void)startDownload {
...
// @ 0x62ef0 — ImageDownloaderDelegate: swap in the decoded image, stop the spinner, drop the
// downloader.
...
// @ 0x62f60 — ImageDownloaderDelegate: on failure just stop the spinner and drop the downloader.
- (void)imageDownloaderDidFail:(ImageDownloader *)downloader didLoad:(NSIndexPath *)indexPath {
    [m_IndicatorView stopAnimating];
    m_ImageDownLoader = nil;
}

// @ 0x62cd0 — dealloc: cancel any in-flight download (kept, not a plain object-only teardown).
// ARC supplies the ivar release and the [super dealloc] chain.
- (void)dealloc {
    if (m_ImageDownLoader != nil) {
