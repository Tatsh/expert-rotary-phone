...
//  pop'n rhythmin
//
//  See SettingHowtoTableViewController.h. Compiled as Objective-C++ (.mm)
//  because it drives the C++ engine singletons through neEngineBridge.h
//  (neSceneManager::, neEngine::). Reconstructed from Ghidra project rb420,
...
//  (double 0x3fd3333340000000), tile 290x53 (0x43910000/0x42540000), tile
//  border width 3.0 / corner 5.0, label 226x36 (0x43620000/0x42100000) /
//  corner 10.0, font size 15.0 (0x41700000). Label texts are UTF-16LE CFStrings
//  (flags 0x7d0): "ゲームプレー" @0x12c6d6, "トレジャーモード" @0x12c6e4.
//
//  -tableView:didSelectRowAtIndexPath: presents the iPad how-to overlay
//  controller HowToViewCtrlPad (the sibling of the phone variant
//  HowToViewCtrl).
//
//  The tile / label frame *centres* in -tableView:cellForRowAtIndexPath: are
//  byte-decoded exact (@ 0x80cb4): [cell frame] stret → sp+0x30; vldr.32
//  s0,[sp,#0x38] = width; vmul.f32 d8,d0,#0x3f000000 (0.5f); itt mi; vmov.f32
//  d16,#0xc1200000 (=-10.0f); vadd.f32 d8,d8,d16 (pre-iOS-7 only); r3 =
//  #0x42000000 = 32.0f at setCenter:.
//
...
- (UINavigationController *)initAtNavigationController __attribute__((objc_method_family(none))) {
    ...
    // Back button sized to the "navi_btn_back" art. Frame is runtime:
    // backImg.size is not a compile-time constant (two blx 0x12fe6c [UIImage
    // size] stret calls @ 0x8051e/0x8052a).
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    tile.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"back_bg_st"]];
    // @ 0x80cb4: [cell frame] stret → sp+0x30; sp+0x38 = frame.size.width; vmul
    // with #0x3f000000 (0.5f) → cx = width * 0.5; itt mi; vmov.f32
    // d16,#0xc1200000 (=-10.0f); vadd.f32 d8,d8,d16 if iOS<7. r3 = #0x42000000
    ...
    label.text = title;
    label.center =
        CGPointMake(cx, 32.0f); // exact: ldr r2,[sp,#0x14] = cx; r3 = #0x42000000 = 32.0f
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 1) {
        NSArray *images = [NSArray arrayWithObjects:@"howto_tre01",
                                                    @"howto_tre02",
                                                    @"howto_tre03",
                                                    @"howto_tre04",
                                                    @"howto_tre05",
        ...
        NSArray *images = [NSArray
            arrayWithObjects:@"howto_01", @"howto_02", @"howto_03", @"howto_04", @"howto_05", nil];
        if (_howtoViewCtrlPad != nil) {
            _howtoViewCtrlPad = nil;
        }
        _howtoViewCtrlPad = [[HowToViewCtrlPad alloc] initWithFileNameArray:images];
