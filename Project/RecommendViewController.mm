...
/**
 * The recommend list's private transition completions and back action.
 */
...
- (void)startOpenAnimation {
    ...
    if (!neSceneManager::isPadDisplay()) {
        ...
        self.navigationController.view.frame = f;
        // Options = 2 (UIViewAnimationOptionAllowUserInteraction): the binary
        // stores 0x2 into the options slot (movs r6,#0x2; str r6,[sp,#0x8] @
        // 0xbc832/0xbc846), not LayoutSubviews.
...
- (void)startCloseAnimation {
    ...
    if (!neSceneManager::isPadDisplay()) {
        ...
        // @ 0xbccb8). Phase 2 (0.25 s): park below the root view
        // (settingNavSetFrameFromView @ 0xbcdf8),
        //   then call -endCloseAnimation.  The phase-2 wrapper is the outer
        //   completion block-invoke @ 0xbcd50.
        UIViewController *root = RootVC();
        // Options = 2 (UIViewAnimationOptionAllowUserInteraction): the outer
        // animate stores 0x2 into the options slot (movs r4,#0x2;
        // str r4,[sp,#0x8] @ 0xbcbba/0xbcbc4), not LayoutSubviews.
        [UIView animateWithDuration:(1.0 / 6.0)
            delay:0.0
            options:UIViewAnimationOptionAllowUserInteraction
            animations:^{
            ...
            completion:^(BOOL finished) {
              // Phase-2 duration is 0.25 s (vmov.f64 d16,0x3fd0... @ 0xbcd68),
              // options = 2 (movs r5,#0x2; str r5,[sp,#0x8] @ 0xbcdde/0xbcde8).
              [UIView animateWithDuration:0.25
                  delay:0.0
                  options:UIViewAnimationOptionAllowUserInteraction
                  animations:^{
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    // "Cell%ld-%ld", data @ 0x1029ae), not an underscore.
    NSString *identifier =
