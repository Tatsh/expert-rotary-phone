...
//   "%@/%@ (%@; iOS %@; %@)"  =  "SHISHAMO MUSIC/<ver> (<model>; iOS <os>;
//   <locale>)"
...
// StoreKit product list, delivered by PurchaseManager's finishRequest:
// callback.
@property(atomic, strong, readonly) NSArray *products; // getter @ 0xb0bc

// Ad-reward app id (ad SDK removed; kept for structural parity — see .mm
// launch).
...
// Convenience accessor for the shared delegate instance.  @ 0x... (thunk
// PTR_s_appDelegate)
+ (instancetype)appDelegate;

// Documents directory + free-space helpers (class methods).
+ (NSString *)appDocumentsDirectory;  // Ghidra: -[AppDelegate
                                      // appDocumentsDirectory] (class method)
+ (NSString *)appAppSupportDirectory; // Application Support dir (downloadable
                                      // data) @ 0x8a1c
// Exclude an existing item from iCloud/iTunes backup
