...
// (verified in newsGetFinished's NSValue wrapping). The two NSString* fields are
// retained; freed via releaseInformationData.
typedef struct {
...
// One recommended music-pack. Obj-C type-encoding "{RecommendData=i@@@@}" (verified
// in getRecommendListFinished's NSValue wrapping). The four NSString* fields are
// retained; freed via releaseRecommendData.
typedef struct {
...
// getPresentListFinished's NSValue wrapping). The NSString* field is retained; freed
// via releasePresentList.
typedef struct {
...
// wrapping). The three NSString* fields are retained; freed via releaseOverScoreLogArray.
typedef struct {
...
// C++ scene objects this singleton bridges back to (see the cppDelegate* properties
...
#ifdef __cplusplus
class MenuMainTask;                 // System/src/Task/MenuMainTask.h (: C_TASK); mode-select scene
...
// screen awards the event bonus when the played song matches one.
...
// POST "uuid=<uuId>&client_ver=<ver>" to the player-get URL. Resets errorGetPlayer
// to -1 while in flight. @ 0x93f14.
...
// Backend error (-1 = none / in-flight, 99 = network error, else server code). @ 0x99760.
...
// The scrolling-ticker text lines and their tap URLs (parallel). @ 0x997cc / 0x997e0.
@property (nonatomic, strong, readonly) NSArray *newsTextArray;
@property (nonatomic, strong, readonly) NSArray *newsUrlArray;
// The parsed store-information posts (NSValue-wrapped InformationData). @ 0x9970c.
...
// Set when the store's UpdateTime is newer than the last viewed time. @ 0x99880 / 0x99898.
@property (nonatomic, assign) BOOL isNewMusicPackReleased;
// C++ mode-select scene notified (via modeSelectRefreshNews) when news finishes.
// @ 0x995ac / 0x995c0.
#ifdef __cplusplus
...
// C++ music-select scene notified (via musicSelUpdateInfoPanel) when it finishes.
// @ 0x995d8 / 0x995ec.
#ifdef __cplusplus
...
// Delegate notified when the present-list request finishes. @ 0x99688 / 0x9969c.
...
// Delegate notified when the present-claim request finishes. @ 0x996b4 / 0x996c8.
...
// Delegate notified when the over-score-log request finishes. @ 0x99a84 / 0x99a98.
...
// The active treasure-event / game-event music ids (NSArray of NSNumber). @ 0x999fc.
@property (nonatomic, strong, readonly) NSArray *treasureEventIdArray;
// Set once each list refreshes so the scenes reload. @ 0x99a24 / 0x99a3c, 0x99a54 / 0x99a6c.
