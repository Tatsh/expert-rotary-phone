...
- (void)loadView {
    ...
    // Binary: -[UIView setAutoresizingSize] category = flexible width + height
    // (inlined, as StoreMainViewController does — the reconstruction inlines this
    // helper rather than the category).
...
// @ 0x4c308 — one manage row: the built-in text/detail labels are positioned
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (cell == nil) {
        ...
        // Built-in text label: black DFSoGei, flexible width, trimmed to leave room
        // for the button.
        CGRect tf = cell.textLabel.frame;
        cell.textLabel.frame =
            CGRectMake(tf.origin.x, tf.origin.y, cell.frame.size.width - 200.0f, tf.size.height);
    ...
    NSDictionary *item =
        [[[MusicManager getInstance] getPurchasedMusicDictionaris] objectAtIndex:indexPath.row];
    ...
    // Right-align the button in the cell, vertically centred (36pt tall, 40pt on
    // iPad).
    ...
    // Fill the built-in labels, copy their text into the shadow labels, then
    // blank them.
...
- (void)tableView:(UITableView *)tableView
    ...
    if (!neSceneManager::isPadDisplay()) {
        ...
        UIImageView *bg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
...
- (void)pushCellButton:(id)sender {
    ...
    // The button lives inside the cell's view tree; walk up to its
    // UITableViewCell.
    ...
    // Ghidra checks getPathFromPurchased:/RhFileExists twice; both use the same
    // id, so it reduces to a single "is the audio file present?" test.
    if (!RhFileExists([[MusicManager getInstance] getPathFromPurchased:musicId])) {
        // Missing -> refresh its info then re-download. Show the shared progress
        // dialog.
...
- (void)downloaderFinished:(Downloader *)downloader {
    ...
    StoreMusicInfo *info = [[StoreMusicInfo alloc] initWithDictionary:[downloader getDataInJSON]];
    if (info != nil) {
