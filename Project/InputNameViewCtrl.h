...
//  "inputname_text_*" caption images); on pad (neSceneManager::isPadDisplay) it is
//  a floating rounded navigation-card panel over a dimmed backdrop, with the caption
//  drawn as DFSoGei labels instead of images. Submitting a non-empty, valid name
//  POSTs "uuid=<uuid>&name=<name>&client_ver=<n>" to StoreUtil +playerNewURL; on
//  success it saves the returned PlayerId + name via UserSettingData and fades the
//  panel out (notifying the scene root -InPlayerNameEndCallBack).
//
...
//
//  .mm because the decide / cancel flow drives the C++ neEngine / neSceneManager
//  singletons via neEngineBridge.
//
