...
//  pop'n rhythmin
//
//  See SettingTableViewController.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin. Uses the C++ neEngine / neSceneManager singletons (SE playback, root VC,
//  isPad flag) through neEngineBridge.h.
//
//  Honesty / uncertainty notes:
//   * The six section headers and the per-row cell labels are Japanese CFStrings decoded
//     from the binary's UTF-16LE __cfstring data (flags 0x7d0). Exact code points recovered:
...
//   * _effectSwitch (a UIButton toggle) and _simpleModeSwitch (a UISwitch) are wired to
//     onEffectOnChanged: / onSimpleModeChanged:; the controls themselves are created outside
//     the methods reconstructed here.
//
...
// dealloc @ 0x7ef98 — ARC-omitted (object ivars only; the MRC original released the strong
// howtoViewCtrlPad overlay, which ARC releases automatically).
...
// @ 0x7f250 — tear down the toggle controls, remove the nav view, and hand control back to
// MainViewController.
- (void)endCloseAnimation {
    // NOTE: the MRC original released _effectSwitch and _simpleModeSwitch here (they were
    // alloc-owned toggle controls torn down with the nav view). Under ARC we drop our strong
    // refs by niling them instead of calling -release.
    _effectSwitch = nil;
...
// @ 0x7f2f0 — NOTE: the binary tail-calls the *super* viewWillDisappear: here (reproduced
// faithfully; matches the sibling SettingOtherTableViewController — likely a copy/paste
// artifact in the original source).
...
// @ 0x7f378 — rows per section = { News:1, Settings:3, HowTo:2, Treasure:1, DeviceChange:1,
// Inquiry:3 } (DAT_0012f880).
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    switch (section) {
        case 0:  return @"お知らせ";        // News
        case 1:  return @"設定";            // Settings
        case 2:  return @"遊び方";          // How to play
        case 3:  return @"トレジャーモード"; // Treasure Mode
        case 4:  return @"機種変更";        // Device change
        case 5:  return @"お問い合わせ";    // Inquiry
        default: return nil;
    }
}

// @ 0x7f390 — plain default cells carrying the per-row title label. Building the game-effect
// row (section 1, row 2) also seeds the cached effect flag from UserSettingData.
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    switch (indexPath.section) {
        case 0:
            if (indexPath.row == 0) { cell.textLabel.text = @"お知らせ"; }
            break;
        case 1:
            if (indexPath.row == 2)      { cell.textLabel.text = @"ゲーム演出"; }
            else if (indexPath.row == 1) { cell.textLabel.text = @"ポップ君サイズ"; }
            else if (indexPath.row == 0) { cell.textLabel.text = @"サウンド"; }
            break;
        case 2:
            if (indexPath.row == 1)      { cell.textLabel.text = @"トレジャーモード"; }
            else if (indexPath.row == 0) { cell.textLabel.text = @"ゲームプレー"; }
            break;
        case 3:
            if (indexPath.row == 0) { cell.textLabel.text = @"リタイア"; }
            break;
        case 4:
            if (indexPath.row == 0) { cell.textLabel.text = @"機種変更"; }
            break;
        case 5:
            if (indexPath.row == 2)      { cell.textLabel.text = @"利用規約"; }
            else if (indexPath.row == 1) { cell.textLabel.text = @"特定商取引法に基づく表記"; }
            else if (indexPath.row == 0) { cell.textLabel.text = @"お問い合わせ"; }
            break;
    }
    return cell;
}

// @ 0x7f764 — on the phone the sub-screen rows carry a disclosure indicator; on the iPad
// none do. (Sections 0/3 have no accessory; section 5 rows likewise.)
...
// @ 0x7f818 — dispatch a tap while this VC is the top of the nav stack: push (or, for the
// iPad how-to, overlay) the matching sub-screen and play the confirm SE.
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        ...
        case 1: {
            ...
            [self.navigationController.navigationBar
                setBackgroundImage:[UIImage imageNamed:navImage] forBarMetrics:UIBarMetricsDefault];
            [self.navigationController pushViewController:sub animated:YES];
            break;
        }
        case 2: {
            // How to play: basic (row 0) or treasure-mode (row 1). On the iPad this is shown
            // as an overlay on the root scene view; on the phone it is pushed.
            NSArray *files;
            if (indexPath.row == 1) {
                files = @[ @"howto_tre01", @"howto_tre02", @"howto_tre03",
                           @"howto_tre04", @"howto_tre05", @"howto_tre06" ];
            } else if (indexPath.row == 0) {
        ...
        case 5: {
            // Inquiry: FAQ (row 0) / 特定商取引法 (row 1) open in Safari; 利用規約 (row 2)
            // pushes the in-app PolicyView.
            if (indexPath.row == 2) {
                ...
                [[UIApplication sharedApplication] openURL:
                    [NSURL URLWithString:@"http://license.konami.com/TOKUSHO/license/index.html"]];
            } else if (indexPath.row == 0) {
                [[UIApplication sharedApplication] openURL:
                    [NSURL URLWithString:@"https://www.faq.konami.jp/app/confirm"]];
            } else {
                return;
            }
            break;
...
// @ 0x80128 — confirming the retire (button index 1) wipes the treasure temp and reports back.
...
// @ 0x801ec — effect toggle: flip the cached flag, play the confirm SE, swap the button art,
// then persist to UserSettingData.
