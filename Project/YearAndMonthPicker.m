...
//  pop'n rhythmin
//
//  See YearAndMonthPicker.h. Reconstructed from Ghidra project rb420, program PopnRhythmin.
//  This file covers the data-source / delegate and the selection accessors; the ~4.6 KB
//  -init (which builds the pre-repeated monthArr and picks the current year/month as the
//  default selection) is a separate reconstruction piece (tracked in HANDOFF.md).
//
...
// @ 0x8ee50 — wire self as its own data source + delegate, build the wrapping month list
// (12 months repeated 14 times = 168 rows so the wheel scrolls endlessly), and default the
// selection to January 2000 (year row 100 -> 1900 + 100; month row 84 -> the middle band).
- (instancetype)init {
    self = [super init];
    if (self != nil) {
        self.delegate = self;
...
// @ 0x8f034 — 200 years on the first wheel; the pre-repeated month list on the second.
...
// @ 0x8f064 — column widths (narrower pre-iOS 7, where the year column is widened).
...
// @ 0x8f0e4 — recenter the wrapping month wheel to its middle band (so it scrolls
// endlessly), then latch the selected year (row + 1900) and month (1..12).
...
// @ 0x8f180 — a per-row label: " YYYY" for the year wheel, the month title for the month
