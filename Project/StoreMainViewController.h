...
@interface StoreMainViewController : UIViewController <StorePackListControllerDelegate,
    ...
    // Pack-table "show more" controls (built by viewDidLoad, driven by
    // selectShowMore).
    ...
    StorePackInfoDownloader
        *m_StorePackInfoDownloader; // fetches a missing pack's detail on restore
}
...
// The pack table's "show more" button action: flips the button to a loading
// title + spinner and kicks off the next page fetch. Ghidra: @ 0x494cc.
- (void)selectShowMore;
...
// Detail-open helpers (implemented in StoreMainViewController.mm alongside the
// viewDidLoad cluster): phone push / pad in-place card / dim-cover tap dismiss.
