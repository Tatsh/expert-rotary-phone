...
// @ 0x5b694 — bind one value row of an option kind: pick the rounded-group
// background slice (top for the first row, under for the last, bar in between —
...
- (void)setData:(int)index {
    ...
    // The last row index for this option kind (HI-SPEED spans many; the rest are
    // short).
    int lastIndex;
    switch (_optionKind) {
    ...
    default:
        lastIndex = 10;
        break; // HI-SPEED (kind 0) and anything else
    }
    ...
    _optionLbl.textColor = [UIColor colorWithRed:48.0f / 255.0f
                                           green:48.0f / 255.0f
                                            blue:48.0f / 255.0f
                                           alpha:1.0f];
    ...
    [_optionLbl setMinimumScaleFactor:10.0f]; // raw 0x41200000 (legacy
    ...
    // The player's currently-selected value for this option kind (drives the
    // check mark).
    int selected = 0;
    switch (_optionKind) {
    ...
    _checkImageView =
        [[UIImageView alloc] initWithFrame:CGRectMake(osVersion < 7.0f ? 228.0f : 245.0f,
                                                      7.0f,
                                                      checkImg.size.width,
                                                      checkImg.size.height)];
