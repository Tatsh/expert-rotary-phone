...
//  friend-manage split hub. Objective-C++ (drives the C++ "ne" engine
//  singletons for the SE / scene-root bridge).
//
...
// Forward-declare so the section-handler methods below can call them; bodies
// are defined after @end (where @implementation { } ivars are already visible).
...
@public                     // the file-static frame helpers (defined after @end) reach these via
                            // self->;
                            // @private ivars are visible but not *accessible* to C functions, so
                            // publish them.
...
// .cxx_construct @ 0xc5414 — compiler-emitted C++ ivar constructor; not
// hand-written.

// @ 0xc3358 — build the dimmed backdrop (tap to close), the artwork panel, the
// left section column (FriendMngTopViewController), the right navigation pane,
// the selection arrow, and a top cover strip; then populate the list section.
// All frames verified byte-exact: section rects (385, 182, 320, 716); arrow rows
...
// @ 0xc40d0 — switch the right pane to the friend-list section. The first
// population (selectedIndex < 0) swaps in place; later taps flip the pane.
// Ghidra flip block helper friendMngShowListView @ 0xc4448 (modeled here as a
// single flip whose completion clears the guard and restores the top VC's right
// bar item).
// Verified: guards on _isAnimationing / _selectedIndex == 0; initWithStyle:Grouped;
// navbar art frilis_navbar; the outer/inner block invokes resolve to
// friendMngSyncRightViewFrame (0xc43b8) / friendMngShowListView (0xc4448) /
...
- (void)onListButtonTouched:(id)sender {
    ...
    if (_selectedIndex < 0) {
        ...
        // Outer flip: collapse right view width → friendMngSyncRightViewFrame @
        // 0xc43b8. Completion = friendMngShowListView @ 0xc4448: swap VC, then
        // inner flip → friendMngSetListFrame @ 0xc45a0; inner completion →
        // friendMngSetListArrowFrame @ 0xc4700.
...
// @ 0xc4760 — switch the right pane to the "presenting" (requests you sent)
// section. Ghidra flip block helper friendMngShowRequestView @ 0xc4ad8.
// Verified: guards on _isAnimationing / _selectedIndex == 1; plain -init (no
...
- (void)onRequestButtonTouched:(id)sender {
    ...
    if (_selectedIndex < 0) {
        ...
        UIBarButtonItem *rightItem = vc.navigationItem.rightBarButtonItem;
        vc.navigationItem.rightBarButtonItem = nil;
        // Outer flip: friendMngSyncRightViewFrame2 @ 0xc4a48 (identical to
        // 0xc43b8). Completion = friendMngShowRequestView @ 0xc4ad8: swap VC +
        // inner flip → friendMngSetRequestFrame @ 0xc4c30; inner completion →
        // friendMngSetRequestArrowFrame @ 0xc4d90.
        [UIView transitionWithView:_rightViewCtrl.view
            duration:0.3
            options:UIViewAnimationOptionCurveEaseIn
            animations:^{
...
// @ 0xc4df0 — switch the right pane to the reply section. Ghidra flip block
// helper friendMngShowReplyView @ 0xc5140.
...
- (void)onReplyButtonTouched:(id)sender {
    ...
    if (_selectedIndex < 0) {
        ...
        _isAnimationing = YES;
        // Outer flip: friendMngSyncRightViewFrame3 @ 0xc50b0 (identical to
        // 0xc43b8). Completion = friendMngShowReplyView @ 0xc5140: swap VC + inner
        // flip → friendMngSetReplyFrame @ 0xc5290; inner completion →
        // friendMngSetReplyArrowFrame @ 0xc5370.
        [UIView transitionWithView:_rightViewCtrl.view
            duration:0.3
            options:UIViewAnimationOptionCurveEaseIn
            animations:^{
...
// for the ObjC block literals captured in on{List,Request,Reply}ButtonTouched:
// and the friendMngShow*View completion helpers. Each has exactly one caller
// class (FriendMngTopSplitViewController) → file-static, no shared header.
//
...
// (and friendMngSyncRightViewFrame2 @ 0xc4a48, friendMngSyncRightViewFrame3 @
