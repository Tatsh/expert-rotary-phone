...
// @ 0x449e0 — a catalogue page finished. The recommend list feeds straight into a
// detail open (deep-link); the normal list rebuilds the table, adds the restore bar
// button on first success, repositions the store_fun banner, refreshes the "show more"
// footer + promotion header, and — if a recommend pack is still pending — starts the
// recommend fetch.
- (void)packListDownloadSuccess:(StorePackListController *)controller {
    ...
    // Lazily install the "復元" (restore) right bar button the first time a page lands.
    if (m_RestoreButton == nil) {
        UIImage *img = [UIImage imageNamed:@"p_store_btnrestore"];
        CGSize size = img ? img.size : CGSizeZero;
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0.0f, 0.0f, size.width, size.height)];
    ...
    // Promotion header: show the real banner when a promotion exists, else the dummy.
    if (m_PromotionView) {
...
// @ 0x45108 — a page fetch failed. Before any row is on screen (table still hidden)
// route through the empty-state label; once rows exist, pop an alert and re-enable the
// table. Either way clear the host's pending recommend-pack id.
...
// @ 0x45258 — the fetch returned an empty catalogue; same split as the error path.
...
// @ 0x44864 — surface a load failure in the pack table's empty area: hide the table
// (tag 10000) + its spinner (tag 0x2711) and show the empty-state label (tag 0x2712).
...
// @ 0x45648 — a promotion banner tile was tapped. Phone pushes a detail screen; iPad
// slides the in-place detail card up over a dim cover.
- (void)storePromotionViewTaped:(StorePromotionView *)view PackID:(int)packID {
    ...
    [UIView setAnimationDidStopSelector:@selector(openDetailAnimStopFromPromotion:finished:context:)];
    [m_CoverViewPad setAlpha:1.0f];
    [m_PackDetailViewPad setAlpha:1.0f];
    [UIView commitAnimations];
}

// @ 0x45510 — iPad: the "open detail" slide finished for a normal cell tap. Resolve the
...
// @ 0x45898 — iPad: the promotion-tap slide finished; load the promoted pack's detail.
...
// @ 0x45a80 — iPad: the detail-close slide finished; tear the card down and restart the
// promotion animation.
...
// @ 0x46420 — detail card asked to close: phone pops the nav stack, iPad slides the card.
...
// @ 0x46470 — the download/purchase modal's cancel button: abort the download, hide the
// modal and let the visible detail re-check its button caption.
...
// @ 0x4658c — fold a pack's music + AC-music infos into the purchased library, optionally
// persisting.
...
// @ 0x45b48 — kick off downloads for a pack's still-missing musics. Grants the pack's
// character ticket, shows the download modal, builds a StoreDownloadTask per missing
// file (both normal + AC music) and starts a StoreDownloadManager — or, if nothing is
// missing, just flips the button caption to "installed".
...
// @ 0x46a7c — re-download: re-register the pack's musics (persisting) then download.
...
// @ 0x46798 — refresh the on-screen state for a just-purchased pack. Phone: if the detail
// screen is up flip its purchase state, else reload the pack's row in the main list
// (section 1); iPad: reload the pack's row (section 0, two packs per row).
...
// @ 0x46ab0 — a purchase completed for a product id. If it matches the pending pack,
// persist the musics, register the product, refresh the row, start the downloads, and
// roll the month's spend total forward for the parental spending guard.
...
// @ 0x46d1c — a purchase failed/cancelled: drop the delegate, hide the modal and report.
- (void)purchaseFailed:(NSString *)productID error:(NSError *)error {
    [[PurchaseManager sharedManager] setMusicDataDelegate:nil];
    m_PurchasingPackInfo = nil;
    [m_StoreViewCtrl hideModalDialog];
    NSString *message = [[NSString alloc] initWithFormat:@"購入はキャンセルされました。\n\n%@",
...
// @ 0x46e58 — remember a restored pack's info and tick its product off the pending list.
...
// @ 0x46ef4 — walk the pending restored product ids: for each, resolve its pack info (or
// spin up a StorePackInfoDownloader to fetch a missing one). Returns YES while a detail
// fetch is still in flight (the download callback re-enters here), NO when all are ready.
...
// @ 0x47134 — all restored packs are gathered: persist them, migrate the purchase-checked
// products, refresh their rows, and — if any file is still missing — offer to download
// them all (alert tag 0x1e); otherwise finish silently.
...
// @ 0x4753c — download every missing music across the restored packs (grants tickets),
// then start the download manager (or hide the modal when nothing is missing).
...
// @ 0x47c14 — StoreKit restore succeeded: reset the restore accumulators, seed the
// pending product-id list from the purchase-checked products, and start walking them.
...
// @ 0x47f38 — the download manager began the next file: show its name in the modal.
...
// @ 0x47ffc — all files downloaded: drop the manager, mark the detail button installed
// and hide the modal.
...
// @ 0x48108 — a download failed: drop the manager, hide the modal, report, and let the
// detail re-check its caption.
- (void)downloadManagerFailed:(StoreDownloadManager *)manager {
    m_DownloadManager = nil;
    [m_StoreViewCtrl hideModalDialog];
    NSString *message =
        [[NSString alloc] initWithString:@"ダウンロードに失敗しました。\nネットワーク接続をご確認下さい。"];
    CommonAlertView *alert = [[CommonAlertView alloc] initWithTitle:@"Error"
                                                            message:message
                                                           delegate:nil
                                                  cancelButtonTitle:@"OK"
...
// @ 0x495e4 — a jacket finished loading: drop it into the on-screen cell (phone: one
// StorePackCell per row; iPad: the left/right pack view of a StoreTableCell).
- (void)imageDownloader:(ImageDownloader *)downloader didLoad:(NSIndexPath *)indexPath {
    ...
    if (!m_IsPad) {
        ...
        NSIndexPath *ip = [NSIndexPath indexPathForRow:(indexPath.row / 2) inSection:indexPath.section];
...
// @ 0x47a04 — CommonAlertView button handler. Tag 0x1f = the restore-confirm alert (begin
// the StoreKit restore on "OK"); tag 0x1e = the download-all prompt (start the downloads
// on "OK", else abandon the restore).
...
// @ 0x4832c — number of pack rows: the pack-id count, halved (rounding up) on iPad where
// two packs share a row.
...
// @ 0x48fc0 — phone has a promotion section + a pack section; iPad has just the packs.
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    return m_IsPad ? 1 : 2;
}

// @ 0x48fd8 — phone promo section is one row; the pack section is numPackRows (+1 for the
// "show more" footer while the list continues).
...
// @ 0x49038 — row heights: phone promo scales to the table width (730:240); pack rows are
// 104 (phone) / 140 (iPad), the "show more" footer 84 (phone) / 60 (iPad).
...
// @ 0x4837c — build a cell: phone section 0 = the promotion banner; the pack section is
// one StorePackCell per pack (phone) or a StoreTableCell holding two StorePackViews
// (iPad); the trailing row is the "show more" footer. Jackets load lazily through an
// ImageDownloader keyed by the row's index path.
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!m_IsPad && indexPath.section == 0) {
        ...
        StorePromotionTableCell *promoCell =
            (StorePromotionTableCell *)[tableView dequeueReusableCellWithIdentifier:@"StorePromotionCell"];
        if (promoCell == nil) {
        ...
        if (!m_IsPad) {
            ...
            NSString *rid = (indexPath.row & 1) ? @"StorePacklistCellOdd" : @"StorePacklistCellEven";
            ...
            NSIndexPath *leftIP = [NSIndexPath indexPathForRow:leftIndex inSection:indexPath.section];
            ...
            if (rightIndex < (NSInteger)[ids count]) {
                ...
                NSIndexPath *rightIP = [NSIndexPath indexPathForRow:rightIndex inSection:indexPath.section];
        ...
        if (moreCell == nil) {
            ...
            moreCell.textLabel.font = [UIFont fontWithName:AppFontName() size:(m_IsPad ? 18.0f : 15.0f)];
...
// Lazy jacket loader shared by the three cellForRowAtIndexPath: paths (inlined in the
// binary): returns the cached artwork, or nil while an ImageDownloader (keyed by the
// index path) fetches it.
- (UIImage *)artworkForInfo:(StorePackInfo *)info atIndexPath:(NSIndexPath *)indexPath {
...
// @ 0x4912c — colour each cell as it appears: phone pack rows take the alternating pack
// backdrop, everything else a flat grey (packs 0.5 iPad, footer 0.6).
...
// @ 0x49258 — a phone pack row was tapped: push its detail (ignored on iPad / when not
// the visible controller / on the "show more" footer).
...
// @ 0x49c84 — coming back on screen (phone): refresh + deselect the previously selected
// pack row so its purchased state updates.
...
// @ 0x49d64 — first appearance kicks off the initial fetch (hiding the empty label +
// table); otherwise re-run the success path to lay everything out.
...
// @ 0x49e88 — leaving the screen: stop the iPad detail card, cancel any in-flight page /
// detail fetch and re-enable the table.
...
// @ 0x4a044 — dealloc: KEPT under ARC because it actively tears down live work — cancels
// the restore detail downloader, detaches the table, stops the jacket loads, cancels the
// download manager and detaches/cancels the promotion view. ARC releases the ivars, so
// the object-only release lines and [super dealloc] are omitted.
- (void)dealloc {
    if (m_StorePackInfoDownloader) {
        [m_StorePackInfoDownloader setDelegate:nil];
        [m_StorePackInfoDownloader cancel];
