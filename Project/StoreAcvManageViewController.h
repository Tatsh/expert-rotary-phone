...
//  for arcade-viewer content (delete / re-download). Lists the purchased arcade-
//  viewer songs, offers a per-row delete / re-download button, runs an integrity
//  "check" pass (fetching missing arcade-song info) on load, and drives the file
//  download through StoreDownloadManager with the shared store modal dialog.
//
...
@interface StoreAcvManageViewController
    : UIViewController <UITableViewDataSource, UITableViewDelegate, DownloaderDelegate,
                        StoreDownloadManagerDelegate, CommonAlertViewDelegate> {
