...
//  This one edits HI-SPEED (eleven values: OFF, HI-SP 1.5 .. HI-SP 6.0). Objective-C++
...
// @ 0x2cbb0 — build the list: a transparent, separator-less UITableView; a back button;
...
// @ 0x2d484 — after loading, poke the scene manager (populates the pad-display flag).
- (void)viewDidLoad {
    [super viewDidLoad];
    neSceneManager::shared();
}

// @ 0x2d4b4 — treat a rightward pan (translation.x > 80) as a back-button press.
- (void)handleGesture:(UIPanGestureRecognizer *)recognizer {
    if (recognizer == nil) {
        return;
    }
...
// @ 0x2d530 — one AcViewerDetailCell per value (reused by "Cell%ld_%ld"), bound to the
// HI-SPEED option kind (0) and the row's value label.
...
// @ 0x2d668 — a new value: store it, refresh, play the decide SE and pop back to the
...
// @ 0x2d738 — BACK: (on a real tap) play the cancel SE, refresh the option list behind
