...
//  The left pane is a PopnLinkTopViewController (the section-button column); the right pane
//  is a UINavigationController whose top controller is swapped between the KONAMI-ID input
//  / score-checker / quiz sections by the left column's buttons (forwarded here through the
//  left VC's delegate). While the player has not yet linked their pop'n-link (e-AMUSEMENT
//  KID) the checker / quiz buttons instead route to the KONAMI-ID input screen
//  (neAppEventCenter::linkButtonsEnabled()). Section swaps are block-based flip transitions;
//  a selection arrow tracks the active row. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin (init @ 0xe0b40, the shared open/close fade animations, the section handlers
//  onInKidButtonTouched:/onScoreCheckerButtonTouched:/onQuizButtonTouched:).
//
//  Animation lifecycle (shared modal-VC pattern):
//    startOpenAnimation  — fade the view (+ its nav view) 0 -> 1 over 0.5s; didStop ->
...
// after a link state change). Ghidra: reloadLeftView @ 0xe2bb8.
- (void)reloadLeftView;

// Section buttons, driven by the left column (PopnLinkTopViewController) via its delegate:
// swap the right pane's top controller and slide the selection arrow. Checker / quiz fall
// back to the KONAMI-ID input while the player is not linked. Ghidra: onInKidButtonTouched:
// @ 0xe19c0 / onScoreCheckerButtonTouched: @ 0xe1fa8 / onQuizButtonTouched: @ 0xe25b0.
