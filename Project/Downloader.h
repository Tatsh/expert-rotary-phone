...
// being verified). Synthesized atomic-retain accessors (backing ivar
// m_AdditionalData): getter @ 0x62afc (objc_getProperty), setter @ 0x62b10
// (objc_setProperty).
...
// POST `body` with the given Content-Type. Ghidra:
// initWithURL:delegate:Post:ContextType:
// @ 0x6224c.
...
// Abort an in-flight request: drop the delegate (so no late callbacks fire),
// cancel + release the NSURLConnection, and free the response buffer. Ghidra: @
// 0x6249c.
- (void)cancel;

// The raw buffered response bytes (e.g. an audio preview clip). Ghidra: @
// 0x62938.
- (NSData *)getData;

// Parse the buffered response as JSON — NSJSONSerialization when available,
// else the bundled TouchJSON category. Ghidra: @ 0x62948.
- (NSDictionary *)getDataInJSON;

// Bytes buffered so far. Ghidra: @ 0x62888.
- (NSUInteger)currentSize;

// Buffered bytes / expected content length (0..1), or 0 when the length is
// unknown. Ghidra: @ 0x628a8.
