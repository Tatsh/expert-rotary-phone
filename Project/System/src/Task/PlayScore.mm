...
//  pop'n rhythmin
//
//  The standard-mode play scene's score / gauge-feedback / song-clear sound logic,
//  reconstructed from Ghidra project rb420, program PopnRhythmin. Three seams the
//  play task declares live here:
//
//    - PlayCurrentScore     Ghidra: FUN_0002ff7c
//        The running score: a weighted accuracy of the per-note-kind hit tally,
//        scaled to a 100000-point maximum. PlayTask_update state 6 stores its result
//        into the play data (+0x9b0) each frame and the results screen reads it back.
//
//    - PlayScoreGaugeUpdate Ghidra: FUN_00031338
//        Called after any frame that resolved a note (PlayJudge_update's tail). Its
//        name is historical: the running score itself is recomputed by the state-6
//        caller via PlayCurrentScore; this routine only (re)fires the per-judge gauge
//        feedback SE, retriggering the loaded source (+0x398) and dropping the prior
//        instance (+0x3a0), gated by the gauge-enable count (+0x9b4) and skipped while
//        the pause menu (state 5) is up. Verified against the disassembly, not the
//        heavier score math the name suggests.
//
//    - PlayEndResultSe      Ghidra: the rank-SE cascade in PlayTask_update
//        (FUN_0002dc14) state 6, ~0x2e0d0..0x2e190. Chooses one of several pre-created
//        SE-instance jingles from the final score + combo/tally, plus a clear fanfare.
//
//  The play data is the standard-mode MainTask (PlayJudge.h's forward-declared
//  MainTaskPlayData); the fields these routines touch are reached by cited byte offset
//  in the pd()/pdw() style PlayJudge.mm established.
//
...
// Same convention as PlayJudge.mm: the standard-mode MainTask is not reconstructed
// as a whole, so its fields are reached by documented byte offset.
namespace {
...
// The per-play SE-instance handles the play data pre-creates. Each is a pointer to an
// SeInstance controller (see SeInstance.h), owned by the scene. Ghidra: the play task
// fires *(playData + off) as an SE-instance through FUN_0002cba4/cac0/cb24.
...
// One tally snapshot of the global note manager. The engine keeps a hit count per note
// kind (0..9) per judge tier (COOL/GREAT/GOOD/BAD). Ghidra: the three stride-0x10
// accumulation loops over the tally columns at NoteMng +0x5170 (COOL), +0x516c (GREAT)
// and +0x5168 (GOOD) in FUN_0002ff7c. The score's denominator is the fixed chart note
// total (DAT_00178ccc, NoteMng::totalNoteCount()), not this running sum.
// Only the three scored tiers are summed; BAD carries no weight and is not read by
// either FUN_0002ff7c or the FUN_00031868 predicate, matching the binary.
struct TallyTotals {
    int cool = 0;
    int great = 0;
    int good = 0;
...
int PlayCurrentScore() {
    ...
    // score = (COOL*1.0 + GREAT*0.7 + GOOD*0.4) * 100000 / totalNotes, accumulating
    // toward the 100000-point maximum as notes are hit.
    const float weighted = static_cast<float>(t.cool)
                         + static_cast<float>(t.great) * kWeightGreat
...
void PlayScoreGaugeUpdate(MainTaskPlayData *playData) {
    // Play the per-tap feedback SE only when the user's touch-sound volume is on, and
    // never while the pause menu is up (state 5).
...
// Ghidra: the SE-instance rank cascade in PlayTask_update (FUN_0002dc14) state 6,
// ~0x2e0d0..0x2e190. `score` is the value PlayCurrentScore produced (play data +0x9b0).
void PlayEndResultSe(void *playData, int score) {
    ...
    if (score < kClearScore) {
        // Below the clear line: the "not cleared" jingle, full-combo variant if unbroken.
        firePlay(playData, fullCombo ? kSeFailFC : kSeFailMiss);
        return;
    }

    // Cleared. The top-rank jingles require a spotless sheet: no GOOD or BAD, i.e.
    // COOL + GREAT covers every note (Ghidra: FUN_00031868 predicate, the summed
    // COOL + GREAT tally columns >= the note-total threshold).
