...
//  animation layers it drives (m_layers @ +0x214..+0x228) are AepLyrCtrl overlays that
//  the intro/score cues also poke as SeInstance controllers (see SeInstance.h). Progress
//  tracked in STUBS.md.
//
...
//  offsets. The whole body is memset 0 by the ctor (FUN_0003d5bc: memset +0x28..+0x3a0
//  == 0x378 bytes). Every offset the reconstructed methods (resultSetup / the update
//  state machine / loadNumberTextures / resultGotoNext / the draw callback) reach by flat
//  `*(T*)(this+off)` in the binary is named at its exact offset (with a `// +0xNN`
//  comment); genuine gaps are `_rsvd_NN[]` fillers. Two interior scratch regions
//  (+0x310..+0x32b and +0x330..+0x33b) are never reached by any reconstructed method and
//  are kept as documented reserved fillers.
//
//  NOTE (target ABI): offsets assume the 32-bit ARMv7 target (pointers and `unsigned
//  long` are 4 bytes), matching the reference reworks; this header is documentary and is
//  not host-compiled.
//
...
private:
    ...
    // The per-frame draw pass reaches the members below through its `context` (= this),
    // so it is a friend rather than reaching them by raw offset.
    friend void PlayResultDrawCallback(int child, int frame, int x, int y, int scaleX,
                                       int scaleY, int anchorX, int anchorY, int color,
                                       int alpha, int rotation, uint32_t blend,
                                       int *clipRect, uint32_t p17, void *context);
    ...
    // --- 12 digit-strip texture rows (0..9 glyphs each), drawn by the num_* branches ---
    ...
    // --- 6 result overlay layers (device-branched names; owner = this). The intro/score
    //     layers [0..3] are also poked as SeInstance controllers (SeInstance*, void*). ---
    ...
    // Indexed slots the cue code names directly: [1] v32 perfect jingle, [6] v38 line-in,
    // [7] se07_count tick, [8] se08_bonus_fai, [9] se09_bonus_cl.
    uint32_t         m_rankSe[11] = {};              // +0x2e4 (..+0x30f)

    uint8_t          _rsvd_310[0x32c - 0x310] = {};  // +0x310 scratch (unreached by this class)
    int              m_countSeInst = 0;              // +0x32c currently-playing count-SE instance id
    uint8_t          _rsvd_330[0x33c - 0x330] = {};  // +0x330 scratch (unreached by this class)
    ...
    uint8_t          _rsvd_35e[0x360 - 0x35e] = {};  // +0x35e
