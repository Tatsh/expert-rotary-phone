...
// Ghidra: base ctor FUN_00027ea8 — sets the vtable, self-links the node,
// defaults the priority to 9, and clears the name + killed flag. It stays
...
// Ghidra: base dtor @ 0x27ec8 (mislabelled "caSourceNode_dtor") — re-install
// the base vtable, unlink the node from the scheduler list, then free the owned
// name buffer. The compiler's deleting-destructor thunk @ 0x27ef8
...
C_TASK::~C_TASK() {
    ...
    delete[] m_name; // the node owns its name buffer (the binary frees and nulls
                     // it here)
    m_name = nullptr;
}
