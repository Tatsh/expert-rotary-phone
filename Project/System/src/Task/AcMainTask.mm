...
void AcMainTask::setupScene() {
    ...
    if (!pad) {
        ...
        m_bgTileW = 0x280;  m_bgTileH = 0x470;  m_selSceneLayout[0] = 0x1a6;
        m_selSceneLayout[1] = -0x149; m_selSceneLayout[2] = 0xd0;   m_selSceneLayout[3] = 0x8a;
        m_selSceneLayout[4] = 0x15;   m_selSceneLayout[5] = -0xdd;  m_selSceneLayout[6] = 0x9c;
        m_selSceneLayout[7] = 0x34;   m_selSceneLayout[8] = 0x11;   m_selSceneLayout[9] = 0xe9;
        m_selSceneLayout[10] = 0x1c1;  m_selSceneLayout[13] = -0x7c;  m_selSceneLayout[14] = 0xb4;
        m_selSceneLayout[15] = 0x7c;
        m_dlgLayoutA[0] = 0x94;   m_dlgLayoutA[1] = 0x334;  m_dlgLayoutA[2] = 0x168;
        m_dlgLayoutA[3] = 0x7d;   m_dlgLayoutA[4] = 0x136;  m_dlgLayoutA[5] = 0x2a8;
        m_dlgLayoutA[6] = 0x110;  m_dlgLayoutA[7] = 0x60;   m_dlgLayoutA[8] = 0x22;
        m_dlgLayoutA[9] = 0x2a8;  m_dlgLayoutA[10] = 0x110;  m_dlgLayoutA[11] = 0x60;
        m_dlgPanelW = 0x1dc;  m_dlgPanelH = 0xe2;   m_dlgBtn1X = 0xf5;
        m_dlgBtn1Y = 0x70;   m_dlgBtn1W = 0xe6;   m_dlgBtn1H = 0x5c;
        m_dlgBtn2X = 5;      m_dlgBtn2Y = 0x70;   m_dlgBtn2W = 0xe6;
        m_dlgBtn2H = 0x5c;   m_dlgLayout954 = 0x20;   m_dlgLayoutB[0] = 0;
        m_dlgLayoutB[1] = 0;      m_dlgLayoutB[2] = 0x8c;   m_dlgLayoutB[3] = 0x50;
        m_dlgLayoutB[4] = 0;      m_dlgLayoutB[5] = 0;      m_dlgLayoutB[6] = 0x8c;
        m_dlgLayoutB[7] = 0x50;   m_dlgLayoutB[8] = 0;      m_dlgLayoutB[9] = 0;
        m_dlgLayoutB[10] = 0x8c;   m_dlgLayoutB[11] = 0x50;   m_dlgLayoutB[12] = 0xcd;
        m_dlgLayoutB[13] = m_layoutAnchorZ + 0x35a;
        m_dlgLayoutB[14] = 0xf0;   m_dlgLayoutB[15] = 0x5c;
    } else {
        m_bgTileW = 0x300;  m_bgTileH = 0x400;  m_selSceneLayout[0] = 0x4de;
        m_selSceneLayout[1] = -0xda;  m_selSceneLayout[2] = 0x122;  m_selSceneLayout[3] = 0xda;
        m_selSceneLayout[4] = 10;     m_selSceneLayout[5] = -0x170; m_selSceneLayout[6] = 0xeb;
        m_selSceneLayout[7] = 0x5c;   m_selSceneLayout[8] = 3;      m_selSceneLayout[9] = 0x119;
        m_selSceneLayout[11] = 0x22f;  m_selSceneLayout[12] = 0x345;  m_selSceneLayout[13] = -0xba;
        m_selSceneLayout[14] = 0x116;  m_selSceneLayout[15] = 0xba;
        m_dlgLayoutA[0] = 0xfe;   m_dlgLayoutA[1] = 0x6b0;  m_dlgLayoutA[2] = 0x168;
        m_dlgLayoutA[3] = 0x7d;   m_dlgLayoutA[4] = 0x19e;  m_dlgLayoutA[5] = 0x620;
        m_dlgLayoutA[6] = 0x110;  m_dlgLayoutA[7] = 0x60;   m_dlgLayoutA[8] = 0x88;
        m_dlgLayoutA[9] = 0x620;  m_dlgLayoutA[10] = 0x110;  m_dlgLayoutA[11] = 0x60;
        m_dlgPanelW = 0x329;  m_dlgPanelH = 0x183;  m_dlgBtn1X = 0x1b0;
        m_dlgBtn1Y = 0xd7;   m_dlgBtn1W = 0x150;  m_dlgBtn1H = 0x6e;
        m_dlgBtn2X = 0x22;   m_dlgBtn2Y = 0xd7;   m_dlgBtn2W = 0x150;
        m_dlgBtn2H = 0x6e;   m_dlgLayout954 = 0x30;   m_dlgLayoutB[0] = 0x119;
        const int y = m_overlayH - 0xba;   // iVar5 in the decompile
        m_dlgLayoutB[1] = y;      m_dlgLayoutB[2] = 0x116;  m_dlgLayoutB[3] = 0xba;
        m_dlgLayoutB[4] = 0x22f;  m_dlgLayoutB[5] = y;      m_dlgLayoutB[6] = 0x116;
        m_dlgLayoutB[7] = 0xba;   m_dlgLayoutB[8] = 0x345;  m_dlgLayoutB[9] = y;
        m_dlgLayoutB[10] = 0x116;  m_dlgLayoutB[11] = 0xba;   m_dlgLayoutB[12] = 0x1ee;
        m_dlgLayoutB[13] = 0x5c8;  m_dlgLayoutB[14] = 0x230;  m_dlgLayoutB[15] = 0x8c;
    }
...
void AcMainTask::setupResolveHandles() {
    ...
    for (int i = 0; i < 26; i++) m_boardFrame[i] = aep.getFrameNo(5, kFrmBoard[i]);
    for (int i = 0; i < 11; i++) m_base1Frame[i] = aep.getFrameNo(5, kFrmBase1[i]);
    if (m_padDisplay != 0) {   // pad only
        m_rouletteMoveFrame = aep.getFrameNo(5, "BT_ROULETTE_MOVE");
    }
    for (int i = 0; i < 10; i++) m_base08Frame[i] = aep.getFrameNo(5, kFrmBase08[i]);
    for (int i = 0; i < 4;  i++) m_base05Frame[i] = aep.getFrameNo(5, kFrmBase05[i]);
    ...
    for (int i = 0; i < 26; i++) m_boardUserNo[i] = aep.getUserNo(5, kUsrBoard[i]);
}
...
void AcMainTask::setupBuildOverlays() {
    ...
    // m_layoutAnchorZ); indices derived from the +0x18/+0x1c store offsets.
...
void AcMainTask::loadTreasureMap() {
    ...
    for (AepLyrCtrl *l : m_rouletteLayers) if (l) l->stopPlay();   // 29 roulette
    for (AepLyrCtrl *l : m_panelLayers)    if (l) l->stopPlay();   // 8 panels
    for (AepLyrCtrl *l : m_arrowLayers)    if (l) l->stopPlay();   // 4 arrows
    if (m_boardBgLayer) m_boardBgLayer->stopPlay();
    ...
    std::memcpy(&m_boardVisited[0], tmp.raw0x35, 15);   // board-visited bitmap (0x894..0x8a2)
...
void AcMainTask::buildMapCharaLayers() {
    ...
    reinterpret_cast<uint32_t &>(m_musicPieceTable[curIdx / 4]) |= (uint32_t)tmp.raw0x08;   // music mask
    reinterpret_cast<uint32_t &>(m_wallPieceTable[curIdx / 4]) |= (uint32_t)tmp.raw0x0c;   // wallpaper mask
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// Sugoroku board draw / logic — the group-5 render callback's inlined sub-passes.
// These operate on the arcade task's own work area, so they are real AcMainTask
// members (Ghidra placed them as free functions taking the task pointer; the
// declared "SugorokuMainTask" a prior agent invented was a mis-attribution).
// ═══════════════════════════════════════════════════════════════════════════════
...
int AcMainTask::sugorokuDrawButtonHitTest() {
    ...
    mgr->drawLayer(m_skillBoardLyr[1], panelW / 2,
                   iVar5, iVar6,
...
void AcMainTask::sugorokuSetupScrollBounds() {
    ...
    m_scrollVelX =
        (m_scrollX < targetX) ? 10.0f : -10.0f;
    m_scrollVelY =
        (m_scrollY < targetY) ? 10.0f : -10.0f;
...
void AcMainTask::sugorokuTaskDispose() {
    ...
    auto dropTex = [](neTextureForiOS *&s) { if (s) { delete s; s = nullptr; } };
    dropTex(m_circleTex);
    dropTex(m_boardBgTex);
    dropTex(m_charaTex);
    dropTex(m_goalCharaTex);
    dropTex(m_blindCircleTex);
    for (neTextureForiOS *&s : m_reserveTex) dropTex(s);

    // 2. Delete the three 10-glyph digit texture sets (points / roulette / ticket).
    for (neTextureForiOS *&s : m_pointsDigitTex) dropTex(s);
    for (neTextureForiOS *&s : m_roulDigitTex)   dropTex(s);
    for (neTextureForiOS *&s : m_ticketDigitTex) dropTex(s);

    // 3. Delete the music-jacket (9), wall-nail (9) and event (12) textures
    //    (Ghidra slot range 0x69..0x86 == byte offsets 0x1a4..0x218).
    for (neTextureForiOS *&s : m_jacketTex)   dropTex(s);
    for (neTextureForiOS *&s : m_wallNailTex) dropTex(s);
    for (neTextureForiOS *&s : m_eventTex)    dropTex(s);
    ...
    auto dropLayer = [](AepLyrCtrl *&lyr) { if (lyr) { lyr->unlink(); delete lyr; lyr = nullptr; } };
    for (AepLyrCtrl *&lyr : m_rouletteLayers) dropLayer(lyr);   // offsets 0x2c..0x9c
    for (AepLyrCtrl *&lyr : m_arrowLayers)    dropLayer(lyr);   // offsets 0xc0..0xcc
    for (AepLyrCtrl *&lyr : m_panelLayers)    dropLayer(lyr);   // offsets 0xa0..0xbc
    ...
    // 9. Release Objective-C objects stored in the blob (null the raw slots, as the
    //    binary does — the retained references were already dropped by the flow above).
    m_mapName = nullptr;
    m_gotCharaArray = nullptr;
    m_treasureMusicArray = nullptr;
    ...
    for (int se : m_rouletteSe) {
        [audioMgr releaseSe:nil
                 resourceId:static_cast<RSND_SOURCE_ID>(static_cast<uint32_t>(se))];
    }
...
void AcMainTask::sugorokuDrawBoard() {
    ...
    const TreasureMap::ConnectStruct *edges =
        reinterpret_cast<const TreasureMap::ConnectStruct *>(
            static_cast<intptr_t>(m_edgesPtr));
...
void AcMainTask::sugorokuDrawPlayerAndUi() {
    ...
    if ((m_boardMoveState & ~1) == 2) warpSX = -warpSX;

    // Player sprite (hidden during warp flash).
    if (m_warpFlash == 0) {
        drawSprite(mgr, m_charaTex,
                   0, 0, 0x228, 0x228,
    ...
    if (m_warpFlash == 0) {
        ...
        if (roulVal >= -1) {
            ...
            if (!resultLyr || !resultLyr->isAnimating()) {
                ...
                case 12: rHandle = m_boardFrame[16]; break;
                case 13: rHandle = m_boardFrame[17]; break;
                case 16: rHandle = m_boardFrame[19]; break;
                case 19: rHandle = m_boardFrame[18]; break;
                case 20: rHandle = m_boardFrame[22]; break;
                case 21: rHandle = m_boardFrame[23]; break;
                case 22: rHandle = m_boardFrame[24]; break;
...
void AcMainTask::sugorokuDrawFriendMeet() {
    ...
    __unsafe_unretained NSString *nameStr =
        (__bridge NSString *)m_mapName;
    if (nameStr) {
        mgr->drawLayer(m_skillBoardLyr[4], 0,
                       iVar7, iVar6 + 0x5a,
