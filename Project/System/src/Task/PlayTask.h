...
//  (scene textures @ +0x28, resolved Aep layer/frame/user-number tables @ +0xc4, pause-
//  menu + note-field + popkun geometry @ +0x978) are the members the play-scene lifecycle
//  seams (PlayScene.mm: PlayTaskInit / PlayBuildFieldLayers / PlayTaskDraw / PlayTaskGotoResult)
//  fill and read, so they are named here at their exact offsets; the few sub-regions no
//  reconstructed function reaches stay documented `_rsvd_NN[]` fillers. The per-note judge
//  pool @ +0x3c8 is the real NoteJudgeState[60] array PlayJudge.h defines.
//
...
public:
    ...
    // This is a flat engine work area the whole play scene shares: the state-machine member
    // update()/resetState()/updateGauge() and the free play-scene lifecycle seams in
    // PlayScene.mm (PlayTaskInit, PlayBuildFieldLayers, PlayLoadCharaTextures, PlayTaskDraw,
    // PlayTaskGotoResult — some are C callbacks that must stay free functions) all reach these
    // fields, so they are public named members rather than raw `*(T*)(this+off)` offset access.

    // +0x28 scene textures (neTextureForiOS*), allocated by PlayLoadCharaTextures and freed in
    // PlayTaskGotoResult. Slot 1 of the first pair is the demo window frame (t_window @ +0x2c).
    ...
    // +0xc4 resolved Aep layer-no / frame-count / user-no tables. PlayBuildFieldLayers fills them
    // (AepManager getLyrNo / layerFrameCount / getFrmNo / getUsrNo); PlayTaskDraw reads them to
    // pick each note / digit / tone / chara sprite. Names track the getLyrNo/getFrmNo tables.
    ...
    // +0x97c device-branched pause-menu + note-field geometry (phone/pad constants). The pause
    // fields are verified against the state-5/6 hit tests in PlayTask_update; the note-field
    // fields are consumed by the delegated note-quad draw.
