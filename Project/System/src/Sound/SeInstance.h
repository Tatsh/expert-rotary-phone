...
//  pop'n rhythmin
//
//  The play scene pre-creates a handful of one-shot "SE-instance" objects — small
//  playback controllers that wrap a loaded sound and drive it through the per-frame
//  SE mixer. Each object carries a play cursor (+0x40, float frames), a playback
//  rate (+0x44, float; negative = play backwards), a frame count (+0x3c, int) and a
//  command byte (+0x58, int: 0 idle, 1 play-forward, 3 play). The combo-milestone
//  jingles (PlayJudge) and the song-clear rank jingles (PlayScore) fire these.
//
...
// One playback controller. Only the four fields the controllers touch are named;
// the object is owned by the play scene and reached as an opaque pointer.
struct SeInstance {
...
// Arm forward (or, for a reverse-rate controller, backward) playback and rewind the
// cursor to the head (or tail). A zero rate is normalised to 1.0. Ghidra: FUN_0002cac0.
...
// Issue the generic "play" command. With mode == 1 the cursor is also rewound: to the
// head for a forward rate, or to the last frame when the rate runs backwards.
// Ghidra: FUN_0002cb24.
