...
// rather than through a fade timer. This is one fade tick (0.05s).
...
// One live SE instance tracked for voice-stealing (the 8-entry seList, 0xc
// bytes each). handle == kFreeInstance marks a free slot. The struct is 12
...
// slots, 0xc bytes each). Unlike m_seList, every slot owns a *fixed*
...
// .cxx_construct — compiler-emitted C++ ivar constructor (constructs the C++
// SeInstance/SeVoiceSlot ivars); not hand-written.
...
// Invalidate the fade timer and tear down the CoreAudio SE engine.
...
// Start initialisation asynchronously via a zero-delay run-loop timer.
- (void)systemStart {
...
// The legacy synchronous start.
- (void)systemStartBlock {
    [self onStartPlayer:nil];
}

// Bring both SE backends up and mark the system started.
...
// Pause both SE backends + both BGM slots.
- (void)systemSuspend {
...
// Configure a freshly-loaded BGM player.
...
// Stop and release the current BGM + its remembered path.
- (void)releaseBgm {
    if (m_bgmPlayer != nil) {
        [m_bgmPlayer stop];
        m_bgmPlayer = nil;
...
// Load a BGM file (skipping the reload if the same path is already loaded).
// Returns NO on a nil path or a decode error.
...
// Load the BGM straight from in-memory data (the play scene hands in the
// decoded .orb "bgm" entry) rather than a file path. Mirrors
...
// Wrap a copy of the raw bytes in an NSData and load it as BGM.
...
// As above but without copying (NSData frees the buffer when done).
...
// As above, letting the caller decide whether NSData frees the buffer.
...
// Stop the BGM (immediately for a ~zero fade, else via a fade timer).
...
// YES only when a player is loaded and actually playing.
- (BOOL)isPlayingBgm {
    if (m_bgmPlayer == nil) {
        return NO;
    }
...
// The player's current playhead (seconds); 0 when nothing is loaded.
...
// The audio device's absolute clock as seen by the BGM player; 0 when nothing
// is loaded.
...
// Move the playhead and re-prime the player so playback resumes cleanly.
...
// Pause the BGM (immediately or via a fade-out timer).
...
// Remember the requested BGM volume (0..1).
...
// Set the BGM player's volume immediately (no fade, no stored target).
...
// Cancel any running BGM fade timer.
...
// Duck the current BGM onto the stack.
- (void)pushBgm {
...
// The group a loaded SE belongs to (0 = caplayer, else AVFoundation), looked up
// in m_seType by call name or by boxed resource id.
...
// Load a sound effect into one of the two backends.
...
// Reserve a playing instance in the right backend, stealing an old instance if
// the backend is out of voices; register it and return the handle.
...
// Play a sound effect; group 0 goes through the CoreAudio caplayer, others
// through the AVFoundation player.
...
// As playSe:resourceId: but with an explicit per-shot volume level rather than
// the group's stored level. Self-contained in the binary: it clamps the level,
// prepares the instance at that level, and starts it; it does not touch
// m_seVolume nor delegate to playSe:resourceId:. The level is an integer clamped
// to [0, 127] and forwarded raw to prepare:resourceId:volume: — the same raw
// 32-bit forwarding the other play/prepare paths use (m_seVolume itself holds
// the integer level 0..127, e.g. init stores 0x7f, not the float 127.0f), so the
// level is passed straight through with no int->float conversion.
//
// The control flow now matches the binary (the prior reconstruction wrongly
// wrote m_seVolume[group] and delegated to playSe:resourceId:), but
// prepare:resourceId:volume: is declared with a float volume, so passing the
// clamped int level implies an int->float conversion the binary does not perform
// (it stores the raw int level into the forwarded slot). Making this bit-faithful
// requires re-typing the whole prepare/backend volume path (m_seVolume is a
// float[] yet init stores the integer 0x7f into it) across sibling files, which
...
// Stop the SE instance `handle` in whichever backend owns it.
...
// Stop every tracked SE instance.
- (BOOL)stopSeAll {
    for (int i = 0; i < 8; i++) {
...
// Reap finished SE instances (state -1/4), freeing their voices, then compact
// the list so the oldest live instance is first.
...
// Steal the oldest instance's voice to make room, shifting the list.
...
// Record a newly-prepared instance in the first free slot.
...
// Set the SE volume for a group (level 0..127): the caplayer sets its 8 voices,
// the AVFoundation pool scales each player. The group index is stored
// unconditionally (no bounds check in the binary) before the group split.
- (void)setSeVolume:(int)volume groupId:(int)group {
    ...
    if (group != 0) {
        // Tail-call into neAVSePlayer::setGroupVolume with the raw integer level;
        // the /127.0f conversion happens inside that method, not here.
        m_seAVPlayer->setGroupVolume(volume);
    } else {
        // The caplayer level is applied by calling the per-voice gain setter once
        // for each of the 8 voices.
...
// Pause the SE instance `instanceId` in whichever backend owns it.
...
// Resume the SE instance `instanceId` in whichever backend owns it.
...
// YES if the SE instance `instanceId` is currently playing (state 2).
...
// Pause every tracked SE instance (all 8 slots, backend per slot).
- (BOOL)onPauseSeAll {
    for (int i = 0; i < 8; i++) {
        if (m_seList[i].group != 0) {
...
// Resume every tracked SE instance.
- (BOOL)offPauseSeAll {
    for (int i = 0; i < 8; i++) {
        if (m_seList[i].group != 0) {
...
// Reap finished voices in the bank, compact the live slots to the front (each
// keeping its fixed voiceIndex), and return the first free slot index, or -1
...
// Reserve a caplayer voice in the given bank (stealing the bank's oldest
// instance if it is full), prepare it on that slot's fixed voice, record the
...
// Prepare a SetGroup voice and start it.
...
// Free a single loaded SE source (by call name or source id) from its backend
// and drop it from the load-order lists and the type table.
...
// Free every loaded SE source in both backends and clear the tables.
- (void)releaseSeAll {
...
// Full SE teardown/reset: release every source, destroy and rebuild both
// backends, reset the lookup lists and both instance pools, then restart
...
// Load the VOICE channel straight from in-memory data.
...
// YES only when a VOICE player is loaded and actually playing.
- (BOOL)isPlayingVoice {
    if (m_voicePlayer == nil) {
        return NO;
    }
...
// Ramp the BGM volume up to its target over `seconds`.
- (void)createBgmFadeInTimer:(float)seconds {
    [self deleteFadeTimer];
    const float kTick = 0.05f; // per-tick interval (0.05s)
...
// Ramp the BGM volume down to zero over `seconds`.
...
// Ramp the BGM volume up one tick; clamp to the target and stop the timer once
// reached. Ignores stale timers that have already been replaced.
...
// Ramp the BGM volume down one tick; on reaching zero stop the timer and either
// pause (when this fade is a pause) or stop the player outright.
...
// Interrupt a channel (0 = BGM, 1 = VOICE), stopping its player.
...
// Resume a channel that was interrupted and had been playing.
- (void)resumePlayer:(int)which {
    ...
    if (which == 1) {
        ...
        if (m_isPlaying && !m_isOnPause) {
            // Resume fades the BGM back in over 0.3s.
            [self playBgm:0.3f];
        }
    }
}
...
// Stop the BGM, the VOICE channel and every SE instance at once.
- (BOOL)stopAll {
    [self stopBgm:0];
    [self stopVoice];
    [self stopSeAll];
...
// A player finished: clear the matching channel's playing flag.
...
// Audio session interrupted: mark the matching channel interrupted.
...
// Interruption ended: resume the matching channel (0 = BGM, 1 = VOICE).
- (void)audioPlayerEndInterruption:(AVAudioPlayer *)player {
    [self resumePlayer:(m_bgmPlayer != player) ? 1 : 0];
}

// Tear the SE backends down.
