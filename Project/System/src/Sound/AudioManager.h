...
/**
 * A lib_rsnd playing-instance identifier returned by playSe and used to stop that instance
 * later.
 */
...
/**
 * The app-wide sound singleton: BGM, VOICE, and sound-effect playback.
 *
...
/**
 * Return the thread-safe lazy singleton, creating it on first use.
...
/**
 * Load a BGM file, skipping the reload when the same path is already loaded.
...
/**
 * Load the BGM straight from in-memory data rather than a file path.
...
/**
 * Load the raw bytes as BGM without copying them.
...
/**
 * Load the raw bytes as BGM without copying, letting the caller keep ownership.
 * @param bytes The raw audio bytes to load without copying.
 * @param length The number of bytes.
...
/**
 * Pause the BGM, instantly or via a fade-out timer.
...
/**
 * Rewind the BGM playhead to the start.
 * @ghidraAddress 0x202e0
 */
- (void)seekBgmToTop;
/**
 * Remember the requested BGM volume applied on the next play.
...
/**
 * Set the BGM volume immediately with no fade and no stored target.
...
/**
 * Stop the BGM, instantly or via a fade-out timer.
...
/**
 * Duck the current BGM onto the stack so another can play over it.
 * @ghidraAddress 0x201dc
 */
- (void)pushBgm;
/**
 * Restore the BGM previously ducked with pushBgm.
 * @ghidraAddress 0x2027c
 */
- (void)popBgm;
/**
 * Report whether a BGM has been ducked onto the stack.
...
/**
 * Load a file into the VOICE channel, a second BGM-like channel with no fade.
 * @param path The file path to load.
...
/**
 * Start the VOICE channel, resuming from the paused position when applicable.
...
- (BOOL)stopVoice;
/**
 * Pause the VOICE channel, remembering its position for the next play.
 * @return YES on success; NO when no VOICE is loaded.
 * @ghidraAddress 0x203c8
 */
- (BOOL)onPauseVoice;
/**
 * Report whether the VOICE channel is playing.
 * @return YES while the VOICE player exists and is playing.
 * @ghidraAddress 0x2042c
 */
- (BOOL)isPlayingVoice;
/**
 * Stop and free the VOICE player.
 * @ghidraAddress 0x1efdc
 */
- (void)releaseVoice;

/**
 * Load a sound effect, keeping it addressable by call name and by the returned source id.
 * @param path The file path to load.
...
- (RSND_SOURCE_ID)loadSe:(NSString *)path
                ...
                callName:(NSString *)name
                   group:(int)group;
/**
 * Play a sound effect by call name or by source id at the group's stored volume.
...
/**
 * Play a sound effect at an explicit per-shot volume rather than the group's stored level.
 * @param name The registered call name, or nil to play by source id.
 * @param resourceId The source id to play when no call name is given.
...
- (RSND_INSTANCE_ID)playSe:(NSString *)name
                resourceId:(RSND_SOURCE_ID)resourceId
                    Volume:(int)volume;
/**
 * Stop the SE instance in whichever backend owns it.
...
/**
 * Stop every tracked SE instance across both backends.
 * @return YES.
 * @ghidraAddress 0x1f630
 */
- (BOOL)stopSeAll;
/**
 * Set the SE volume for a group.
...
/**
 * Play a sound effect through the caplayer's fixed-voice SetGroup pool.
...
- (RSND_INSTANCE_ID)playSeSetGroup:(NSString *)name
                        resourceId:(RSND_SOURCE_ID)resourceId
                           groupId:(int)groupId;
/**
 * Pause a single playing SE instance in whichever backend owns it.
...
/**
 * Resume a single paused SE instance in whichever backend owns it.
...
/**
 * Report whether a single SE instance is currently playing.
...
/**
 * Pause every tracked SE instance across both backends.
 * @return YES.
 * @ghidraAddress 0x1f568
 */
- (BOOL)onPauseSeAll;
/**
 * Resume every tracked SE instance across both backends.
 * @return YES.
 * @ghidraAddress 0x1f5cc
 */
- (BOOL)offPauseSeAll;
/**
 * Free a loaded SE source by call name or source id.
...
- (void)releaseSeAll;
/**
 * Stop and free the current BGM resource and its remembered path.
 * @ghidraAddress 0x1ef74
 */
- (void)releaseBgm;
/**
 * Tear down every playing SE instance and mixer state during scene teardown.
