...
//  pop'n rhythmin
//
//  The app-wide sound singleton. BGM plays through AVAudioPlayer with a
//  push/pop stack (so a screen can duck the current BGM and restore it); VOICE
//  is a second BGM-like channel; sound effects go through a low-latency C++
//  CoreAudio player (caplayer / lib_rsnd, group 0) or an AVFoundation player
//  (other groups).
//
...
//  changelog (log.txt). Per that manual the class is REFLEC-derived, modelled
//  on jubeat's AudioManager, with SE using the same API as the SSC sound
//  library and a dedicated VOICE channel added. It depends on inc/caplayer.h
//  and the lib_rsnd static library (CoreAudio) plus the AudioToolbox and
...
// initialisation on a timer and returns immediately; poll isStart for
// completion. systemStartBlock does the same work synchronously (the legacy ~3s
// block).
...
// applicationDidBecomeActive. Ghidra: systemSuspend @ 0x205e0 / systemResume @
// 0x2065c.
...
// (PTR_s_setJustBgmVolume__0015b43c), called from -[SoundSettingView
// bgmSliderValChanged:].
...
// stopAll @ 0x1f694.
- (BOOL)stopAll;
// YES while the BGM player exists and is playing. Ghidra: isPlayingBgm @
// 0x1fff8.
- (BOOL)isPlayingBgm;
// The BGM player's playhead (seconds). Ghidra: bgmCurrentTime @ 0x1ff58 /
// setBgmCurrentTime: @ 0x1ffb0 (the setter also re-primes the player via
// prepareToPlay).
...
- (RSND_SOURCE_ID)loadSe:(NSString *)path
                ...
                callName:(NSString *)name
                   group:(int)group; // Ghidra: @ 0x1e914
...
// Play an SE through the caplayer's fixed-voice "SetGroup" pool: groupId
// selects one of two banks of 8 caplayer voices (each slot owns a fixed voice),
// stealing the bank's oldest voice when it is full. Returns the play-instance
// id. Ghidra: playSeSetGroup:resourceId:groupId: @ 0x1f380.
- (RSND_INSTANCE_ID)playSeSetGroup:(NSString *)name
                        resourceId:(RSND_SOURCE_ID)resourceId
                           groupId:(int)groupId;
// Pause / resume / query a single playing SE instance in whichever backend owns
// it. Ghidra: onPauseSe: @ 0x1f434, offPauseSe: @ 0x1f498, isPlayingSe: @
// 0x1f4fc.
...
// releaseSe:resourceId: (PTR_s_releaseSe_resourceId_), called during scene
// teardown.
...
- (void)releaseSeAll;
// Free the current BGM resource. Ghidra: AudioManager::releaseBgm
// (PTR_s_releaseBgm).
- (void)releaseBgm;
