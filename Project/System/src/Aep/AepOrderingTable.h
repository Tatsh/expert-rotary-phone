...
// A queued text draw command (Ghidra: the type-6 entry pushAepOtTextCmd fills, same
// 0x134-byte pool slot as AepSpriteCommand, reinterpreted by type). The string sits in
...
struct AepTextCommand {
    ...
    // their meaning (position / size / colour); they are copied through verbatim by the
    // push, so they are named positionally here to avoid asserting the wrong roles.
    int32_t arg0;             // +0x10c
    int32_t arg1;             // +0x110
    int32_t arg2;             // +0x114
    int32_t arg3;             // +0x118
...
public:
    ...
    // Screen extents and the device-pixel render scale the immediate-mode primitive
    // helpers (the drawAepOt* set) transform their coordinates by, plus the per-slot
    // GL texture-handle table sprites resolve their texture from. Ghidra:
    // aepOtSetScreenParams FUN_00010bbc (writes +0x04 / +0x08 / +0x9a1a4 / +0x9a1a8).
    void setScreenParams(void **textureTable, int screenW, int screenH, float scale);
    int screenW() const { return m_screenW; }             // +0x04
    int screenH() const { return m_screenH; }             // +0x08
    void **textureTable() const { return m_textureTable; }// +0x9a1a4
    float renderScale() const { return m_renderScale; }   // +0x9a1a8
...
// ---------------------------------------------------------------------------
// The ordering table's immediate-mode primitive draw helpers. Each takes the OT (for
// the render scale + texture table), transforms its coordinates by OT::renderScale(),
// unpacks the packed colour (0xRRGGBB) and 0..100 alpha, and issues the corresponding
// neGraphics primitive. These are the type-dispatch handlers the flush runs per queued
// command; they are also called directly by the UI draws. The neDraw* renderer entries
// are reconstructed in parallel in the neGraphics unit (forward-declared in the .mm).
// ---------------------------------------------------------------------------
...
// Queue a text draw command (type 6) at `priority`. `colorVec` (16 bytes) overrides the
// per-glyph colour vector; when null it defaults to {0,0,screenW,screenH}. Ghidra:
// pushAepOtTextCmd (FUN_0001154c).
...
// Immediate-mode primitive draws (Ghidra: FUN_00010f98 / _11054 / _1113c / _111f8 / _11310).
...
// Textured sprite draws (Ghidra: FUN_00010c90 / FUN_00010e18 -> drawAepSpriteClipped
// FUN_00012020). `spriteRec` points at the sprite's 8-byte source-rect record.
...
// The clipped textured-quad immediate draw (Ghidra: FUN_00012020). `frameObj` is the
// animated texture object: it holds the sub-frame count (+0x04), the per-frame width and
// duration tables (+0x08 / +0x0c) and the render-state slots (+0x14). It picks the active
// sub-frame from `frameTime`, composes the source rect, sets the render state and issues
// neDrawTexturedQuad, optionally through a scaled clip rect.
