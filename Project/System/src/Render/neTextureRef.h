...
//  pop'n rhythmin
//
//  Two small ref-counted texture holders that sit on top of the shared
//  AepTexture cache: a single-texture handle (neTextureRef) and a multi-frame
//  set (neTextureFrames, e.g. an animation loaded from an index blob). Both
//  release their AepTexture cache references on destruction. Reconstructed from
...
// A polymorphic single-texture handle. +0x04 holds the cached AepTexture,
// released on destroy; the remaining metadata fills out a 0x18-byte record
// (neTextureFrames stores these contiguously). Ghidra: dtor FUN_00015edc.
class neTextureRef {
public:
    virtual ~neTextureRef(); // Ghidra: FUN_00015edc (+ compiler-emitted deleting
                             // dtor FUN_00015f00)
    ...
    // the AepSprite draw path (drawAepSpriteClipped) on each frame's
    // neTextureRef.
...
// neTextureRef records. Each array is owned (RAII); the handles are
