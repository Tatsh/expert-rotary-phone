/**
 * @file
 * The OpenGL ES 1.1 rendering abstraction: the interface every on-screen sprite, texture,
...
/**
 * Abstract OpenGL ES render interface: the enum vocabulary and pure-virtual drawing API.
 *
...
public:
    /**
     * Framebuffer attachment points (three entries), consumed when validating that the
    ...
    /**
     * Renderbuffer storage formats. These pair one to one with RenderKind above and are the
    ...
    /**
     * glMatrixMode targets, with GL_MODELVIEW as the out-of-range default: mode minus one in
    ...
    /**
     * glHint targets (five entries), listed alphabetically by GL name.
     */
    enum Hint {
    ...
    /**
     * glFog(GL_FOG_MODE, ...) modes (three entries).
     */
    enum FogMode {
    ...
    /**
     * glEnableClientState and glDisableClientState array targets (eight entries): the
    ...
    /**
     * glEnable and glDisable capabilities (35 entries), listed alphabetically by GL name.
    ...
    /**
     * glCullFace values (three entries).
     */
    enum CullFace {
    ...
    /**
     * glFrontFace values. These are the only two legal glFrontFace arguments, so the wrapper
     * maps them inline.
     */
    enum FrontFace {
    ...
    /**
     * GL source blend factors (nine entries), validated against BLEND_SRC_VALUE_MAX.
     */
    enum BlendSrcValue {
    ...
    /**
     * GL destination blend factors (eight entries), validated against BLEND_DEST_VALUE_MAX.
     */
    enum BlendDestValue {
    ...
    /**
     * Texture upload formats. Exactly these two are recognised; anything else asserts.
     */
    enum TexFormat {
    ...
    /**
     * glTexParameter pname values (four entries).
     */
    enum TexParamType {
    ...
    /**
     * glTexParameter values (eight entries). getTexParameter provides the reverse mapping.
     */
    enum TexParamValue {
    ...
    /**
     * Depth and alpha compare functions (eight entries), sharing one table, GL_NEVER through
    ...
    /** Alias for the depth-test compare function; asserts below DEPTH_TEST_FUNC_MAX. */
    using DepthTestFunc = CompareFunc;
    /** Alias for the alpha-test compare function; asserts below ALPHA_TEST_FUNC_MAX. */
    using AlphaTestFunc = CompareFunc;

    /**
     * Engine primitive ordinals mapped to GL modes (seven entries), indexed by the mode the
    ...
    /**
     * Set the alpha-test comparison function and reference value.
     * @param func The comparison function to apply.
    ...
    /**
     * Upload a level-zero 2D texture image.
    ...
    /**
     * Set a texture parameter on the bound 2D texture.
    ...
    /**
     * Read a texture parameter back from the bound 2D texture.
    ...
    /**
     * Delete a GL buffer, invalidating any cached binding that referred to it.
     * @param buffer The name of the buffer to delete.
     */
    virtual void deleteBuffer(unsigned buffer) = 0;

    /**
     * Attach a renderbuffer, of the given storage type, at the given attachment point.
...
/**
 * OpenGL ES 1.1 backend implementing the neIGLES render API and the ne::C_RENDER drawing
 * slots.
 */
class neGLES_11 : public neIGLES {
public:
    /**
     * Construct the backend and prime the cached GL state block to its power-on defaults.
    ...
    /**
     * Un-hide the enum-typed setBlendFunc so the drawing-slot overload below can delegate to
     * it.
     */
    ...
    /**
     * Select the current matrix mode, caching it and only re-issuing glMatrixMode on change.
     * @param mode The matrix mode to make current.
    ...
    /**
     * Configure blending, caching the factors and equation and skipping redundant calls.
     * @param src The source blend factor.
     * @param dest The destination blend factor.
     * @param equation The GL blend equation passed to glBlendEquationOES.
    ...
    /**
     * Upload a level-zero 2D texture image (GL_TEXTURE_2D, GL_UNSIGNED_BYTE).
     * @param format The pixel format of the source data.
     * @param width The image width in pixels.
     * @param height The image height in pixels.
     * @param pixels The source pixel data.
    ...
    /**
     * Delete a GL buffer, first invalidating any cached binding that referred to it.
     * @param buffer The name of the buffer to delete.
    ...
    /**
     * Delete an OES framebuffer object.
    ...
    /**
     * Delete an OES renderbuffer object.
    ...
    /**
     * Attach a 2D texture's level zero at the given attachment point.
     * @param kind The attachment point to bind to.
    ...
    /**
     * Attach a renderbuffer at the given attachment point.
     * @param kind The attachment point to bind to.
     * @param renderbuffer The name of the renderbuffer to attach.
    ...
    /**
     * Return the GL_RENDERBUFFER_OES constant the view caches for -presentRenderbuffer:.
    ...
    /**
     * Generate one OES framebuffer name.
    ...
    /**
     * Generate one OES renderbuffer name.
    ...
    /**
     * Report whether the currently-bound OES framebuffer is complete.
    ...
    /**
     * Probe GL capabilities, load the texcoord-normalising texture matrix, and set the
    ...
    /**
     * Select the matrix mode and load the given matrix into it.
    ...
    /**
     * Specify the colour array (four unsigned bytes per vertex).
    ...
    /**
     * Specify the vertex-position array (GL_FLOAT components).
    ...
    /**
     * Specify the texture-coordinate array (two GL_SHORT components).
    ...
    /**
     * Upload data into the bound element buffer.
    ...
    /**
     * Delete a GL texture, first clearing every bound-texture-cache slot holding its name so
    ...
    /**
     * Bind a texture to the active unit through a per-unit redundant-bind cache, skipping
    ...
    /**
     * Apply a texture parameter on the bound 2D texture.
     * @param type The texture parameter to set.
     * @param value The value to assign to the parameter.
     */
    ...
    /**
     * Configure blending using the default add equation.
    ...
    /**
     * Configure blending with an explicit blend equation.
     * @param src The source blend factor ordinal.
     * @param dst The destination blend factor ordinal.
    ...
    /**
     * Enable or disable a client-side array. The drawing ordinal is one less than the
    ...
    /**
     * Draw indexed primitives from the bound element buffer.
     * @param mode The primitive mode ordinal.
    ...
    /**
     * The eight-slot bound-texture-name cache, indexed by the active texture unit.
    ...
    /** The bound-texture-name cache for this backend. */
