...
//  pop'n rhythmin
//
//  OpenGL ES 1.1 backend. Every enum-to-GL mapping table below is decoded
//  byte-for-byte from the binary's read-only data region; the wrapper bodies
//  follow the "validate against _MAX, index the table, call the matching glXxx"
//  pattern.
//
...
// The GL ES OES framebuffer attachment points.
...
// Renderbuffer storage format per RenderType (pairs 1:1 with RenderKind; the
// attach path is inlined).
...
// Index 0 is the GL_MODELVIEW out-of-range default; 1..3 index the table.
...
// glFog(GL_FOG_MODE) modes (3 entries).
...
// glEnableClientState array targets (8 entries).
...
// glEnable / glDisable capabilities (35 entries).
...
// Depth/alpha compare functions (8 entries).
...
// GL src blend factors (9 entries; setBlendFunc asserts src < BLEND_SRC_VALUE_MAX).
...
// GL dest blend factors (8 entries; setBlendFunc asserts dest < BLEND_DEST_VALUE_MAX).
...
// glTexParameter pname forward map.
...
// glTexParameter value forward map (8 entries).
...
// Reverse of glGetTexParameteriv: map a GL value back to our enum.
...
// Engine primitive ordinal -> GL mode (7 entries), indexed by
...
// Installs the vtable and primes the whole cached-GL-state block to the
// backend's power-on defaults.
//
// This reconstruction models only the subset of that block its method subset
// actually reads back: the buffer-bind caches (ivars +0x44/+0x50/+0x5c/+0x6c),
// the two 8-slot texture caches (_boundTextures +0xb4, texBindCache +0xfc), the
...
// explicit blend writes below: the buffer/texture/matrix caches are zero in the
// binary and the blend cache is add-equation/ONE/ZERO.
//
...
neGLES_11::neGLES_11()
    : _blendEquation(GL_FUNC_ADD_OES), // ivar +0x19c
...
// Caches the mode (ivar +0x2c) and only re-issues glMatrixMode on change.
...
// Caches (equation +0x19c, src +0x1a0, dest +0x1a4), skips redundant calls, then
// glBlendEquationOES(equation); glBlendFunc(src, dest).
...
// Upload a 2D texture (GL_TEXTURE_2D, level 0, UNSIGNED_BYTE).
...
// Read a texture parameter back as our enum.
...
// Invalidate any cached binding referring to this buffer (ivars
// +0x44/+0x50/+0x5c/+0x6c and the 8-slot texture array at +0xb4), then delete.
...
// Attach a 2D texture's level 0 at attachment point `kind`.
...
// Attach a renderbuffer at attachment point `kind`.
void neGLES_11::framebufferRenderbuffer(RenderKind kind, unsigned renderbuffer) {
    glFramebufferRenderbufferOES(
        GL_FRAMEBUFFER_OES, RenderKindToGL(kind), GL_RENDERBUFFER_OES, renderbuffer);
}

// Is the currently-bound OES framebuffer complete? (result discarded by the sole
// caller, neGLView -layoutSubviews).
bool neGLES_11::isFramebufferComplete() {
    return glCheckFramebufferStatusOES(GL_FRAMEBUFFER_OES) == GL_FRAMEBUFFER_COMPLETE_OES;
}

// Returns the GL_RENDERBUFFER_OES constant the view caches in its
// m_PresentTarget ivar and later hands to -presentRenderbuffer: (no GL call).
unsigned neGLES_11::presentTarget() const {
    return GL_RENDERBUFFER_OES;
}

// Thin glGenFramebuffersOES(1, &out) wrapper.
...
// Thin glGenRenderbuffersOES(1, &out) wrapper.
...
// Read the bound renderbuffer's pixel width.
...
// Read the bound renderbuffer's pixel height.
...
// The renderer's pInit virtual. Probes the GL extension string for
// GL_OES_matrix_palette, reads GL_MAX_TEXTURE_SIZE, loads the
...
void neGLES_11::initialize() {
    ...
    // binary copies each token with memcpy and a single nul terminator, with no
    // length clamp, then strncmp's it against the palette extension name.
    ...
    // whose diagonal is (1/32767, 1/32767, 1, 1) to scale them back to 0..1, then load
    // it under MATRIX_MODE_TEXTURE.
    neMatrix4 texScale = {};
...
void neGLES_11::vertexPointer(const void *ptr, int size, int stride) {
    // The type is GL_FLOAT: the caching variant tail-calls glVertexPointer(size,
    // 0x1406, stride, ptr), and 0x1406 is GL_FLOAT. That variant additionally
    // skips the call when the last (ptr, size, stride) triple is unchanged and no
    // array buffer is bound; it is uncalled in the binary, so only the plain
    // specification is reproduced here.
...
// Thin glGenTextures wrapper. The renderer `this` is discarded; the caller's
// &outName is passed straight through, so this is exactly glGenTextures(1, &outName).
...
// Clear every bound-texture-cache slot holding this name (so a later reused name
// is not treated as still bound), then glDeleteTextures. This is the slot the
// ne::C_TEXTURE teardown path dispatches through (deleteTexture), which is why the
// raw glDeleteTextures would leave a stale cache entry.
...
// Per-unit redundant-bind cache: index texBindCache by the active texture unit
// (ivar +0xf8), skip when the cached name already matches, else update the slot
// and glBindTexture. Keeping the cache coherent is why deleteTexture must clear
// it on a delete.
...
// The drawing-slot client-array selector. Unlike setEnable, which indexes the
// full EnableState table directly, this slot indexes a client-array enum table
// starting ONE entry past MATRIX_PALETTE (the ClientState table plus one). So the
...
// The primitive-draw slot. Its body is in the binary's undisassembled vtable-only
// region, but its behaviour is pinned by its sole callers neDrawColorArray and
...
// Indexed-draw slot. No caller exists in the reconstruction (and the index-type
// constant GL_UNSIGNED_SHORT / 0x1403 does not appear anywhere in the binary), so
// this path is never exercised; it is reconstructed as the standard ES 1.1 indexed
