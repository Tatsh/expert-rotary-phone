...
//  pop'n rhythmin
//
//  See FriendScoreMainView.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin:
...
//
//  Honesty notes:
//   - The three tab tables (N/H/Ex) are plain UITableViewControllers whose
//   delegate AND data
//     source are this main view; -tableView:...: dispatches on which
//     controller's tableView is asking. On phone they are the pages of a
//     UITabBarController; on pad they sit side by side.
//   - initAtNavigationControllerWithMusicId: is a very large layout routine.
//   Every UIKit call,
//     asset name, device branch and animation is reproduced; the exact
//     sub-pixel frame offsets were partly inlined by the decompiler as raw
//     float vector ops, so a few frame origins are approximated from the
//     recovered constants and flagged inline. Behaviour (which views are
//     created, wired and added) is exact.
//   - downloaderFinished: parses the server JSON, builds five parallel friend
//   arrays
//     (playerId / scoreN / scoreH / scoreEx / flag), re-orders them to match
//     DownloadMain's friend list, appends the local player's own row, then
//     emits three NSValue-wrapped ScoreDataStruct display arrays (one per
//     difficulty) with finishing place, score rank, full-combo / perfect flags
//     and the "a rival beat you" notice flag (from OverScoreData). The Flag
//     bitfield is: bit0/1/2 = full-combo N/H/Ex, bit3/4/5 = perfect N/H/Ex.
//   - For the local player's own row the binary additionally reconciles the
//   server "Me" scores
//     against the app-event-center in-memory score store (Ghidra free helpers
//     fetchScoreDataForMusic / updateHighScore / saveScoreData on the
//     g_pNeAppEventCenter singleton, plus scoreToRank) and persists any new
//     best back into the local ScoreData store, for each of the three
//     difficulties. That write-back is reconstructed here: the singleton that
//     the store helpers take (&g_pNeAppEventCenter) is the shared PlayScore
//     struct, so its musicId / difficulty (g_wResultSheet) / rank
//     (g_wResultClearRank) fields are modelled with a local PlayScore. The
//     server per-difficulty medal bits drive updateHighScore's GOOD/BAD/
//     full-combo tallies exactly as the binary derives them (see the
//     downloaderFinished body).
//   - Under ARC the ScoreDataStruct NSString field is __unsafe_unretained
//   (mirroring
//     FriendListData in DownloadMain.h). -dealloc is kept: it cancels the
//     in-flight Downloader and detaches this controller from DownloadMain's
//     friend-list delegate.
//
...
- (UINavigationController *)initAtNavigationControllerWithMusicId:(unsigned int)musicId
    ...
    UIImageView *bgView =
        [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, bgImg.size.width, bgImg.size.height)];
    [bgView setImage:bgImg];
    [self.view addSubview:bgView];

    // Wrap self in a navigation controller with a custom back button + nav-bar
    // art.
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (!isPad) {
            UIImageView *bg =
                [[UIImageView alloc] initWithImage:[UIImage imageNamed:tableBgName[i]]];
    ...
    if (isPad) {
        ...
        // Which difficulties already have a rival ("over-score") record for this
        // song — the matching tab shows a blinking warning badge.
        ...
        // Tab-bar frame + per-item art. iOS 7 flattens the bar and needs template
        // images.
        ...
        UIEdgeInsets imgInsets =
            isOS7 ? UIEdgeInsetsMake(-1.0f, 0, 2.0f, 0) : UIEdgeInsetsMake(-1.0f, 0, 6.0f, 0);
        NSString *const onName[3] = {@"frisco_tab_no_on", @"frisco_tab_hy_on", @"frisco_tab_ex_on"};
        NSString *const offName[3] = {
            @"frisco_tab_no_off", @"frisco_tab_hy_off", @"frisco_tab_ex_off"};
        for (int i = 0; i < 3; i++) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            // Disasm 0xab554/0xab698/0xab7ea: options = #0x18 = Repeat | Autoreverse
            // (the reconstruction dropped the Autoreverse bit, leaving a hard alpha
            // snap-back).
            [UIView
    ...
    UIImageView *artwork =
        [[UIImageView alloc] initWithImage:[UIImage imageWithData:[md artwork2xData]]];
    {
    ...
    UIImageView *titleView =
        [[UIImageView alloc] initWithImage:[UIImage imageWithData:[md musicNameImage2xData]]];
    {
...
// @ 0xabdd8 — cancel the in-flight request and detach from DownloadMain's
...
- (NSArray *)arrayForTableView:(UITableView *)tableView {
    if (tableView == _tblViewCtrlN.tableView) {
        return _frScoreNArray;
    }
    if (tableView == _tblViewCtrlH.tableView) {
        return _frScoreHArray;
    }
    if (tableView == _tblViewCtrlEx.tableView) {
        return _frScoreExArray;
    }
    return nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (tableView == _tblViewCtrlH.tableView) {
        page = 1;
        rows = _frScoreHArray;
    } else if (tableView == _tblViewCtrlEx.tableView) {
        page = 2;
    ...
    if ((NSUInteger)indexPath.row < rows.count) {
        ...
        // higher plus how many tied ones precede it.
...
- (void)downloaderFinished:(Downloader *)downloader {
    ...
    if ([json objectForKey:@"ErrorCode"] != nil) {
        ...
        // Re-order the server rows to match DownloadMain's friend list (server
        // order may differ); friends missing from the response get a -1 placeholder
        // score row.
        ...
        for (NSUInteger i = 0; i < friendList.count; i++) {
            ...
            NSUInteger found = i;
            BOOL matched =
                (i < playerIds.count) && [wantId isEqualToString:[playerIds objectAtIndex:i]];
        ...
        // The local player's own best per difficulty. The binary first reads it
        // from the local score store (Ghidra: the fetchScoreDataForMusic N/H/EX
        // loop at the head of downloaderFinished @ 0xac7f0), then reconciles it
        // with the server "Me" record, showing the better of the two.
        int myScore[3] = {0, 0, 0};
        ...
        // Reconcile the server "Me" record with the local store and, per
        // difficulty, persist any new server best BACK into the local ScoreData
        // store (updateHighScore + saveScoreData).
        //
        // In the binary the save path overlays the shared PlayScore struct on the
        // app-event-center singleton (&g_pNeAppEventCenter) and drives the free
        // store helpers on it; the three "result-screen" globals it pokes are just
        ...
        // We model that faithfully with a local PlayScore (neEngineBridge.h)
        // carrying those exact fields; the binary's extra
        // neAppEventCenter::shared() force-init before the overlay is a singleton
        // side effect the local model does not need.
        if (me != nil) {
            ...
            if (mN && mH && mEx && mFlg) {
                ...
                for (int d = 0; d < 3; d++) {
                    ...
                    // Server medal bits for this difficulty (shared Flag layout, bit0/1/2
                    // = the full-combo group, bit3/4/5 = the perfect group):
                    int fcMedal = (flag >> d) & 1;       // bit d      -> myFullCombo[d]
                    int pfMedal = (flag >> (d + 3)) & 1; // bit (d+3)  -> myPerfect[d]

                    // Persist when the server beat the local score, OR when the server
                    // holds a medal the local record lacks (the "even if not higher, save
                    // the new medal" branch).
                    BOOL writeBack = (myScore[d] < srvScore) || (!myFullCombo[d] && fcMedal) ||
                                     (!myPerfect[d] && pfMedal);
                    if (!writeBack) {
                        continue;
                    }

                    // updateHighScore(ps, srvScore, cool=1, great=2, good, bad,
                    // fullCombo):
                    //   fullCombo = fcMedal                       (the +0x04 full-combo
                    //   bit) good      = pfMedal ? 0 : 3               (uVar33 in the
                    //   decompile) bad       = fcMedal ? 0 : ((pfMedal<<2)^4)(uVar38: 0
                    //   if a cool/FC medal, else
                    //                                              0 when the perfect bit
                    //                                              is set, 4 otherwise)
                    // The cool/great bases (1, 2) are the constants the binary passes
                    // verbatim.
        ...
        // The rival records that beat someone for this song (for the "notice"
        // marker).
        NSArray *allOver =
            [OverScoreData getAllOverScoreData:[[AppDelegate appDelegate] managedObjectContext]];
        ...
        for (int d = 0; d < 3; d++) {
            ...
            for (NSUInteger i = 0; i < playerIds.count; i++) {
                ...
                for (NSUInteger j = 0; j < playerIds.count; j++) {
                    if ([[scores[d] objectAtIndex:i] intValue] < [[scores[d]
                ...
                // Friend rows carry the friend's display name; the local player's row
                // (the last one) leaves it nil so the cell fills in the "you" band from
                // UserSettingData.
                NSString *name = nil;
