...
//  transfer. The player types the player-id and the 6-digit convert pass issued on
//  their old device into two UITextFields, taps the decide button, and the screen
//  POSTs {uuid, player_id, convert_code} to the convert endpoint. On success the
//  full server-side save (player name/id, chara, tickets, treasure points, invite /
//  login-bonus state, per-music scores, treasure map progress, purchased chara
//  tickets) is restored into UserSettingData + the Core Data stores, the collabo /
//  invite / login-bonus / treasure music is re-opened, and a "done" alert is shown.
//  (Contrast ConversionView, the ISSUING side that uploads the local save and shows
//  the freshly-minted pass.) It is raised over the main menu inside its own
//  UINavigationController (phone; see -initAtNavigationController) or bare (pad, with
//  a tap-to-dismiss cover view), and fades itself in and out. Shown by
//  MainViewController -GotoInConversionPass; on close it calls back
//  -[MainViewController InConversionPassEndCallBack].
//
//  Reconstructed from Ghidra project rb420, program PopnRhythmin:
//    init                                                    @ 0x911d0
//    initAtNavigationController                              @ 0x91e84
//    dealloc                                                 @ 0x92064  (releases _downloader only -> ARC-omitted)
//    onBackBtn                                               @ 0x920b4
//    startOpenAnimation                                      @ 0x920e8
//    endOpenAnimation                                        @ 0x92220
//    startCloseAnimation                                     @ 0x92238
//    endCloseAnimation                                       @ 0x92368
//    didReceiveMemoryWarning                                 @ 0x9240c  (super-only)
//    viewDidLoad                                             @ 0x92438  (super-only)
//    viewDidUnload                                           @ 0x92464  (super-only)
//    viewWillAppear:                                         @ 0x92490  (super-only)
//    viewDidAppear:                                          @ 0x924bc  (super-only)
//    viewWillDisappear:                                      @ 0x924e8  (super-only)
//    viewDidDisappear:                                       @ 0x92514  (super-only)
...
//  Built in InputConversionPassViewController.mm (Objective-C++: the SE / scene-root
//  bridge drives the C++ neEngine / neSceneManager singletons). ARC.
//
...
@interface InputConversionPassViewController : UIViewController
