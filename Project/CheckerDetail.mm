...
//
//  The screen plots three scores per arcade sheet on a vertical "0..100%" graph:
//    * top  — the venue top score  (icon/plate/digit set "..._top"/"scobase_?top")
//    * ave  — the venue mean score ("..._ave"/"scobase_?ave")
//    * you  — the player's best    ("..._you"/"scobase_?you")
//  Sheets are indexed 0..3 = EX / Hyper / Normal / Easy (matching the image name
//  suffixes e / h / n / easy). A score of -1 means "no record" and that sheet's
//  column is skipped. Score values run 0..100000 (percent x 1000).
//
...
// Score-line images behind each sheet column (0/off = dim, 1/on = lit). The _960
// variants are the tall (960px) phone layout; the plain set is the iPad/short one.
static NSString *const kBaselineOff[4]    = { @"ppc_ps_baseline_e0",  @"ppc_ps_baseline_h0",
                                              @"ppc_ps_baseline_n0",  @"ppc_ps_baseline_easy0" };
static NSString *const kBaselineOn[4]     = { @"ppc_ps_baseline_e1",  @"ppc_ps_baseline_h1",
                                              @"ppc_ps_baseline_n1",  @"ppc_ps_baseline_easy1" };
static NSString *const kBaselineOff960[4] = { @"ppc_ps_baseline_e0_960",  @"ppc_ps_baseline_h0_960",
                                              @"ppc_ps_baseline_n0_960",  @"ppc_ps_baseline_easy0_960" };
static NSString *const kBaselineOn960[4]  = { @"ppc_ps_baseline_e1_960",  @"ppc_ps_baseline_h1_960",
                                              @"ppc_ps_baseline_n1_960",  @"ppc_ps_baseline_easy1_960" };

// Plotted marker icons (off = dim when the sheet is not selected, on = lit).
static NSString *const kTopIconOff[4]  = { @"ppc_ps_i_e_topoff",  @"ppc_ps_i_h_topoff",
                                           @"ppc_ps_i_n_topoff",  @"ppc_ps_i_easy_topoff" };
static NSString *const kTopIconOn[4]   = { @"ppc_ps_i_e_top",  @"ppc_ps_i_h_top",
                                           @"ppc_ps_i_n_top",  @"ppc_ps_i_easy_top" };
static NSString *const kAveIconOff[4]  = { @"ppc_ps_i_e_aveoff",  @"ppc_ps_i_h_aveoff",
                                           @"ppc_ps_i_n_aveoff",  @"ppc_ps_i_easy_aveoff" };
static NSString *const kAveIconOn[4]   = { @"ppc_ps_i_e_ave",  @"ppc_ps_i_h_ave",
                                           @"ppc_ps_i_n_ave",  @"ppc_ps_i_easy_ave" };
static NSString *const kYouIconOff[4]  = { @"ppc_ps_i_e_youoff",  @"ppc_ps_i_h_youoff",
                                           @"ppc_ps_i_n_youoff",  @"ppc_ps_i_easy_youoff" };
static NSString *const kYouIconOn[4]   = { @"ppc_ps_i_e_you",  @"ppc_ps_i_h_you",
                                           @"ppc_ps_i_n_you",  @"ppc_ps_i_easy_you" };

// Score-plate (number background) images per sheet.
static NSString *const kScoBaseTop[4]  = { @"ppc_ps_scobase_etop",  @"ppc_ps_scobase_htop",
                                           @"ppc_ps_scobase_ntop",  @"ppc_ps_scobase_easytop" };
static NSString *const kScoBaseAve[4]  = { @"ppc_ps_scobase_eave",  @"ppc_ps_scobase_have",
                                           @"ppc_ps_scobase_nave",  @"ppc_ps_scobase_easyave" };
static NSString *const kScoBaseYou[4]  = { @"ppc_ps_scobase_eyou",  @"ppc_ps_scobase_hyou",
                                           @"ppc_ps_scobase_nyou",  @"ppc_ps_scobase_easyyou" };

// Column tag bases: sheet-select buttons are 200+sheet, name-mode plates 204+sheet.
...
// @ 0xd7418 — redraw `image` through a device-gray bitmap context to grayscale it.
- (UIImage *)convertGrayScaleImage:(UIImage *)image {
    ...
    if (image == nil) {
        ...
        context = CGBitmapContextCreate(NULL, (size_t)image.size.width, (size_t)image.size.height,
                                        8, 0, colorSpace, (CGBitmapInfo)0);
    }
...
- (instancetype)initWithScoreData:(ArcadeScoreData *)scoreData {
    ...
    // Layout nudges applied only on iPad (0 on phone), and differing by iOS version.
    CGFloat headerYOffset = 0.0f;
    CGFloat contentYOffset = 0.0f;
    if (isPad) {
        if (UIDevice.currentDevice.systemVersion.floatValue >= 7.0f) {
            headerYOffset = 54.0f;   contentYOffset = 94.0f;
        } else {
            headerYOffset = 10.0f;   contentYOffset = 50.0f;
        }
    }
    ...
    UIImageView *header = [[UIImageView alloc] initWithFrame:CGRectMake(5.0f, headerY,
                                                                        headerImg.size.width,
                                                                        headerImg.size.height)];
    ...
    titleLabel.textColor = [UIColor colorWithRed:0.341176f green:0.317647f blue:0.298039f alpha:1.0f];
    ...
    genreLabel.textColor = [UIColor colorWithRed:0.960784f green:0.960784f blue:0.960784f alpha:1.0f];
    ...
    UIImageView *graphBase = [[UIImageView alloc] initWithFrame:CGRectMake(10.0f,
    ...
    // Score-line strips per column; lit strip on the selected sheet, dim on the rest.
    CGFloat lineX = 45.0f;
    for (int i = 0; i < 4; i++) {
        if (topScores[i] >= 0) {
            if (_selectedSheet == -1) {
            ...
            _scoreLineOff[i] = [[UIImageView alloc] initWithFrame:CGRectMake(lineX, 6.0f,
                                                                             offImg.size.width,
                                                                             offImg.size.height)];
            [_scoreLineOff[i] setImage:offImg];
            _scoreLineOn[i] = [[UIImageView alloc] initWithFrame:CGRectMake(lineX, 6.0f,
                                                                            offImg.size.width,
                                                                            offImg.size.height)];
    ...
    // Plotted marker icons: top / ave / you, each at its score height in the column.
    CGFloat iconX = 52.0f;
    for (int i = 0; i < 4; i++) {
        if (topScores[i] >= 0) {
            ...
            _topIconOff[i] = [[UIImageView alloc] initWithFrame:CGRectMake(iconX, topIconY,
                                                                           topOffImg.size.width,
                                                                           topOffImg.size.height)];
            [_topIconOff[i] setImage:topOffImg];
            _topIconOn[i] = [[UIImageView alloc] initWithFrame:CGRectMake(iconX, topIconY,
                                                                          topOffImg.size.width,
                                                                          topOffImg.size.height)];
            ...
            _meanIconOff[i] = [[UIImageView alloc] initWithFrame:CGRectMake(iconX, aveIconY,
                                                                            aveOffImg.size.width,
                                                                            aveOffImg.size.height)];
            [_meanIconOff[i] setImage:aveOffImg];
            _meanIconOn[i] = [[UIImageView alloc] initWithFrame:CGRectMake(iconX, aveIconY,
                                                                           aveOffImg.size.width,
                                                                           aveOffImg.size.height)];
            ...
            _myIconOff[i] = [[UIImageView alloc] initWithFrame:CGRectMake(iconX, youIconY,
                                                                          youOffImg.size.width,
                                                                          youOffImg.size.height)];
            [_myIconOff[i] setImage:youOffImg];
            _myIconOn[i] = [[UIImageView alloc] initWithFrame:CGRectMake(iconX, youIconY,
                                                                         youOffImg.size.width,
                                                                         youOffImg.size.height)];
            ...
            BOOL notSelected = (i != _selectedSheet);
            if (notSelected) {
                [_topIconOff[i] setHidden:NO];  [_topIconOn[i] setHidden:YES];
                [_meanIconOff[i] setHidden:NO]; [_meanIconOn[i] setHidden:YES];
                [_myIconOff[i] setHidden:NO];   [_myIconOn[i] setHidden:YES];
            } else {
                [_topIconOff[i] setHidden:YES]; [_topIconOn[i] setHidden:NO];
                [_meanIconOff[i] setHidden:YES];[_meanIconOn[i] setHidden:NO];
                [_myIconOff[i] setHidden:YES];  [_myIconOn[i] setHidden:NO];
            }
    ...
    // Score plates (number backgrounds) per column. The three plates in a column are
    // driven by their score heights but nudged apart by a plate-height so they never
    // overlap, and clamped between Y=55 and the graph baseline. The precise fixed-point
    // clamp chain is transcribed from the binary (init @ 0xd752c) and reproduced here.
    ...
    for (int i = 0; i < 4; i++) {
        if (topScores[i] >= 0) {
            ...
            if (topRawY < 55.0f) topRawY = 55.0f;
            if (aveRawY < 55.0f) aveRawY = 55.0f;
            if (youRawY < 55.0f) youRawY = 55.0f;
            ...
            // sits under it, each at least a plate-height apart and above the baseline.
            ...
            _topScoreBase[i] = [[UIImageView alloc] initWithFrame:CGRectMake(colX, topPlateY,
            ...
            // Transparent overlay on the top plate that carries the holder-name label.
            _topNameBase[i] = [[UIImageView alloc] initWithFrame:CGRectMake(colX, topPlateY,
                                                                            topPlateImg.size.width,
                                                                            topPlateImg.size.height)];
            ...
            _meanBase[i] = [[UIImageView alloc] initWithFrame:CGRectMake(colX, avePlateY,
                                                                         avePlateImg.size.width,
                                                                         avePlateImg.size.height)];
            [_meanBase[i] setImage:avePlateImg];

            _myBase[i] = [[UIImageView alloc] initWithFrame:CGRectMake(colX, youPlateY,
            ...
            BOOL notSelected = (i != _selectedSheet);
            if (notSelected) {
                [_topScoreBase[i] setHidden:YES]; [_topNameBase[i] setHidden:YES];
                [_meanBase[i] setHidden:YES];     [_myBase[i] setHidden:YES];
            } else {
                [_topScoreBase[i] setHidden:NO];  [_topNameBase[i] setHidden:YES];
                [_meanBase[i] setHidden:NO];      [_myBase[i] setHidden:NO];
            }
    ...
    for (int i = 0; i < 4; i++) {
        if (topScores[i] >= 0) {
            ...
            [sheetButton addTarget:self action:@selector(touchedSheetButton:)
    ...
    for (int i = 0; i < 4; i++) {
        if (topScores[i] >= 0) {
            ...
            // label is (re)built onto _topNameBase alongside each digit, as in the binary.
            int topValue = topScores[i];
            int topDigits = 1;
            for (int t = topValue; t > 9; t /= 10) topDigits++;
            {
                CGFloat digitX = 60.0f;
                int value = topValue;
                for (int d = 0; d < topDigits; d++) {
                    UIImage *digitImg =
                        [UIImage imageNamed:[NSString stringWithFormat:@"ppc_ps_num_top_%d", value % 10]];
                    UIImageView *digitView = [[UIImageView alloc] initWithImage:digitImg];
                    [digitView setFrame:CGRectMake(digitX, 25.0f,
                                                   digitImg.size.width, digitImg.size.height)];
                    ...
                    switch (i) {
                        case 0:
                            nameLabel.textColor = [UIColor colorWithRed:0.960784f green:0.960784f
                                                                   blue:0.960784f alpha:1.0f];
                            nameLabel.text = scoreData.topNameEx;
                            break;
                        case 1:
                            nameLabel.textColor = [UIColor colorWithRed:0.341176f green:0.317647f
                                                                   blue:0.298039f alpha:1.0f];
            ...
            int aveDigits = 1;
            for (int t = aveValue; t > 9; t /= 10) aveDigits++;
            {
                CGFloat digitX = 60.0f;
                int value = aveValue;
                for (int d = 0; d < aveDigits; d++) {
                    UIImage *digitImg =
                        [UIImage imageNamed:[NSString stringWithFormat:@"ppc_ps_num_ave_%d", value % 10]];
                    UIImageView *digitView = [[UIImageView alloc] initWithImage:digitImg];
                    [digitView setFrame:CGRectMake(digitX, 25.0f,
                                                   digitImg.size.width, digitImg.size.height)];
            ...
            int youDigits = 1;
            for (int t = youValue; t > 9; t /= 10) youDigits++;
            {
                CGFloat digitX = 60.0f;
                int value = youValue;
                for (int d = 0; d < youDigits; d++) {
                    UIImage *digitImg =
                        [UIImage imageNamed:[NSString stringWithFormat:@"ppc_ps_num_you_%d", value % 10]];
                    UIImageView *digitView = [[UIImageView alloc] initWithImage:digitImg];
                    [digitView setFrame:CGRectMake(digitX, 25.0f,
                                                   digitImg.size.width, digitImg.size.height)];
...
// @ 0xd97c4 — switch the active sheet (button tag 200..203), lighting its column
// and dimming the previously selected one.
- (void)touchedSheetButton:(id)sender {
