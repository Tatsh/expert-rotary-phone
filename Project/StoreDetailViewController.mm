...
//  pop'n rhythmin
//
//  See StoreDetailViewController.h. Reconstructed from Ghidra project rb420,
//  program PopnRhythmin. -init and -loadView are byte/disasm-reconstructed; the
//  table data-source / delegate rows, sample playback and StoreDetailHeaderView
//  build are a separate piece (see HANDOFF.md), stubbed here so the class is
//  well-formed.
//
...
- (id)init {
    ...
    if (self != nil) {
        ...
        UIButton *backBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, sz.width, sz.height)];
...
- (void)loadView {
    ...
    [buy addTarget:self
                  action:@selector(onPurchaseButton:)
        forControlEvents:UIControlEventTouchUpInside];
    ...
    packBgImage0 = [[UIImage imageNamed:@"store_pack_bg_0"] stretchableImageWithLeftCapWidth:4
                                                                                topCapHeight:4];
    packBgImage1 = [[UIImage imageNamed:@"store_pack_bg_1"] stretchableImageWithLeftCapWidth:4
                                                                                topCapHeight:4];

    artworkDownloaders = [[NSMutableDictionary alloc] initWithCapacity:32];

    // --- dummy cover (transparent, hidden) with a 2x spinner, shown during
    // purchase work ---
...
// @ 0x72970 — release every owned ivar (the table / labels / spinners are
// hierarchy-owned and are not released here). Cancels the in-flight detail +
// sample downloads first. @ 0x72970 additionally releases packBgImage0,
...
// @ 0x706a4 — the buy button decision tree (phone parallel of
// StorePackDetailViewPad -doPurchase:). Owned: re-download the missing songs,
...
- (void)loadInfo {
    ...
    m_StorePackInfoDownloader = [[StorePackInfoDownloader alloc] initWithStorePackInfo:packInfo];
    [m_StorePackInfoDownloader setDelegate:self];
    // @ 0x70538 passes r2 = 1 (YES): downloadDetail:YES, not NO.
    [m_StorePackInfoDownloader downloadDetail:YES];
}
...
- (void)showPackInfo {
    ...
    // Kick off the pack jacket download (keyed by an index path, row 0 / section
    // 1).
...
// @ 0x71458 — a Downloader finished. Two flows: (a) the sample clip -> load it
// as looping BGM, play it, mark the row playing; (b) the recommend-pack POST
...
// @ 0x72860 — support only the portrait orientations (legacy rotation hook).
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if ((NSUInteger)row >= songCount) {
        ...
        if (cell == nil) {
            cell =
                [[StoreDetailCopyrightCell alloc] initWithStyle:UITableViewCellStyleDefault
    ...
    // --- song row ---
    [tableView
        dequeueReusableCellWithIdentifier:@"StoreDetailTableMusicCell"]; // (always builds fresh)
    StoreDetailMusicCell *cell =
    ...
    if (music != nil) {
        ...
        NSString *hard =
            (music.lvHard == 11) ? @"10+" : [NSString stringWithFormat:@"%d", music.lvHard];
        // @ 0x105aab: the format literal is "LEVEL:  %d / %d / %@" (colon, two
        // spaces, and spaces around the slashes), not "LEVEL %d/%d/%@".
        cell.labelLevels.text = [NSString
            stringWithFormat:@"LEVEL:  %d / %d / %@", music.lvBasic, music.lvMedium, hard];
        [cell setLink:music.iTunesURL]; // Ghidra selector itunesURL

        // Jacket: cached/in-flight downloader if present, else start one;
...
// @ 0x722a0 — tap a song row to toggle its preview clip: re-tapping the
