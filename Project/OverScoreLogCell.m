...
// Each row of OverScoreLogViewController's _overScoreLogDataArray is an NSValue
// boxing the OverScoreLogData struct below (DownloadMain builds them with
// +[NSValue value:withObjCType:], the same pattern it uses for the
// friend/download/information/recommend lists); the cell copies the row out
// with -[NSValue getValue:].

// Plain C struct the data element writes into (Ghidra ivar m_overScoreLogData,
// filled at param_1 + m_overScoreLogData). Only the fields this cell reads are
// named; pointers are
...
// .cxx_construct @ 0x6a29c — compiler-emitted C++ ivar constructor; not
// hand-written.

// @ 0x69804 — pull this row's values out of the data element, tear down any
// recycled subviews, then rebuild the banner background + five labels +
...
- (void)setOverScoreLogData:(NSValue *)overScoreLogData {
    ...
    if (m_lblMusicName) {
        [m_lblMusicName removeFromSuperview];
        m_lblMusicName = nil;
    }
    if (m_lblFriendName) {
        [m_lblFriendName removeFromSuperview];
        m_lblFriendName = nil;
    }
    if (m_lblUpdateDate) {
        [m_lblUpdateDate removeFromSuperview];
        m_lblUpdateDate = nil;
    }
    if (m_lblMyScore) {
        [m_lblMyScore removeFromSuperview];
        m_lblMyScore = nil;
    }
    if (m_lblFriendScore) {
        [m_lblFriendScore removeFromSuperview];
        m_lblFriendScore = nil;
    }
    if (m_imgViewSheet) {
        [m_imgViewSheet removeFromSuperview];
        m_imgViewSheet = nil;
    }
    ...
    m_lblMusicName =
        [[UILabel alloc] initWithFrame:CGRectMake(28.0f, 23.0f, legacy ? 160.0f : 172.0f, 18.0f)];
    ...
    if (m_overScoreLogData.sheetType == 2) {
        sheetImageName = @"acv_custom_ex";
    } else if (m_overScoreLogData.sheetType == 1) {
        sheetImageName = @"acv_custom_heper"; // sic (binary spelling)
    } else if (m_overScoreLogData.sheetType == 0) {
        sheetImageName = @"acv_custom_normal";
    }
    ...
    [m_imgViewSheet
        setFrame:CGRectMake(
                     legacy ? 190.0f : 207.0f, 24.0f, sheetImg.size.width, sheetImg.size.height)];
    ...
    m_lblFriendName.textColor = [UIColor colorWithRed:0.34902f
                                                green:0.31765f
                                                 blue:0.30980f
                                                alpha:1.0f];
    m_lblFriendName.highlightedTextColor = [UIColor whiteColor];
    m_lblFriendName.font = [UIFont fontWithName:@"BullyBold"
                                           size:11.0f]; // getFontNameBullyBold() @ 0x5ef90
    ...
    m_lblFriendScore = [[UILabel alloc]
        initWithFrame:CGRectMake(28.0f, legacy ? 60.0f : 61.0f, legacy ? 75.0f : 85.0f, 18.0f)];
    ...
    m_lblUpdateDate.textColor = [UIColor colorWithRed:0.34902f
                                                green:0.31765f
                                                 blue:0.30980f
                                                alpha:1.0f];
