...
// @ 0x8c7f0 — build the view tree: an iPad-only engraved header label, the manage table
// view (styled per device), then scan the purchased-music list for songs not yet present
// in the purchased-arcade list and, if any, kick off the integrity check. (setAutoresizingSize
// is a UIView category helper in the app; inlined here as the equivalent flexible mask.)
- (void)loadView {
    [super loadView];
    ...
    if (neSceneManager::isPadDisplay()) {
        ...
        title.textColor = [UIColor colorWithRed:0.188f green:0.188f blue:0.188f alpha:1.0f]; // 0x3e40c0c1
        ...
        frame = CGRectMake(frame.origin.x + 20.0f, frame.origin.y + 50.0f,
                           frame.size.width - 40.0f, frame.size.height - 115.0f);
    }
    ...
    // Collect the ids of owned (non-arcade) songs that are NOT present in the purchased
    // arcade list — those need their arcade-viewer info fetched (see -startCheck).
...
// @ 0x8cf28 — dequeue/build a manage row: a bottom-detail cell with an engraved title/genre
// (drawn by a pair of shadow labels over the blanked built-in labels) and a right-aligned
// delete / download action button. The button image (and, on iPad, its title) reflects
// whether the arcade file is already on disk.
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (cell == nil) {
        ...
        // Built-in title label: narrower than the cell (leaves room for the button).
        cell.textLabel.frame = CGRectMake(cell.textLabel.frame.origin.x, cell.textLabel.frame.origin.y,
                                          cell.frame.size.width - 200.0f, cell.textLabel.frame.size.height);
        ...
        // Title shadow label (tag 0xe020) — engraved gray text with a white highlight.
    ...
    [button sizeToFit];
    // Right-align and vertically centre the button (Ghidra: cell.width - buttonWidth - 10,
    // (cell.height - buttonHeight) * 0.5). buttonHeight = 40 iPad / 36 phone.
    ...
    // Populate the built-in labels, mirror their text into the shadow labels, then blank
    // the built-ins so only the engraved shadow labels are visible.
...
// @ 0x8d8f8 — per-device row background: phone alternates two grays, iPad uses a stretchable
// pack-background image over a clear cell.
...
// @ 0x8da48 — the per-row action button was tapped: walk up to the owning cell, resolve the
// row's arcade song, and either (file missing) re-download it via the store modal dialog, or
// (file present) confirm deletion. Ignored while another action or a check pass is running.
...
// @ 0x8de20 — start the arcade file download for the working row via StoreDownloadManager.
...
// @ 0x8df94 — fetch the arcade-viewer info for the first pending id (see -loadView).
- (void)startCheck {
...
// @ 0x8e03c — a Downloader finished. If it is the info downloader, register the fetched
// arcade song, then advance: process the next pending id, or (when the check list is drained)
// download the working row's file / reload the table.
...
// @ 0x8e1a4 — the info download failed. Abandon the whole check batch (reload) if one was
// running; otherwise fall through to the file download for the working row.
...
// @ 0x8e250 — the store modal dialog's abort button: cancel any in-flight info/file download,
// hide the dialog, and clear the working row.
...
// @ 0x8e2f8 — delete-confirm alert result: on "はい" (index 1) delete the working row's arcade
// file and reload; either way clear the working row.
...
// @ 0x8e3e4 — file download finished: reload, hide the dialog, clear the working row.
...
// @ 0x8e45c — file download failed: report it, hide the dialog, clear the working row.
...
// @ 0x8e610 — the view was torn down: release the table (the ARC-managed ivar is niled after
// super).
- (void)viewDidUnload {
    [super viewDidUnload];
    if (m_TableView != nil) {
...
// @ 0x8e748 — tear-down: drop the alert delegate, detach the table's data source/delegate,
// cancel the active file download and the in-flight info download. KEPT under ARC because it
// cancels downloads and detaches delegates (not object-only). Object releases and [super
// dealloc] are ARC-omitted.
- (void)dealloc {
    [m_DeleteAlertView setDelegate:nil];
