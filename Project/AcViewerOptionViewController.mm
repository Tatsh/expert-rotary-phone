...
//  viewer's per-song option list. Objective-C++ (.mm) because it drives the C++ "ne"
//  engine singletons via neEngineBridge (scene manager, event-center AC-viewer
//  selection, and the AcMainTask exit / apply-settings hooks).
//
...
// custom header (song banner, difficulty banner, title/genre and BPM labels); and, off
// the AC-main flow, a back button, the PLAY / CONTINUE buttons and the "friman" backdrop.
- (instancetype)init {
    ...
    // Back button (skipped on the AC-main flow, which installs its own in initForAcMain:).
    if (!_forAcMain) {
    ...
    // Header container: the banner plus a 17 pt gap above and below it. (The binary
    // derives the height from the banner/difficulty image sizes via vector adds; the
    // banner-height + 34 form matches the recovered layout.)
    ...
    if (!_forAcMain) {
        ...
        if (!neSceneManager::isPadDisplay()
            || musicId != neAppEventCenter::acViewerSelMusicId()
            ...
            [playBtn addTarget:self action:@selector(touchedPlayButton:) forControlEvents:UIControlEventTouchUpInside];
            [self.view addSubview:playBtn];
        } else {
            // Same song/difficulty already in play (iPad): CONTINUE + PLAY-FROM-START.
            UIButton *continueBtn = [[UIButton alloc] init];
            [continueBtn setBackgroundImage:[UIImage imageNamed:@"acv_bt_play_contin"] forState:UIControlStateNormal];
            continueBtn.frame = CGRectMake(30.0f, y, playImg.size.width, playImg.size.height);
            [continueBtn addTarget:self action:@selector(touchedResumeButton:) forControlEvents:UIControlEventTouchUpInside];
            [self.view addSubview:continueBtn];

            UIButton *firstBtn = [[UIButton alloc] init];
            [firstBtn setBackgroundImage:[UIImage imageNamed:@"acv_bt_play_first"] forState:UIControlStateNormal];
            firstBtn.frame = CGRectMake(170.0f, y, playImg.size.width, playImg.size.height);
            [firstBtn addTarget:self action:@selector(touchedPlayButton:) forControlEvents:UIControlEventTouchUpInside];
...
// @ 0xdfe30 — after loading, add a right-swipe pan recogniser (phone only) that acts as a
// back gesture.
- (void)viewDidLoad {
    [super viewDidLoad];
    if (!neSceneManager::isPadDisplay()) {
...
// @ 0xdff0c — treat a rightward pan (translation.x > 80) as a back-button press.
...
// @ 0xdff88 — one AcViewerOptionCell per row (reused by "Cell%ld_%ld"), bound to the
// row's option kind.
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        cell = [[AcViewerOptionCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0: [cell setData:0]; break;
            case 1: [cell setData:1]; break;
            case 2: [cell setData:2]; break;
            case 3: [cell setData:3]; break;
            default: break;
        }
    }
    return cell;
...
// @ 0xe00c8 — push the tapped option's detail screen and swap the nav-bar background to
// match; guarded so it does nothing mid-animation or when not the top view controller.
...
// @ 0xe0374 — PLAY: log the play, commit the pending selection and start the game. On
// iPad the play scene shows behind the panel (fade the black board, hide the panel, ask
// the AcMainTask to exit the *viewer*); on phone just close this screen.
- (void)touchedPlayButton:(id)sender {
    ...
    if (neSceneManager::isPadDisplay()) {
        ...
        neEngine::acMainRequestGameExit(static_cast<AcMainTask *>(AppDelegate.appDelegate.acMainTask));
    } else {
        [self startCloseAnimation];
    }
}

// @ 0xe0490 — CONTINUE: apply the current options to the running AcMainTask and hide the
// panel (animated) so the in-progress play resumes.
...
// @ 0xe053c — BACK: off the AC-main flow, restore the "friman" nav-bar and pop this
// screen; on the AC-main flow, apply the options to the task and fade the panel closed.
- (void)touchedBackButton:(id)sender {
    ...
    neEngine::playSystemSe(2);
    if (!_forAcMain) {
        [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"acv_friman_navbar"]
                                                     forBarMetrics:UIBarMetricsDefault];
...
// @ 0xe0664 — fire-and-forget analytics POST recording that an AC play started (uuid body,
// user-agent + store headers), sent to the arcade-viewer play-log endpoint.
- (void)sendLog {
...
// @ 0xe0820 — AC-main flow: add the nav controller's view over the root scene and fade
// this screen (+ its nav view) 0 -> 1 over 0.3 s.
...
// @ 0xe0960 — fade this screen (+ its nav view) 1 -> 0 over 0.3 s; didStop routes to
// endCloseAnimation (or endCloseAnimationForAcMain on the AC-main flow).
...
// @ 0xe0a78 — remove the nav view and notify the root that the AC viewer closed.
