...
//
// (both emit dmb barriers, confirming atomic).
- (int)recommendPackId {
...
// pack.
//
// (recommendPackId > 0 short-circuits before the callback).
...
//
// (pad test is (orientation - 1) < 2, i.e. Portrait or
// PortraitUpsideDown).
...
//
// (binary also releases the five object ivars; ARC-synthesised here).
- (void)dealloc {
