...
//  pop'n rhythmin
//
//  See StoreDialogView.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin.
//
//  Byte-decoded constants (all little-endian float hex verified against the
//  binary):
//    * card layer: cornerRadius 8.0 (0x41000000), borderWidth 2.0 (0x40000000),
//      shadowRadius 8.0 (0x41000000), shadowOffset CGSizeZero, shadowOpacity
//      0.5 (0x3f000000), shouldRasterize YES; backgroundColor colorWithWhite:0
//      alpha:0.7 (0x3f333333).
//    * spinner box 40x40 (0x42200000); style 0 = WhiteLarge; center = (w/2,
//    trunc(h * 0.2)),
//      0.2 = DAT_00041c44 (0x3e4ccccd).
//    * label frame (0, 0, w-30, 24) — 30.0 = 0xc1f00000, 24.0 = 0x41c00000;
//    clear bg, white text,
//      center aligned, 1 line, font DFSoGei @18.0 (0x41900000).
//    * progress (Bar, style 1) frame (30, trunc(h/2)+10, w-60, 11) — 30.0 =
//    0x41f00000,
//      -60.0 = DAT_00041db8 (0xc2700000), 11.0 = 0x41300000; initial progress
//      0.3 (0x3e99999a).
//    * abort button: image "store_btn_abort.png" stretchable(6,6); custom type;
//    frame (0,0,140,imgH)
//      140.0 = 0x430c0000; white title, font DFSoGei @20.0 (0x41a00000), title
//      shadow (0,-1.0) (0xbf800000) color white/alpha 0.6 (0x3f19999a); title
//      "中止" (UTF-16 0x4e2d 0x6b62); center = (w/2, trunc(h * 0.83)), 0.83 =
//      DAT_00041dbc (0x3f547ae1); event 0x40 = TouchUpInside.
//    * -layout: label offset ±10.0 (0x41200000 / 0xc1200000).
//
//  NEON note: every subview frame/center is computed with packed NEON float
//  vectors that spill through the stack; the size lanes and the constants above
//  are byte-exact, the origin/center geometry is reconstructed best-effort.
//  Where the binary truncates a lane with vcvt.s32.f32 the (int) cast below
//  reproduces it. getFontNameDFSoGei() == AppFontName().
//
//  The compiler-artifact .cxx_construct/.cxx_destruct (if any) are not
//  reconstructed.
//
...
- (instancetype)initWithFrame:(CGRect)frame abortable:(BOOL)abortable {
    ...
    if (self != nil) {
        // Propagate the main screen scale to the layer (guarded for pre-scale
        // OSes).
        ...
        // Binary factors this into a -[UIView setAutoresizingCenter] category:
        // flexible margins on all four sides keep the card centered in its
        // superview.
        self.autoresizingMask =
            UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin |
            UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin;

        // Spinner: 40x40 white-large, centered horizontally, ~0.2 of the height
        // down.
        m_IndicatorView =
        ...
        m_IndicatorView.center =
            CGPointMake(frame.size.width * 0.5f, (float)(int)(frame.size.height * 0.2f));
        [self addSubview:m_IndicatorView];

        // Message label: full width minus 30pt, 24pt tall; white, centered.
        // -layout: recenters it.
        m_LabelMessage =
            [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 0.0f, frame.size.width - 30.0f, 24.0f)];
        ...
        // Progress bar: 30pt inset each side, just below the card's vertical
        // center.
        m_ProgressView = [[UIProgressView alloc] initWithProgressViewStyle:UIProgressViewStyleBar];
        m_ProgressView.frame = CGRectMake(
            30.0f, (float)((int)(frame.size.height * 0.5f) + 10), frame.size.width - 60.0f, 11.0f);
        ...
        if (abortable) {
            ...
            UIImage *abortImage =
                [[UIImage imageNamed:@"store_btn_abort.png"] stretchableImageWithLeftCapWidth:6
                                                                                 topCapHeight:6];
            // buttonWithType: returns an autoreleased button that the binary retains;
            // under ARC the strong ivar store keeps it alive.
            ...
            m_ButtonAbort.center =
                CGPointMake(frame.size.width * 0.5f, (float)(int)(frame.size.height * 0.83f));
...
// dealloc @ 0x41dc0 — release-only (release-chains
// m_IndicatorView/m_LabelMessage/m_ProgressView/ m_ButtonAbort then [super
// dealloc]); ARC-omitted.
...
// Synthesized accessors: -delegate @ 0x41f8c, -setDelegate: @ 0x41f9c,
// -indicatorView @ 0x41fac, -labelMessage @ 0x41fbc, -progressView @ 0x41fcc,
// -buttonAbort @ 0x41fdc (see @property / @synthesize).
