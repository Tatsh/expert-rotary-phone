...
//  pop'n rhythmin
//
//  See FreeRequestListViewController.h. Reconstructed from Ghidra project rb420,
...
//     Content-Type "application/json"). The parsed rows are NSValue-wrapped FriendListData
//     (see DownloadMain.h) — the same struct/type-encoding the sibling
//     -[DownloadMain getFriendListFinished] produces; -downloaderFinished: here mirrors that
//     parse exactly, keyed on the "List" array (vs. "Friend"), with FriendShip forced to 0
//     (no "FriendShip" key is read for this list).
//   - The binary builds the per-field values into ~26 parallel NSMutableArrays first, then a
//     second pass assembles each FriendListData; that is behaviourally identical to the direct
//     per-entry assembly used here. Under ARC the struct's NSString* fields are held
//     __unsafe_unretained (see DownloadMain.h), so no explicit retain/release is issued —
//     -releaseFriendList only unboxes, matching -[DownloadMain releaseFriendList].
...
// @ 0xe5430 — build the header plate, dimmed dummy overlay + spinner, and the back button.
- (instancetype)initWithStyle:(UITableViewStyle)style {
    if ((self = [super initWithStyle:style])) {
        ...
        UIImage *backImg =
            [UIImage imageNamed:(neSceneManager::isPadDisplay() ? @"pl_checker_return"
                                                                : @"navi_btn_back")];
...
// @ 0xe5e40 — raise the FreeRequestDetail confirm overlay for the tapped row (section 0 only).
...
// @ 0xe60cc — unbox each FriendListData row (its two __unsafe_unretained NSString fields),
// then drop the array. Mirrors -[DownloadMain releaseFriendList].
- (void)releaseFriendList {
...
// @ 0xe61e0 — recommend-friend list arrived: alert on a nil response, else parse the "List"
// array into FriendListData rows and reload. Always drops the downloader + hides the spinner.
- (void)downloaderFinished:(Downloader *)downloader {
    ...
    if (json == nil) {
        // (the binary additionally probes [json["ErrorCode"] isKindOfClass:[NSNumber class]] here,
        //  but discards the result; on a nil response it just shows the failure alert.)
        ...
        if (list != nil) {
            ...
            for (NSDictionary *entry in list) {
                ...
                for (int d = 0; d < 3; d++) {
                    ...
                    int perfect =
                        [entry[[@"Perfect" stringByAppendingString:kDiff[d]]] intValue];
...
// @ 0xe6c80 — download failed: drop the downloader, hide the spinner, show the failure alert.
...
// @ 0xe6d60 — POST the recommend-friend request (once), revealing the spinner overlay.
- (void)startGetRecommendFriendHttp {
    if (_downloader != nil) {
        return;
    }
    NSString *body =
        [NSString stringWithFormat:@"uuid=%@", [[AppDelegate appDelegate] uuId]];
    _downloader = [[Downloader alloc]
        initWithURL:[StoreUtil getRecommendFriendURL]
           delegate:self
...
// @ 0xe6ea4 — back button: pop this list (blocked while a confirm overlay is open).
- (void)backButtonFunc {
    if (_freeRequestDetail != nil && [_freeRequestDetail isEnabled]) {
        return;
    }
...
// @ 0xe5bb4 — cancel the in-flight download (so no late callback fires into a dead controller)
// and unbox the retained row strings before teardown. Kept under ARC because it cancels a
// Downloader; the object-ivar releases are ARC-managed.
- (void)dealloc {
    if (_downloader != nil) {
        [_downloader cancel];
    }
