...
- (instancetype)initWithCountryCode:(NSString *)countryCode categoryId:(NSString *)categoryId {
    if ((self = [super init])) {
        _serviceName = [NSString stringWithFormat:@"%@_%@_%@",
                        @"ApplilinkRecommend.AdId", countryCode, categoryId];
    }
    return self;
}
...
- (id)getWithCountryCode:(NSString *)countryCode categoryId:(NSString *)categoryId
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0f) {
        ...
        id record = [self getPasteboardWithUdid:udid countryCode:countryCode
                                     categoryId:categoryId error:&innerError];
        if (innerError != nil) {
            if (error != NULL) {
                *error = innerError;
            }
...
- (id)setWithAdIdFrom:(NSString *)adIdFrom
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0f) {
        ...
        NSString *udid = [RecommendAdId sha1:[[manager advertisingIdentifier] UUIDString]];
        NSError *innerError = nil;
        [self setPasteboardWithUdid:udid countryCode:countryCode categoryId:categoryId
                           adIdFrom:adIdFrom adType:adType error:&innerError];
        if (innerError != nil) {
            if (error != NULL) {
                *error = innerError;
            }
        ...
        // Encrypt each field (op 0 = encrypt) under a key derived from the service name.
        NSData *key = [RecommendAdId createHash:[_serviceName dataUsingEncoding:NSUTF8StringEncoding]];
        NSData *encAdIdFrom = [RecommendAdId cryptorToData:0
                                                     value:[adIdFrom dataUsingEncoding:NSUTF8StringEncoding]
                                                       key:key];
        NSData *encCountry = [RecommendAdId cryptorToData:0
                                                    value:[countryCode dataUsingEncoding:NSUTF8StringEncoding]
                                                      key:key];
        NSData *encCategory = [RecommendAdId cryptorToData:0
...
- (BOOL)deleteWithCountryCode:(NSString *)countryCode categoryId:(NSString *)categoryId
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0f) {
        ...
        NSString *udid = [RecommendAdId sha1:[[manager advertisingIdentifier] UUIDString]];
        NSError *innerError = nil;
        [self deletePasteboardWithUdid:udid countryCode:countryCode categoryId:categoryId
                                 error:&innerError];
        if (innerError != nil) {
            if (error != NULL) {
                *error = innerError;
...
// @ 0xea914 — decrypt (op 1 = decrypt) the archived local-pasteboard record's data fields back
// into UTF-8 strings. AdType is optional and only present when it was supplied at store time.
- (NSDictionary *)convertToData:(NSDictionary *)dict {
    NSMutableDictionary *out = [NSMutableDictionary dictionaryWithDictionary:dict];
    NSData *key = [RecommendAdId createHash:[_serviceName dataUsingEncoding:NSUTF8StringEncoding]];

    NSData *adIdFrom = [out objectForKey:@"AdIdFrom"];
    [out setObject:[[NSString alloc] initWithData:[RecommendAdId cryptorToData:1 value:adIdFrom key:key]
                                         encoding:NSUTF8StringEncoding]
            forKey:@"AdIdFrom"];

    NSData *countryCode = [out objectForKey:@"CountryCode"];
    [out setObject:[[NSString alloc] initWithData:[RecommendAdId cryptorToData:1 value:countryCode key:key]
                                         encoding:NSUTF8StringEncoding]
            forKey:@"CountryCode"];

    NSData *categoryId = [out objectForKey:@"CategoryId"];
    [out setObject:[[NSString alloc] initWithData:[RecommendAdId cryptorToData:1 value:categoryId key:key]
                                         encoding:NSUTF8StringEncoding]
            forKey:@"CategoryId"];

    NSData *adType = [out objectForKey:@"AdType"];
    if (adType != nil) {   // decompiler aliases this test onto AdIdFrom's register; it guards AdType
        [out setObject:[[NSString alloc] initWithData:[RecommendAdId cryptorToData:1 value:adType key:key]
                                             encoding:NSUTF8StringEncoding]
                forKey:@"AdType"];
    }
    return out;
...
- (id)getPasteboardWithUdid:(NSString *)udid
    ...
    NSString *url = [[RecommendCore baseUrlSsl] stringByAppendingString:@"/ad/external/pasteboard/get.php"];
    ...
    if (![statusObj isKindOfClass:[NSString class]] && ![statusObj isKindOfClass:[NSNumber class]]) {
        statusObj = nil;
    }
    ...
    if (!status || errorCode != 100000000) {
        ...
        if (error != NULL) {
            *error = [RewardNetworkError localizedRewardNetworkErrorWithCode:code userInfo:response];
        }
        return nil;
    }
...
- (void)setPasteboardWithUdid:(NSString *)udid
    ...
    NSString *url = [[RecommendCore baseUrlSsl] stringByAppendingString:@"/ad/external/pasteboard/set.php"];
    NSError *requestError = nil;
    NSDictionary *response = [RecommendWebAPI requestSynchronousWithURL:url
                                                                method:@"POST"
                                                            parameters:params
...
- (void)deletePasteboardWithUdid:(NSString *)udid
    ...
    NSString *url = [[RecommendCore baseUrlSsl] stringByAppendingString:@"/ad/external/pasteboard/delete.php"];
    NSError *requestError = nil;
    NSDictionary *response = [RecommendWebAPI requestSynchronousWithURL:url
                                                                method:@"POST"
                                                            parameters:params
...
// @ 0xeac08 — lowercase hex SHA-1 of the UTF-8 bytes of a string (40-char digest string).
...
// @ 0xea72c — raw 20-byte SHA-1 digest of arbitrary data (used as the AES key material).
...
// @ 0xea7d8 — AES-128 (PKCS7-padded) transform. operation is kCCEncrypt (0) or kCCDecrypt (1);
// key is used as a 16-byte AES key with a zero IV. Returns the transformed data, or nil on error.
