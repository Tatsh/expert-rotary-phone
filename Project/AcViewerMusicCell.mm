...
// One difficulty level label (identical layout for all four tiers; only the
// colour and the level number differ). The binary unrolls this inline four
// times; collapsed here.
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style
    ...
    if ((self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])) {
        ...
        CGFloat firstX =
            (UIDevice.currentDevice.systemVersion.floatValue < 7.0f ? 22 : 32) + _offsetForPad2;
...
// dealloc @ 0x40954 — ARC-omitted (object ivars only: releases the four
// difficulty buttons).

// @ 0x409e0 — bind one arcade song row: a banner background (on iPad centred
// and vertically offset by half its height), the song or genre title, and the
...
- (void)setData:(AcMusicData *)data {
    ...
    // Banner background: created at cell bounds, then resized to the banner
    // image.
    ...
    if (!_isPad) {
        ...
        // iPad: centre the banner horizontally and drop it so its top sits at y =
        // 0.
    ...
    // Title label (song name, or genre name when the arcade viewer is in genre
    // mode).
    _titleLbl = [[UILabel alloc] init];
    _titleLbl.backgroundColor = [UIColor clearColor];
    _titleLbl.textColor = [UIColor colorWithRed:48.0f / 255.0f
                                          green:48.0f / 255.0f
                                           blue:48.0f / 255.0f
                                          alpha:1.0f];
    ...
    _titleLbl.frame =
        CGRectMake(_offsetForPad2 + 20, 15.0f, osVersion < 7.0f ? 263.0f : 280.0f, 18.0f);
    _titleLbl.backgroundColor = [UIColor clearColor];
    ...
    if ([data lvEasy] < 1) {
        ...
        _lvEsLbl = AcvMakeLevelLabel([UIColor colorWithRed:113.0f / 255.0f
                                                     green:179.0f / 255.0f
                                                      blue:255.0f / 255.0f
                                                     alpha:1.0f],
                                     [data lvEasy]);
    ...
    if ([data lvNormal] < 1) {
        ...
        _lvNLbl = AcvMakeLevelLabel([UIColor colorWithRed:59.0f / 255.0f
                                                    green:117.0f / 255.0f
                                                     blue:28.0f / 255.0f
                                                    alpha:1.0f],
                                    [data lvNormal]);
    ...
    if ([data lvHyper] < 1) {
        ...
        _lvHLbl = AcvMakeLevelLabel([UIColor colorWithRed:207.0f / 255.0f
                                                    green:123.0f / 255.0f
                                                     blue:15.0f / 255.0f
                                                    alpha:1.0f],
                                    [data lvHyper]);
    ...
    if ([data lvEx] < 1) {
        ...
        _lvExLbl = AcvMakeLevelLabel([UIColor colorWithRed:255.0f / 255.0f
                                                     green:133.0f / 255.0f
                                                      blue:188.0f / 255.0f
                                                     alpha:1.0f],
                                     [data lvEx]);
