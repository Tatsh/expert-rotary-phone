...
//
//  Touch handling: while a finger is down, -touchesBegan/Moved/Ended track a single
//  touch point (_touchedPoint, seeded to (-1,-1) = "no touch") and toggle the
//  preview's -isTouched flag via -isEnablePoint:, which tests whether the point lies
//  within _radius of the pop-kun centre.
//
//  Honesty / recovery notes:
//   * All CGRect / CGPoint reads in the binary are NEON-spilled with compiler-emitted
//     "receiver == nil -> zero the struct" guards; those are elided here and modelled
//     as ordinary Cocoa -frame / -locationInView: calls (behaviourally identical for
//     the non-nil case that always holds at runtime). _touchedPoint (CGPoint @0xb4) is
//     one such spilled frame.
//   * Float constants are the exact IEEE-754 values decoded from the binary (annotated
//     inline with their hex): 0.95 (0x3f733333), 40.0 (0x42200000), 10.0 (0x41200000),
//     110.0 (0x42dc0000), 148.0 (0x43140000), 160.0 (0x43200000), 268.0 (0x43860000),
//     68.0 (0x42880000), -1.0 (0xbf800000).
//   * -isEnablePoint: calls a free helper pointInCircle(px,py,cx,cy,radius) in the
//     binary; it is inlined here as the equivalent squared-distance test.
//   * The two pop-kun image names ("ta_popkun_before"/"ta_popkun_after") are the ASCII
//     CFStrings @ 0x1389d8 / 0x1389e8 passed to -[TouchRangeView initWithFilename:touched:].
//   * _radius persists via UserSettingData: read with +touchRadius and written with
//     +saveTouchRadius: (both selectors are present in the binary but NOT yet declared
//     in UserSettingData.h -- only +popkunSize is; see the notes at the call sites).
//
...
- (void)viewDidLoad {
    ...
    // --- Radius slider ---
    _radiusSlider = [[UISlider alloc]
        initWithFrame:CGRectMake(0, 0, W * 0.95f, 10.0f)];   // 0x3f733333 / 0x41200000
    ...
    [_radiusSlider addTarget:self action:@selector(sliderValChanged:)
    ...
    [_toucheRangeView setFrame:CGRectMake(W * 0.5f - imgW * 0.5f, 268.0f, imgW, imgH)]; // 0x43860000
...
// didReceiveMemoryWarning @ 0x8a9d0 — super-only override, omitted (no added behavior)
...
// @ 0x8aa9c — live slider drag: track the value (the radius circle is re-tested on the
// next touch via -isEnablePoint:).
...
// @ 0x8ab04 — YES if `point` lies within _radius of the pop-kun centre. The centre is
// the live view width's midpoint horizontally, and the pop-kun's vertical midpoint
// (top 268 + height/2). Modelled from the binary's pointInCircle() helper.
...
// @ 0x8abd0 — first touch down: if no touch is active, record its point and light up
// the preview when it falls inside the radius.
...
// @ 0x8ad0c — drag: find the touch whose previous location matches the tracked point,
// update it, and toggle the preview only when the enabled-state actually changes.
...
// @ 0x8af28 — lift: when every touch has ended (or the tracked touch's is the one that
// lifted), clear the preview and reset the tracked point to "none".
...
// @ 0x8b16c — back button: play the cancel SE, restore the settings nav bar background
// and pop.
- (void)backButtonFunc {
