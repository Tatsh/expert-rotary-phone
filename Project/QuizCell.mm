...
- (void)setData:(NSString *)text
    ...
    // (223/255) once the player has chosen the wrong one; otherwise a dark tint
    // (50/255).
    ...
    CGFloat red = ((rightId == selectId) || (selectId < 0) || (rightId != answerId)) ?
                      (50.0f / 255.0f) :
                      (223.0f / 255.0f);
    label.textColor = [UIColor colorWithRed:red
                                      green:(46.0f / 255.0f)
                                       blue:(43.0f / 255.0f)
                                      alpha:1.0f];
    ...
    // Answer-number badge: the "top" glyph (1-based) for unanswered / chosen
    ...
    _answerIdView = [[UIImageView alloc]
        initWithFrame:CGRectMake(0.0f, 0.0f, idImage.size.width, idImage.size.height)];
