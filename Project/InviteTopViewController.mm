...
- (UINavigationController *)initAtNavigationController {
    ...
    // "player" panel image (centred horizontally, y=15), interactive so its button works.
    UIImage *inviteImg = [UIImage imageNamed:@"invite_view"];
    UIImageView *inviteView = [[UIImageView alloc] initWithImage:inviteImg];
    inviteView.frame = CGRectMake((frame.size.width - inviteImg.size.width) * 0.5f, 15.0f,
                                  inviteImg.size.width, inviteImg.size.height);
    inviteView.userInteractionEnabled = YES;

    // "guest" panel image (centred horizontally, below the player panel: y = playerH + 30).
    ...
    guestView.frame = CGRectMake((frame.size.width - guestImg.size.width) * 0.5f,
                                 ...
                                 guestImg.size.width, guestImg.size.height);
    guestView.userInteractionEnabled = YES;

    [scroll addSubview:inviteView];
    [scroll addSubview:guestView];
    scroll.contentSize = CGSizeMake(frame.size.width,
                                    inviteImg.size.height + guestImg.size.height + 100.0f);

    // "player" button on the player panel (bottom-anchored: y = panelH - btnH - 20).
    ...
    playerBtn.frame = CGRectMake((inviteImg.size.width - playerBtnImg.size.width) * 0.5f,
                                 ...
                                 playerBtnImg.size.width, playerBtnImg.size.height);
    playerBtn.exclusiveTouch = YES;
    [playerBtn addTarget:self action:@selector(touchedInviteButton:)
        forControlEvents:UIControlEventTouchUpInside];
    [inviteView addSubview:playerBtn];

    // "guest" button on the guest panel (bottom-anchored: y = panelH - btnH - 15).
    ...
    guestBtn.frame = CGRectMake((guestImg.size.width - guestBtnImg.size.width) * 0.5f,
                                ...
                                guestBtnImg.size.width, guestBtnImg.size.height);
    guestBtn.exclusiveTouch = YES;
    [guestBtn addTarget:self action:@selector(touchedInputButton:)
        forControlEvents:UIControlEventTouchUpInside];
...
// @ 0xe7860 — "player" button: push the my-invite-code screen (play the decide SE).
- (void)touchedInviteButton:(id)sender {
...
// @ 0xe7914 — "guest" button: push the invite-code input screen (play the decide SE).
- (void)touchedInputButton:(id)sender {
    if (self.navigationController.topViewController != self) {
        return;
    }
...
// @ 0xe7c90 — pull the view, notify the root VC the invite flow closed, clear the guard.
- (void)endCloseAnimation {
