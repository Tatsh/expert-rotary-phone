...
//  pop'n rhythmin
//
//  See CustomWebView.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin.
//
...
- (instancetype)initWithURL:(NSURL *)url {
    ...
    if (self) {
        ...
        if (!neSceneManager::isPadDisplay()) {
            ...
            // pad: x=570.0 (movt #0x440e → 0x440e8000), y=15.0 (movt #0x4170 →
            // 0x41700000). Byte-verified.
            CGSize sz = closeSmallImg ? closeSmallImg.size : CGSizeZero;
        ...
        [_closeBtnSmall addTarget:self
                           action:@selector(pushCloseBtn)
                 forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_closeBtnSmall];

        // --- big close button (pinned into the scroll content, revealed by the KVO
        // observer) ---
        ...
        [_closeBtnBig setFrame:CGRectMake((contentSize.width - bigSz.width) * 0.5f,
                                          0.0f,
                                          bigSz.width,
                                          bigSz.height)];
        [_closeBtnBig setHidden:YES];
        [_closeBtnBig setBackgroundImage:closeBigImg forState:UIControlStateNormal];
        [_closeBtnBig addTarget:self
                         action:@selector(pushCloseBtn)
               forControlEvents:UIControlEventTouchUpInside];
        [_webView.scrollView addSubview:_closeBtnBig];

        // Observe the scroll view's content size so the big close button can be
        // repositioned once the page lays out (see -observeValueForKeyPath:…).
        ...
        // 50×50 (vmov.f32 0x42480000), centred: x = width*0.5−25
        // (0xc1c80000=−25.0), y = height*0.5−25. Byte-verified.
        _indicator.frame = CGRectMake(self.frame.size.width * 0.5f - 25.0f,
                                      self.frame.size.height * 0.5f - 25.0f,
                                      50.0f,
                                      50.0f);
...
- (void)close {
    [UIView animateWithDuration:0.5
        ...
        completion:^(BOOL finished) {
          // @ 0x5e7b5 — completion block: if a C close callback was registered,
          // invoke it with its param, then detach the panel.
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    [_closeBtnBig
        setFrame:CGRectMake(bigFrame.origin.x, y, bigFrame.size.width, bigFrame.size.height)];
}
...
- (void)touchedFollowButton {
    ...
    if (![UserSettingData isFollowBonusGet]) {
        ...
        CommonAlertView *alert = [[CommonAlertView alloc]
                initWithTitle:nil
                      message:[NSString
                                  stringWithFormat:@"トレジャーポイント%dPゲットしました", 3000]
                     delegate:nil
            cancelButtonTitle:nil
