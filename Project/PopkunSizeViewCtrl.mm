...
//  PopnRhythmin. Objective-C++ (MRC) for the neSceneManager / neEngine C++
//  bridge.
//
//  viewDidLoad builds the screen entirely in code. The device class
//  (neSceneManager::isPadDisplay()) selects one of two layouts:
//
//   * iPhone (non-pad): controls are centred horizontally in the live view
//   width W
//     and stacked with per-control Y offsets seeded into offsetYForPad1..4. A
//     "popkun_size_text" caption image sits at the top and a custom
//     "navi_btn_back" bar button (-> backButtonFunc) is installed as the left
//     nav item.
//   * iPad (pad): W is fixed at 428pt; offsetYForPad1..4 are small paddings;
//   the
//     controls use hard-coded centres inside the panel, an info UILabel
//     ("ポップ君の大きさを変更できるよ！") and a "popkun_size_preview" image
//     are added, and the system back button is hidden (setHidesBackButton:YES)
//     rather than replaced with a custom bar button.
//
...
//     modelled as ordinary Cocoa -frame/-center/-size calls (behaviourally
//     identical for the non-nil case that always holds at runtime).
//   * Float/colour constants are the exact IEEE-754 values decoded from the
//   binary
//     (annotated inline with their hex).
//   * The info-label text is a UTF-16 CFString @ 0x12ca5c, bytes
//     DD30 C330 D730 1B54 6E30 2759 4D30 5530 9230 0959 F466 6730 4D30 8B30
//     8830 01FF = "ポップ君の大きさを変更できるよ！".
//   * offsetYForPad1..4 (int ivars @0xcc/0xd0/0xd4/0xd8) are the per-control Y
//   bias
...
// The size slider's backing font (CFString cf_BullyBold in the binary); a
// bundled display face with no AppFont helper, so the literal name is used
// directly.
...
- (void)viewDidLoad {
    ...
    // @ 0x8b4d0 — bare -[CustomAlertView init] (inherited UIImageView init; no
    // custom designated initializer is used here).
    ...
    if (!isPad) {
        ...
        [self.view
            setBackgroundColor:[UIColor
                                   colorWithPatternImage:[UIImage imageNamed:@"popkun_size_bg"]]];
    ...
    _sizeSlider = [[UISlider alloc] initWithFrame:CGRectMake(0, 0, sliderW, 30.0f)]; // h 0x41f00000
    if (isPad) {
    ...
    if (isPad) {
        ...
        infoLbl.textColor = [UIColor colorWithRed:0.188235f
                                            green:0.188235f
                                             blue:0.188235f
        ...
        UIImageView *preview =
            [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"popkun_size_preview"]];
...
// dealloc @ 0x8c1fc — ARC-omitted (chained to super only; all sub-views
