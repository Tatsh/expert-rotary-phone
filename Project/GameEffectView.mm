...
//  pop'n rhythmin
//
//  See GameEffectView.h. Reconstructed from Ghidra project rb420, program PopnRhythmin
//  (initWithStyle: @ 0x72d4c, dealloc @ 0x72eb0, viewDidLoad @ 0x72edc,
...
//  Objective-C++ for the C++ engine bridge (neSceneManager:: / neEngine::).
//
//  Byte-decoded constants (float hex -> decimal): phone row height 61.0 (0x42740000),
//  font 15.0 (0x41700000, cells) / 16.0 phone & 14.0 pad headers, header container
//  320 x 61 phone (x 15) / 320 x 32 pad (x 5), header height 61.0 phone / 32.0 pad.
//  UIControlEventTouchUpInside = 0x40. reloadRows animation 5 = UITableViewRowAnimationNone.
//  Checkmark placement offsets: phone (+245, +15); pad iOS>=7 (+230, +8) / pre-iOS7
//  (+210, +8) — recovered from DAT_00073514/0c/10; see NEON note below.
//
//  HONESTY NOTE — verbatim label texts. The cell/ header UILabel text is set from
//  deduplicated CFString constants that, when byte-decoded, read as strings unrelated
//  to this screen: row-0 cell text is the UTF-16LE "フレンドを解除します" (@ 0x12ce90,
//  flags 0x7d0, len 10), row-1 cell text is the ASCII "custom_bg" (@ 0x10b242), and the
//  section-header label text is the ASCII "Sheet" (@ 0x108f25). These are byte-exact
//  from the binary and appear to be leftover / linker-merged placeholder literals — the
...
// patterned background and 61 px rows; on iPad the table is made borderless and clear.
...
// @ 0x73128 — one toggle row: a text label (see HONESTY NOTE), an on/off checkmark, and
// (iPad only) a "custom_bt02" row background. Reuse id is "Cell<section>-<row>".
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    // Verbatim leftover/placeholder label texts (see HONESTY NOTE in the file header).
    if (indexPath.row == 0) {
...
// @ 0x73518 — play the decide SE, flip the tapped row's stored flag, then reload the row
// so its checkmark updates.
...
// @ 0x735dc — section header: a clear container UIView holding a centred label. The
// label text is the verbatim "Sheet" leftover constant (see HONESTY NOTE). Phone uses a
// 320x61 container at x=15 with a 16 pt font; iPad a 320x32 container at x=5 with 14 pt.
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    CGRect frame;
    CGFloat fontSize;
    if (!neSceneManager::isPadDisplay()) {
        frame = CGRectMake(15.0f, 0.0f, 320.0f, 61.0f);   // 0x41700000 / 0x43a00000 / 0x42740000
        fontSize = 16.0f;                                  // 0x41800000
    } else {
        frame = CGRectMake(5.0f, 0.0f, 320.0f, 32.0f);    // 0x40a00000 / 0x43a00000 / 0x42000000
...
// @ 0x737d8 — back-button action: play the cancel SE, restore the "settings_navbar" bar
// background, pop self, then re-apply the stored SE volume.
- (void)backButtonFunc {
