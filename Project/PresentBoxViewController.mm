...
//  pop'n rhythmin
//
//  See PresentBoxViewController.h. Reconstructed from Ghidra project rb420,
//  program PopnRhythmin. Objective-C++ for the C++ neSceneManager / neEngine
//  singletons. ARC.
//
//  Honesty notes:
//   - Present rows are NSValue-wrapped PresentData
//   ({presentId,itemId,itemNum,info};
//     see DownloadMain.h). -downloadMainFinished: is the shared DownloadMain
//     callback (performSelector-style NSNumber result): <0 = network error, ==1
//     = a present claim finished, ==0 = the present list finished. On a claim
//     it credits the player's Crypt109 charaTicket (itemId 1) or treasurePoint
//     (itemId 0) for every matching row (all rows when getPresentId == -1, i.e.
//     "acquire all"), and on the acquire-all path shows a "一括受け取り" gift
//     alert before re-fetching the list.
//   - The alert strings are exact CFString decodes (UTF-16LE): network-failure
//   message
//     "通信に失敗しました。\n電波状態の良い場所でやり直して下さい。" (OK); the
//     acquire-all gift alert "一括受け取り" / "プレゼントを受け取りました。"
//     (OK); the per-row claim confirm uses the item text as title ("キャラチケ
//     %d枚" / "トレジャーポイント %dP"), the row's Info blurb as message,
//     "キャンセル" (index 0) and "受け取る" (index 1).
//   - Open/close animations: on phone a 0.3s alpha fade (didStop ->
//   end*Animation); on
//     pad a two-phase ~1/6 s frame slide via animateWithDuration.  Open: park
//     at rootView.height, slide to y=420 (setNavViewFrameA @ 0x24f40), then
//     settle to y=470 (setNavViewFrameB @ 0x25078), then endOpenAnimation.
//     Close: slide from 470 → 420 (setNavViewFrameC @ 0x25320), then park below
//     screen (setNavViewFrameFromSubview @ 0x25460), then endCloseAnimation.
//     The binary's two first-phase animation blocks are byte-identical (both
//     target y=420); the
...
//   - -endCloseAnimation removes the nav host and pokes the menu's
//   -PresentBoxEndCallBack
//     (the root VC is a MainViewController, which owns that selector @ 0xe0d4).
//   - The dummy overlay carries the download spinner; -isAnimationing is an
//   atomic read
...
/**
 * The present box's private actions and claim helpers.
 */
@interface PresentBoxViewController () <DownloadMainDelegate>
/** The open transition finished; clear the animation guard. */
- (void)endOpenAnimation;
/** The close transition finished; tear the box down. */
- (void)endCloseAnimation;
/** The back-button action. */
- (void)backButtonFunc;
/** Claim every present in the box. */
- (void)allGetFunc;
/**
...
@implementation PresentBoxViewController {
    ...
    NSValue *_presentDataValue;        // the row awaiting confirm (bound in
                                       // touchedGetButton:)
}
...
- (instancetype)initWithStyle:(UITableViewStyle)style {
    if ((self = [super initWithStyle:style])) {
        ...
        // Empty-state banner ("pbox_no_pbox"), hidden until the list is known
        // empty.
        ...
        // Acquire-all button ("pbox_bt_acquis"): y = -10 pre-iOS 7, else -15; on
        // pad its x-centre is pinned to 170.
        ...
        _btnGetAll.frame =
            CGRectMake(0, (sysVer < 7.0f) ? -10.0f : -15.0f, btnImg.size.width, btnImg.size.height);
        if (neSceneManager::isPadDisplay()) {
            _btnGetAll.center = CGPointMake(170.0f, _btnGetAll.center.y);
        }
        _btnGetAll.hidden = YES;
        [_btnGetAll addTarget:self
                       action:@selector(allGetFunc)
             forControlEvents:UIControlEventTouchUpInside];
...
// @ 0x24988 — detach from the DownloadMain present delegates (kept under ARC
...
- (void)startOpenAnimation {
    ...
    if (!neSceneManager::isPadDisplay()) {
        ...
        // Phase 1 (~1/6 s): slide from off-screen to y = 420 (setNavViewFrameA @
        // 0x24f40). Phase 2 (~1/6 s): settle to y = 470 (setNavViewFrameB @
        // 0x25078), then
        //   call -endOpenAnimation.
        [UIView animateWithDuration:0.16666667
            delay:0
            options:0 // DAT_00024f30
            animations:^{
...
- (void)startCloseAnimation {
    ...
    if (!neSceneManager::isPadDisplay()) {
        ...
        // Phase 1 (~1/6 s): slide from y = 470 back to y = 420 (setNavViewFrameC @
        // 0x25320). NOTE: the binary's open- and close-animation first blocks are
        // byte-identical (both target y = 420); the symmetry here is intentional —
        // reproduced for fidelity. Phase 2 (~1/6 s): park below screen
        // (setNavViewFrameFromSubview @ 0x25460), then
        //   call -endCloseAnimation.
        UIViewController *rootVC = neSceneManager::rootViewController();
        [UIView animateWithDuration:0.16666667
            delay:0
            options:0 // DAT_00025310
            animations:^{
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    [cell.getBtn addTarget:self
                    action:@selector(touchedGetButton:event:)
          forControlEvents:UIControlEventTouchUpInside];
    return cell;
}
...
- (void)downloadMainFinished:(NSNumber *)result {
    ...
    if (result.intValue == 1) {
        // A present claim finished: credit every matching row (all rows for
        // "acquire all").
        ...
        for (NSUInteger i = 0; i < n; i++) {
            ...
            if (dm.getPresentId == -1 || dm.getPresentId == data.presentId) {
                if (data.itemId == 1) {
                    [UserSettingData
                        saveCharaTicket:(short)([UserSettingData charaTicket] + data.itemNum)];
                } else if (data.itemId == 0) {
                    [UserSettingData
                        saveTreasurePoint:(short)([UserSettingData treasurePoint] + data.itemNum)];
                }
        ...
        if (dm.getPresentId == -1) {
            CGPoint center =
                neSceneManager::isPadDisplay() ? CGPointMake(170.0f, 240.0f) : CGPointZero;
            CustomAlertView *gift =
                [[CustomAlertView alloc] initWithView:self.tableView
                                               center:center
    ...
    // Present list finished: snapshot the parsed rows and update the empty /
    // scroll state.
...
- (void)touchedGetButton:(id)sender event:(UIEvent *)event {
    ...
    UIViewController *rootVC = neSceneManager::rootViewController();
    CGPoint center = neSceneManager::isPadDisplay() ? CGPointMake(170.0f, 190.0f) : CGPointZero;
    _customAlert = [[CustomAlertView alloc] initWithView:self.view
                                                  center:center
                                                    type:CustomAlertViewTypeGift
                                                   title:title
                                                 message:data.info
