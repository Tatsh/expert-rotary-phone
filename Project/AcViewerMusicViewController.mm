...
//  (AC) viewer's song list. Objective-C++ (.mm) because it drives the C++ "ne"
//  engine singletons via neEngineBridge (scene manager pad-display flag, the
//  AC-viewer event-center selection, and the system-SE hooks).
//
...
// The genre-category header banner, indexed by AcMusicData.category (clamped
// 0..23). The binary looks these up from the CFString table at
// PTR_cf_ppc_mlist_header_etc (0x133568); a nil source array falls back to
...
// @ 0xcba44 — build the list: a transparent, separator-less UITableView; the
...
- (instancetype)initWithData:(NSArray *)acMusicDataArray {
    ...
    // Header height = banner height + (17 top gap from bannerView.origin.y) + 12
    // bottom gap; the binary derives it from the banner image size and bannerView
    // frame via vector adds.
    ...
    // Back button (skipped on iPad when this screen is already the nav top view
    // controller).
    neSceneManager::shared();
    BOOL skipBackButton =
        neSceneManager::isPadDisplay() && self.navigationController.topViewController == self;
    if (!skipBackButton) {
        neSceneManager::shared();
        NSString *backName =
    ...
    _changeButton = [[UIButton alloc]
        initWithFrame:CGRectMake(0.0f, 0.0f, _genreButton.size.width, _genreButton.size.height)];
    ...
    [_changeButton addTarget:self
                      action:@selector(touchedChangeButton:)
            forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItem =
        [[UIBarButtonItem alloc] initWithCustomView:_changeButton];
    return self;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (cell == nil) {
        ...
        [cell.easyBtn addTarget:self
                         action:@selector(touchedSheetButton:event:)
               forControlEvents:UIControlEventTouchUpInside];
        [cell.normalBtn addTarget:self
                           action:@selector(touchedSheetButton:event:)
                 forControlEvents:UIControlEventTouchUpInside];
        [cell.hyperBtn addTarget:self
                          action:@selector(touchedSheetButton:event:)
                forControlEvents:UIControlEventTouchUpInside];
        [cell.exBtn addTarget:self
                       action:@selector(touchedSheetButton:event:)
             forControlEvents:UIControlEventTouchUpInside];
    }
...
// @ 0xcc82c — a difficulty button was tapped: only when this screen is the nav
// top VC; map the button tag (100..103) to a difficulty (0..3), seed the
