...
// The shared DynaFont gothic face, referenced literally by this cell in the binary
// (cf_DFSoGei_W5_WIN_RKSJ_H); identical to AppFontName().
...
- (void)setMapData:(NSValue *)mapValue isSelect:(BOOL)isSelect {
    ...
    UIImage *banner = [UIImage imageNamed:(isSelect ? @"map_select_banner_on" : @"map_select_banner")];
    ...
    UIImageView *iconView = [[UIImageView alloc] initWithFrame:CGRectMake(iconX, 7.0f,
    ...
    // "Cleared" badge — only when every sub-map (0..2) of this map has been cleared.
    NSManagedObjectContext *moc = [[AppDelegate appDelegate] managedObjectContext];
    for (short sub = 0; sub < 3; sub++) {
        TreasureData *td = [TreasureData getTreasureData:v.mapId subMapId:sub inManagedObjectContext:moc];
    ...
    UIImageView *clearView = [[UIImageView alloc] initWithFrame:CGRectMake(253.0f, 3.0f,
                                                                           clearImage.size.width,
                                                                           clearImage.size.height)];
    [clearView setImage:clearImage];
    if (!neSceneManager::isPadDisplay()) {
        if (UIDevice.currentDevice.systemVersion.floatValue < 7.0f) {
            // Pre-iOS 7 phone: nudge the badge up-left to line up with the older banner art.
