...
// populated from the SDK bundle's "Error" strings table; each entry falls back to
// a hard-coded English default when the bundle/string is unavailable. The message
// for the requested code (or 1000, the "unexpected" bucket) is installed under
...
+ (NSError *)localizedRewardNetworkErrorWithCode:(NSInteger)code
    ...
    if (g_pRewardErrorMessageDict == nil) {
        ...
        struct { NSInteger code; NSString *key; NSString *fallback; } entries[] = {
            ...
            { 0x3ed, @"ApplilinkErrorLoginTokenRequestError",     @"Login token request unexpected error." },
            { 0x3ee, @"ApplilinkErrorContentsServer",             @"Contents server error occurred." },
            { 0x3ef, @"ApplilinkInvalidContentsServerStatus",     @"Invalid response status from contents server." },
            ...
            { 0x3f5, @"ApplilinkPasteBoardErrorInvalidField",     @"Failed to get pasteboard index pointer." },
            { 0x3f6, @"ApplilinkPasteBoardErrorUnarchiveFailed",  @"Failed to un-archive pasteboard data" },
            ...
            { 0x3fa, @"ApplilinkPasteBoardErrorInvalidDataType",  @"Failed to get directed pasteboard data type." },
            ...
            { 0x401, @"ApplilinkErrorSdkVersionNotSupported",     @"Reward SDK is supported in iOS 5.0 and later." },
