/**
 * @file
 * The iPad in-place pack-detail panel.
 *
...
/**
 * Receives the iPad detail card's purchase and close requests.
 */
@protocol StorePackDetailViewPadDelegate <NSObject>
@optional
/**
 * The card's buy button was tapped.
...
/**
 * The iPad in-place pack detail card: the jacket, description, song rows and buy button.
 */
...
/**
 * Kick the pack-detail download: if the pack already has its song list, tint and show the
 * card; otherwise grey it, spin the loading indicator and start a StorePackInfoDownloader.
 * @ghidraAddress 0x4f680
 */
- (void)loadInfo;
/**
 * Tear the bound pack down; the iPad detail-close path.
 */
- (void)removePackInfo;

/**
 * Populate the detail card from the bound pack: the name, comment, copyright, jacket, buy
...
/**
 * Choose the purchase button's label for the current ownership and download state.
 * @ghidraAddress 0x4ef54
 */
- (void)selfCheckButtonText;

/**
 * Set the purchase button to its enabled "buy (price)" state.
 * @ghidraAddress 0x4f024
 */
- (void)setButtonTextBuy;

/**
 * Set the purchase button to its enabled, localised "INSTALL" state.
 * @ghidraAddress 0x4f0b8
 */
- (void)setButtonTextInstall;

/**
 * Set the purchase button to its disabled, localised "INSTALLING" state.
 * @ghidraAddress 0x4f144
 */
- (void)setButtonTextInstalling;

/**
 * Set the purchase button to its installed state: a greyed "INSTALLED" once the pack has
...
/**
 * Stop the preview clip: cancel the in-flight download, reset every song row's button, and
...
/**
 * A song row's sample button was tapped: toggle its preview.
 *
...
/**
 * A song row's iTunes button was tapped: open that song's iTunes page.
 * @param sender The tapped button.
...
/**
 * The artist-site button was tapped: open the pack's artist URL.
 * @ghidraAddress 0x50080
 */
- (void)selectWebButton;

/**
 * Hand the purchase off to the delegate, which drives StoreKit.
 * @ghidraAddress 0x4fca4
 */
- (void)doPurchase;

/**
 * Whether the displayed pack has songs and all of them are already downloaded; the purchase
...
/**
 * Whether this pack is one of the recommended packs: its id is in the decoded recommend
...
/**
 * The pack purchase button's full decision tree: when the pack is already owned, re-download
