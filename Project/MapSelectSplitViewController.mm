...
//  pop'n rhythmin
//
//  See MapSelectSplitViewController.h. Reconstructed from Ghidra project rb420,
//  program PopnRhythmin:
//    init                                         @ 0x754d8
//    dealloc                                      @ 0x764dc
//    viewDidLoad                                  @ 0x765dc  (super-only,
//    omitted below) didReceiveMemoryWarning                      @ 0x76608
//    (super-only, omitted below) viewWillAppear:                              @
//    0x76634 setSelectIndexPath:                          @ 0x766b8
...
//
//  Honesty notes:
//   - Every subview frame is a literal IEEE-754 constant recovered from the
//   binary; the two
//     container sizes are derived from their backing image frame (width - 20,
//     height - 130 for the left map clip; width - 20, height - 190 for the
//     right area clip), matching the FloatVectorAdd deltas at DAT_00075c98
//     (-130) / DAT_00075c9c (-190).
//   - Image resource names are best-effort decodes of the binary's C-string
//   symbols
//     (cf_map_select_bg -> "map_select_bg", etc.).
//   - The 7 file-static helpers are block invoke functions Ghidra surfaced as
//   top-level
//     symbols; in the reconstructed ObjC++ they are expressed as static
//     functions called from inside UIView transition animation/completion
//     blocks. The binary stores captured float widths as int32_t in block
//     structs (via vcvt.s32.f32 / vcvt.f32.s32); the ObjC++ captures them
//     directly as CGFloat. The arrow "advance" helpers shift origin.x right by
//     the arrow's own width (displacing it off panel); "reset" restores
//     origin.x to _arrowFrm.origin.x (bringing it back). The "Alt" variants are
//     separate compiled instances of the same logic at the scrollViewDidScroll:
//     call sites.
//   - -scrollViewDidScroll: origin.x guard corrected from origin.y (prior
//   placeholder
//     error) to origin.x, matching the binary's comparison of the first float
//     of each CGRect stret result against _arrowFrm.origin.x (DAT comparisons
//     in the decompile).
//   - -dealloc is KEPT: it detaches this controller from DownloadMain's
//   event-info delegate and
//     tears down the how-to overlay. There is no retained NSTimer (the
//     auto-scroll carousel uses -performSelector:withObject:afterDelay:), so
//     nothing to invalidate. The child-controller / index-path -release calls
//     are ARC-omitted.
//   - The header map-name lookup unwraps _mapSelectViewCtrl.mapDataArray
//   NSValues; that record's
...
// Block invoke body: restore _arrowImageView.frame.origin.x to
...
// Block invoke body: shift _arrowImageView.frame.origin.x right by
...
// Block invoke body: restore _rightDummyView and _rightHeaderDummyView widths
...
// .cxx_construct @ 0x787f0 — compiler-emitted C++ ivar constructor; not
// hand-written.
...
- (instancetype)init {
    if ((self = [super init])) {
        ...
        charaView.frame =
            CGRectMake(341.0f, 608.0f, charaView.frame.size.width, charaView.frame.size.height);
        charaView.backgroundColor = [UIColor clearColor];
        [self.view addSubview:charaView];

        // Left "map select" backing image (interactive; hosts the clipped map
        // list).
        UIImageView *mapSelectBg =
            [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"map_select_mapselect_bg"]];
        mapSelectBg.frame =
            CGRectMake(48.0f, 69.0f, mapSelectBg.image.size.width, mapSelectBg.image.size.height);
        mapSelectBg.userInteractionEnabled = YES;
        [self.view addSubview:mapSelectBg];

        UIView *mapClip = [[UIView alloc] init];
        mapClip.frame = CGRectMake(8.0f,
                                   65.0f,
        ...
        _mapSelectViewCtrl =
            [[MapSelectViewController alloc] initWithStyle:UITableViewStyleGrouped];
        [_mapSelectViewCtrl setMapSelectDelegate:self];
        _mapSelectViewCtrl.view.frame =
            CGRectMake(0.0f, 0.0f, mapClip.frame.size.width, mapClip.frame.size.height);
        ...
        _rightImageView =
            [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"map_select_areaselect_bg"]];
        _rightImageView.frame = CGRectMake(
            390.0f, 69.0f, _rightImageView.image.size.width, _rightImageView.image.size.height);
        ...
        _subMapSelectViewCtrl = [[SubMapSelectViewController alloc]
            initWithTreasureData:[_mapSelectViewCtrl treasureDataArray]
        ...
        _rightHeaderDummyView.frame =
            CGRectMake(8.0f, 84.0f, _rightImageView.frame.size.width - 20.0f, 60.0f);
        ...
        _rightHeaderLabel.textColor = [UIColor colorWithRed:0.2706f
                                                      green:0.2510f
                                                       blue:0.2313f
                                                      alpha:1.0f];
        ...
        // "No area" placeholder, centred in the right panel (hidden unless the map
        // has no areas).
        _rightEmptyImageView =
            [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"area_select_no_area"]];
        ...
        // Bottom event banner (backing image + clip); the carousel is filled by
        // -updateEventInfo.
        _eventImageView =
            [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"map_select_event_bg"]];
        _eventImageView.frame = CGRectMake(
            2.0f, 733.0f, _eventImageView.image.size.width, _eventImageView.image.size.height);
        ...
        UIButton *backButton = [[UIButton alloc]
            initWithFrame:CGRectMake(10.0f, 10.0f, backImage.size.width, backImage.size.height)];
...
- (void)dealloc {
    ...
    // calls are ARC-managed.
    if (_howtoViewCtrlPad != nil) {
        [_howtoViewCtrlPad.view removeFromSuperview];
    }
}

// viewDidLoad @ 0x765dc and didReceiveMemoryWarning @ 0x76608 are super-only
// overrides — omitted.
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (_scrollView != scrollView) {
        ...
        // animating, displace it off the panel and collapse/restore the right
        // clips. Guard: compare origin.x (first float of the frame struct) against
        // _arrowFrm.origin.x.
        if (_arrowImageView.frame.origin.x != _arrowFrm.origin.x) {
            return;
        }
        ...
        // animating guard (Ghidra block LAB_00077b3c_1:
        // [_mapSelectViewCtrl.tableView setScrollEnabled:1]; _isAnimationing = 0 —
...
- (void)updateEventInfo {
    ...
    if (events.count == 0) {
        ...
        _scrollView = [[UIScrollView alloc]
            initWithFrame:CGRectMake(0.0f, 0.0f, eventStrip.size.width, eventStrip.size.height)];
        ...
        if (events.count != 0) {
            ...
            UIImage *eventImage =
                [UIImage imageNamed:[NSString stringWithFormat:@"event_0_%03d", firstId]];
        ...
        _pageCtrl = [[UIPageControl alloc]
            initWithFrame:CGRectMake(0.0f, 0.0f, _eventDummyView.frame.size.width, 30.0f)];
