...
//  pop'n rhythmin
//
//  See FriendListDetail.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin (initWithFrame:friendData: @ 0xb4280, addCntNum:sheet:y:view: @
//  0xb5230, dealloc @ 0xb53fc, startOpenAnimation @ 0xb5480, endOpenAnimation @
//  0xb5558, startCloseAnimation @ 0xb5570, endCloseAnimation @ 0xb5640,
//  downloaderFinished: @ 0xb5678, downloaderProceed: @ 0xb57cc,
...
//  startRemoveFriendHttp @ 0xb5be8, isEnabled @ 0xb5c98). Objective-C++ for the
//  neSceneManager device check and neEngine SE.
//
//  Layout note: the ~4KB init lays every element out against a global scale
//  (1.0 phone @0xb5224 / 2.0 pad @0xb5228) via NEON vector multiplies. The
//  device-branched offset globals at 0xb4a90.. are recovered exactly
//  (read_memory): DAT_000b4a90=-48.0, b4a94=-50.0, b4a98=72.0, b4a9c=176.0,
//  b4aa0=-80.0 (pad), b4aa4=80.0 (pad). The subview tree, images, text
//  bindings, colours, fonts, tags, the phone/pad chara branch and the score
//  grid (columns per difficulty at x {139,190,242}, digit art
//  frilis_num_{n,h,e}{0-9}) are reproduced exactly. The delBtn/nameLabel frames
//  are device-branched NEON expressions over those offsets and runtime image
//  .size; their exact origin algebra is only partially attributable and is kept
//  structural (flagged inline).
//
...
// Per-difficulty (Normal/Hyper/Extra) digit glyphs; addCntNum composes a
// 3-digit count from them.
    ...
    for (int i = 0; i < 3; i++) {
        ...
        UIImageView *iv = [[UIImageView alloc]
            initWithFrame:CGRectMake(
                              (baseX + dx) * s, y * s, img.size.width * s, img.size.height * s)];
        [iv setImage:img];
        [view addSubview:iv];
        v /= 10;
        dx -= 15;
...
- (instancetype)initWithFrame:(CGRect)frame friendData:(NSValue *)friendData {
    ...
    UIImageView *window = [[UIImageView alloc]
        initWithFrame:CGRectMake((frame.size.width - windowImg.size.width) * 0.5f,
                                 ...
                                 windowImg.size.width,
                                 windowImg.size.height)];
    ...
    if (isFriend) {
        ...
        // Frame @ 0xb482c: y offset = DAT_000b4a90=-48.0 (phone) /
        // DAT_000b4a94=-50.0 (pad), added to a scaled window/delImg-width
        // expression; kept structural (partial NEON attrib).
        ...
        [delBtn addTarget:self
                      action:@selector(startRemoveFriendHttp)
            forControlEvents:UIControlEventTouchUpInside];
    ...
    // Portrait (tappable, tag 0x65 -> FriendListDetailChara). Phone: icon plate +
    // sgc_icon; iPad: the large sugo_chara art loaded from Application Support.
    UIImageView *charaView;
    if (!isPad) {
        UIImageView *plate = [[UIImageView alloc] initWithFrame:CGRectMake(18, 36, 55, 55)];
    ...
    // Frame @ 0xb4e34: device-branched over exact constants 24.0/30.0 and offset
    // globals DAT_000b4a98=72.0, b4a9c=176.0, b4aa0=-80.0 (pad), b4aa4=80.0
    // (pad); kept structural.
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if ([[[touches anyObject] view] tag] == 0x65) {
        ...
        FriendListDetailChara *card = [[FriendListDetailChara alloc] initWithFrame:portrait.frame
...
- (void)commonAlertView:(CommonAlertView *)alertView clickedButtonAtIndex:(NSInteger)index {
    ...
    NSString *body = [NSString
        stringWithFormat:@"uuid=%@&player_id=%@", AppDelegate.appDelegate.uuId, data.playerId];
    _dlRemoveFriend = [[Downloader alloc] initWithURL:[StoreUtil removeFriendURL]
                                             delegate:self
...
- (void)downloaderFinished:(Downloader *)downloader {
    ...
    CommonAlertView *alert = [[CommonAlertView alloc] initWithTitle:@"フレンド解除"
                                                            message:message
                                                           delegate:delegate
