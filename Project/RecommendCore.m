...
@property (nonatomic, copy) RecommendOpenAppliListCallback callbackForOpenAppliList;  // @ 0xfe660 / 0xfe674
...
// @ 0xfc33c — perform [super init] on the shared "RewardCore" serial queue (recommendCoreInitBlock
...
+ (NSString *)baseUrlSsl {
    NSString *env =
        [[NSUserDefaults standardUserDefaults] objectForKey:@"ApplilinkRecommend.env"];
...
- (void)startWithCountryCode:(NSString *)countryCode
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"ApplilinkRecommend.postInstalled"]) {
        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            RecommendAdId *adId =
                [[RecommendAdId alloc] initWithCountryCode:countryCode categoryId:categoryId];
            NSError *loadError = nil;
            NSDictionary *record =
                [adId getWithCountryCode:countryCode categoryId:categoryId error:&loadError];
            ...
            [self postApplicationInstallWithAdIdFrom:adIdFrom
                ...
                if (error == nil) {
                    [adId deleteWithCountryCode:countryCode categoryId:categoryId error:nil];
                    [[NSUserDefaults standardUserDefaults]
                        setBool:YES forKey:@"ApplilinkRecommend.postInstalled"];
                }
                if (callback) {
                    callback(error);
                }
...
// @ 0xfcc0c — wrap the raw fetch (recommendCoreLoadOpenAppliList @ 0xfcc78) that filters the
// installed apps, lazily builds the web-view controller and loads /ad/external/index.php.
...
// Shared error mapping used by both response handlers (recommendCoreAppliListResponseHandler /
// recommendCorePostResponseHandler): translate the server error_code/kind into a localized
// RewardNetworkError.
- (NSError *)appliListErrorFromResponse:(NSDictionary *)response {
...
- (BOOL)redirectWithRequest:(NSURLRequest *)request {
    ...
    // applilink://ext-app:80 — parse the query for a "default_scheme=" launch and/or an
    // {ad_id_from, country_code, category_id, ad_type} record to persist.
    if (query != nil) {
        NSString *adIdFrom = nil;
        ...
        if (adIdFrom != nil && country != nil && category != nil) {
            RecommendAdId *adId =
                [[RecommendAdId alloc] initWithCountryCode:country categoryId:category];
            NSError *error = nil;
    ...
    // Otherwise launch a scheme URL embedded directly in the "applilink://ext-app:80/<url>" path.
...
// callbackForOpenAppliList / categoryId / lastErrorForOpenAppliList / webViewController /
