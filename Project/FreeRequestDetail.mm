...
//  pop'n rhythmin
//
//  See FreeRequestDetail.h. Reconstructed from Ghidra project rb420, program PopnRhythmin:
//    initWithFrame:friendData:            @ 0xe3170
//    addCntNum:sheet:y:view:              @ 0xe40ac
//    dealloc                              @ 0xe4278  (Ghidra spells it "deallc")
...
//
//  Honesty notes:
//   - Asset names, tags, fonts, colors, texts, animation durations (0.3s, DAT_000e43c8 /
//     DAT_000e44a0), the dim alpha (0.3), the per-difficulty count-sheet layout, the
//     Downloader wiring, and the alert strings are exact (CFString / DAT decodes). The
//     card's *sub-view frames* are reconstructed from heavily float-vector'd decompiler
//     output and are best-effort: the DAT_000e39xx constants they use are decoded
//     (151/368/36/72/176/60/80/212/-80/81 pt) but the exact origin/size arithmetic per
//     label — including the pad-only re-centering passes — could not be recovered cleanly,
//     so a few label frames below are approximate. Structure and behaviour are faithful.
//   - _scaleForPad is 1.0 on phone / 2.0 on pad (DAT_000e40a0 / DAT_000e40a4); button and
//     digit-image frames are multiplied by it.
//   - friendData wraps a FriendListData (DownloadMain.h): playerId / name / charaId and the
//     rank[3][7] + perfect[3] + fullComboOnly[3] tallies drive the count sheet. Its NSString
//     fields are __unsafe_unretained; this overlay only reads them (the owning list holds
//     the array), so no retain/release is issued (ARC).
//   - -downloaderFinished: mirrors the binary's ErrorCode switch exactly; the code->message
//     mapping was verified from the tbb jump table (codes 0/1/2/7 -> the generic retry
//     message, 3/4/5/6/8/9 -> specific messages, a nil JSON body -> the success message).
//   - The friend request POSTs "uuid=%@&player_id=%@&message=%@" (message empty) to
//     +[StoreUtil requestFriendURL], Content-Type "application/json", body UTF-8.
//   - Label 3 (Ghidra cf__, a short JP static string) could not be decoded; @"さん" is a
...
- (instancetype)initWithFrame:(CGRect)frame friendData:(NSValue *)friendData {
    if ((self = [super initWithFrame:frame])) {
        ...
        // Dimmed dummy overlay carrying the request spinner (hidden until a request runs).
        ...
        if (fd.playerId != nil) {
            ...
            [requestBtn setFrame:CGRectMake(151.0f * _scaleForPad, 368.0f * _scaleForPad,
                                            reqImg.size.width, reqImg.size.height)];  // DAT 0xe3948/0xe394c
            [requestBtn setImage:reqImg forState:UIControlStateNormal];
            [requestBtn addTarget:self action:@selector(startRequestFriendHttp)
                 forControlEvents:UIControlEventTouchUpInside];
            [window addSubview:requestBtn];

            UIButton *cancelBtn = [[UIButton alloc] init];
            UIImage *canImg = [UIImage imageNamed:@"fpl_btn_cancel"];
            [cancelBtn setFrame:CGRectMake(36.0f * _scaleForPad, 368.0f * _scaleForPad,
                                           canImg.size.width, canImg.size.height)];   // DAT 0xe3950/0xe394c
            [cancelBtn setImage:canImg forState:UIControlStateNormal];
            [cancelBtn addTarget:self action:@selector(touchedCancel)
                 forControlEvents:UIControlEventTouchUpInside];
            [window addSubview:cancelBtn];
        }

        // Friend character art (clamp charaId to >= 0). Ids <= 29 are bundled; larger ids
        // are downloaded characters read from the Application Support directory.
        ...
        if (!isPad) {
            ...
            UIImage *charaImg;
            if (charaId > 29) {
                charaImg = [UIImage imageWithContentsOfFile:
        ...
        UIColor *textColor = isPad ? [UIColor whiteColor]
        ...
        nameLbl.frame = CGRectMake(28.0f * _scaleForPad, 30.0f * _scaleForPad,
                                   176.0f * _scaleForPad, 60.0f * _scaleForPad);  // approx (DAT 0xe3958/0xe3954)
        nameLbl.adjustsFontSizeToFitWidth = YES;
        [window addSubview:nameLbl];

        // Player-id label (15pt * scale). On pad the frame is re-centred (approximated).
        ...
        [idLbl sizeToFit];
        idLbl.center = CGPointMake(80.0f * _scaleForPad, 212.0f * _scaleForPad + 5.0f);  // approx (DAT 0xe3960/0xe3964)
        ...
        captionLbl.center = CGPointMake(81.0f * _scaleForPad, 212.0f * _scaleForPad + 5.0f);  // approx (DAT 0xe40a8)
        [window addSubview:captionLbl];

        // Per-difficulty count sheet on the card: 4 clear-medal grades (S/AAA/AA/A) x
        // 3 difficulties (N/H/Ex), then a Perfect row and a FullCombo row.
        for (int grade = 0; grade < 4; grade++) {
            for (int diff = 0; diff < 3; diff++) {
                [self addCntNum:fd.rank[diff][grade] sheet:diff y:(0x95 + grade * 0x23) view:window];
            }
        }
        for (int diff = 0; diff < 3; diff++) {
...
// (frilis_num_<n|h|e><digit>) for difficulty row `sheet` (0=N,1=H,2=Ex) at vertical
// position `y`, into `view`. All frames are scaled by _scaleForPad.
- (void)addCntNum:(int)count sheet:(int)sheet y:(int)y view:(UIView *)view {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        UIImageView *iv = [[UIImageView alloc]
            initWithFrame:CGRectMake((kBaseX[sheet] - i * 15) * _scaleForPad,
                                     y * _scaleForPad,
...
// @ 0xe44e0 — friend-request response: nil JSON body -> success; else map the "ErrorCode"
// number to a result message. Always drops the downloader + hides the spinner, then alerts.
...
// @ 0xe46a4 — request failed: drop the downloader, hide the spinner, show the network alert.
...
// @ 0xe477c — POST the friend request (once) for this row's player id, revealing the spinner.
- (void)startRequestFriendHttp {
    FriendListData fd;
    ...
    if (_downloader == nil && fd.playerId != nil) {
        ...
        NSString *body = [NSString stringWithFormat:@"uuid=%@&player_id=%@&message=%@",
                          [[AppDelegate appDelegate] uuId], fd.playerId, @""];
        _downloader = [[Downloader alloc]
            initWithURL:[StoreUtil requestFriendURL]
               delegate:self
...
// @ 0xe493c — a touch that ends outside the card (tag != 400) dismisses the overlay.
...
// @ 0xe4278 (Ghidra "deallc") — cancel the in-flight request so no late callback fires into
// a dead overlay. Kept under ARC because it cancels a Downloader; the _dummyView / _downloader
// object releases are ARC-managed.
- (void)dealloc {
    if (_downloader != nil) {
        [_downloader cancel];
    }
