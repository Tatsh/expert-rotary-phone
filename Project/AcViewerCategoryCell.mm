...
// @ 0x1a878 — install the category banner. The banner is centered inside a full-width
// background view on iOS 7+ phones, used directly as the cell background on older phones,
// and added as a horizontally-centered subview on iPad.
- (void)setData:(NSArray *)dataList {
    ...
    if (!isPad) {
        ...
        if (sysVer >= 7.0f) {
            ...
            [baseImg setFrame:CGRectMake((cw - image.size.width) * 0.5f, 0.0f,
