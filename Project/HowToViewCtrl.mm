...
//  pop'n rhythmin
//
//  See HowToViewCtrl.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin. -init, -viewDidLoad and the page-control / scroll interactions
//  (backButtonFunc, pageControlDidChanged:, scrollViewDidScroll:) are
//  decompiled. Some frame origins are NEON-spilled and flagged.
//
...
- (void)viewDidLoad {
    ...
    // The horizontally-paged strip (count pages wide) over the optional
    // background.
    ...
    // In "close-button" mode, hide the back button and add a right-bar close
    // button; with more than one page it stays hidden until the last page
    // (revealed in scrollViewDidScroll:).
...
// dealloc @ 0x83538 — ARC-omitted (released object ivars only: _fileNameArray,
// _scrollView, _pageCtrl, _fromNaviBarImage, _backGroundImage).
