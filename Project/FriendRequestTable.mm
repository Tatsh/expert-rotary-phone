...
//  pop'n rhythmin
//
//  See FriendRequestTable.h. Reconstructed from Ghidra project rb420, program
//  PopnRhythmin:
...
//
//  Honesty notes:
//   - The sent-request list is POSTed to +[StoreUtil getFriendRequestURL] (body
//   "uuid=<uuId>",
//     Content-Type "application/json"). The response's "Send" array is parsed
//     into NSValue-wrapped records boxed with the binary's literal Obj-C
//     type-encoding "{RequestDataStruct=@@@s}", which is layout-identical to
//     FriendRequestCell's FriendRequestDataStruct (playerId / name / date /
//     charaId) — so the boxed values feed -[FriendRequestCell setFriendData:]
//     directly.
//   - The binary builds four parallel NSMutableArrays (ids / names / dates /
//   charaIds) in a first
//     pass, then assembles each record in a second pass; that is behaviourally
//     identical to the direct per-entry assembly used here. Under ARC the
//     struct's NSString* fields are held
//     __unsafe_unretained (see FriendRequestCell.h), so -releaseSendDataArray
//     only unboxes — it matches the sibling -[FreeRequestListViewController
//     releaseFriendList]. (The binary also manually -retain'd/-release'd the
//     boxed id fields; that is a no-op under this ARC port.)
//   - Error alert strings are exact CFString decodes (UTF-16LE): title nil, OK
//   button, and the
//     failure message
//     "通信に失敗しました。\n電波状態の良い場所でやり直して下さい。" (used both
//     when the response carries an "ErrorCode" and on a transport error).
//   - The init frame arithmetic uses the binary's exact constants (verified
//   DAT_000b76xx floats);
//     the iPad table-height base is the messiest part of the original and is
//     reproduced as closely as the decompile allows (see the comment in
//     -initWithStyle:).
//
...
- (instancetype)initWithStyle:(UITableViewStyle)style {
    if ((self = [super initWithStyle:style])) {
        ...
        //   phone = view.height - 239 (DAT_000b7610 -234, minus a 5pt bottom
        //   margin); iPad  = plate.height + 140 (DAT_000b7618). The iPad path never
        //   reads the host view frame in the binary — d9 still holds the plate
        //   height there — so the base is the plate height, not the view height.
        CGFloat tableY = (isPad && isOS7) ? 278.0f : 234.0f;
        CGFloat tableH = isPad ? (plateSize.height + 140.0f) : (viewFrame.size.height - 239.0f);
        ...
        // Background plate (its own height + 100pt of slack). iPad overlays a
        // "fripre_table_font".
        UIImageView *plateView = [[UIImageView alloc] initWithImage:plateImg];
        plateView.frame = CGRectMake(0,
                                     0,
                                     plateSize.width,
                                     plateSize.height + 100.0f); // DAT_000b7948
        if (isPad) {
...
- (void)downloaderFinished:(Downloader *)downloader {
    ...
    if (errorCode != nil) {
        ...
        if (send != nil && [send count] != 0) {
            ...
            for (NSDictionary *entry in send) {
                ...
                if (playerId != nil && name != nil && date != nil) {
                    ...
                    // Binary boxes with the literal encoding "{RequestDataStruct=@@@s}"
                    // (layout- identical to FriendRequestDataStruct).
        ...
        if (_sendDataArray == nil || [_sendDataArray count] == 0) {
            // No sent requests: show the "fripre_empty" placeholder centred in the
            // view and stop the table scrolling.
            CGRect viewFrame = self.view ? self.view.frame : CGRectZero;
            UIImage *emptyImg = [UIImage imageNamed:@"fripre_empty"];
            _lonelyImageView = [[UIImageView alloc] initWithImage:emptyImg];
            _lonelyImageView.frame =
                CGRectMake(viewFrame.size.width * 0.5f - emptyImg.size.width * 0.5f,
